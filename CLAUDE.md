...
Parser auto-detected from extension (the JS/TS family → TypeScript, `.svelte`, `.css`); `--content` and `--stdin` require `--parser svelte|typescript|css`. For TypeScript, `--source-type script|module` selects the parse goal (for `format`, `--content`/`--stdin` only — a path is formatted as a **module, retried as a script** if that parse fails, reporting whichever attempt's error reached further when both fail (the module's on a tie) — except `.mjs`/`.mts`, modules by their own name, which take no retry; `parse` is module-only by default).
