...
`format` writes paths **in place** (only when output differs) and prints changed paths to stdout; `--content`/`--stdin` print to stdout. Directories recurse over the JS/TS family (`.ts`/`.mts`/`.cts`/`.js`/`.mjs`/`.cjs`, all parsed as TypeScript — JSX/TSX out of scope), `.svelte`, and `.css` with gitignore-aware, reproducible discovery (see [Configuration](#configuration); full rules in ./docs/cli.md §Multi-File Formatting); a named file or directory is bounded by the ignore files alone — one a rule excludes is skipped, quietly for a file a `.formatignore`/`.prettierignore` rule excludes and with a warning otherwise (a run whose every argument was such a file exits 0), and the safety nets and build-output heuristic grade no named path — while a named file is held to the extension check first (an unsupported extension is an argument error, for `parse <file>` too unless `--parser` names the grammar). `--list` prints the discovered in-scope files without formatting (path mode only; an empty scope exits 0). Files format in parallel; `--jobs N` overrides the default worker count of `min(logical CPUs, ceil(1.5 × physical cores))` (rationale in docs/cli.md — the work doesn't scale onto SMT siblings), itself held to `4 × logical` and announced when that bites — a blast-radius bound, since an unbounded count takes OS task slots until the refusal, and a refused thread narrows the pool rather than failing the run. Exit codes: 0 clean, 1 would-change (`--check`, which also works with `--content`/`--stdin`), 2 errors; missing path args fail the run upfront, per-file and traversal errors report and continue.
