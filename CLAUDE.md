...
The rule that moves with it today is the **leading-zero numeric literal** — `LegacyOctalIntegerLiteral` (`010`, base 8) and `NonOctalDecimalIntegerLiteral` (`08`), which strict code disallows by *production*: they lex under every mode and are rejected at the point the token becomes a node, where the enclosing code's strictness is settled. Two constructs stay put regardless: the `with` statement is rejected under **every** mode, and the legacy string escapes (`"\7"`, `"\08"`) are **accepted** under every mode — a deferred early error like the rest below.
