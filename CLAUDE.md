...
`format` writes paths **in place** (only when output differs) and prints changed paths to stdout; `--content`/`--stdin` print formatted source to stdout. Directories recurse over `.ts`/`.svelte`/`.css`. Discovery always skips the safety nets (`.git`, `node_modules`, `.hg`/`.svn`/`.jj`). The **format root** (the scope boundary, derived from the argument, never the cwd) is the **repo root** inside a git tree — a hard stop, so `--check` is reproducible — else the **filesystem root**. Inside a repo it honors `.gitignore` (hierarchically, repo-rooted like git), `.formatignore` (hierarchically, repo root down), and a repo-root `.prettierignore` (shadowed by a repo-root `.formatignore`); outside a repo, `.gitignore`/`.prettierignore` are not read and `.formatignore` is honored hierarchically from the filesystem root down (a `~/.formatignore` is global config for loose files). When a `.gitignore` is in scope it is authoritative; otherwise a built-in heuristic also skips hidden directories and `dist`/`build`/`target` (an explicit tsv-layer `!` re-include overrides it). An explicitly named file argument bypasses the ignore files. `--list` prints the discovered in-scope files (one per line) without formatting — a read-only view of what `format` would touch (path mode only; an empty scope still exits 0). Files format in parallel (`--jobs N` overrides the thread count; path mode only). Exit codes: 0 clean, 1 would-change (`--check`, which also works with `--content`/`--stdin`), 2 errors; missing path args fail the run upfront, while per-file and traversal errors report and continue.
