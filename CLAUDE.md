...
Parser auto-detected from extension (the JS/TS family → TypeScript, `.svelte`, `.css`); `--content` and `--stdin` require `--parser svelte|typescript|css`. For TypeScript, `--source-type script|module` selects the parse goal (for `format`, `--content`/`--stdin` only — a path is formatted as a **module, retried as a script** if that parse fails, reporting, when both fail, the module's error if the script retry died on an `import`/`export`/`import.meta` (the file is a module), else whichever attempt's error reached further (the module's on a tie) — except `.mjs`/`.mts`, modules by their own name, which take no retry; `parse` is module-only by default).

`format` writes paths **in place** (only when output differs) and prints changed paths to stdout; `--content`/`--stdin` print to stdout. Directories recurse over the JS/TS family (`.ts`/`.mts`/`.cts`/`.js`/`.mjs`/`.cjs`, all parsed as TypeScript — JSX/TSX out of scope), `.svelte`, and `.css` with gitignore-aware, reproducible discovery (see [Configuration](#configuration); full rules in ./docs/cli.md §Multi-File Formatting); an explicitly named file argument bypasses the ignore files (but not the extension check — an unsupported extension is an argument error, for `parse <file>` too unless `--parser` names the grammar). `--list` prints the discovered in-scope files without formatting (path mode only; an empty scope exits 0). Files format in parallel; `--jobs N` overrides the default worker count of `min(logical CPUs, ceil(1.5 × physical cores))` (rationale in docs/cli.md — the work doesn't scale onto SMT siblings), itself held to `4 × logical` and announced when that bites — a blast-radius bound, since an unbounded count takes OS task slots until the refusal, and a refused thread narrows the pool rather than failing the run. Exit codes: 0 clean, 1 would-change (`--check`, which also works with `--content`/`--stdin`), 2 errors; missing path args fail the run upfront, per-file and traversal errors report and continue.
