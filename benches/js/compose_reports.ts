 ...
 * `mixed_vintage`: the conformance commit differs from SOME perf sibling's.
 */
async function read_conformance_vintage(): Promise<{
	...
	try {
		report = JSON.parse(
			await readFile(`${results_dir}report.conformance.node.json`, 'utf8')
		) as Report;
	} catch {
		return null;
	}
...
if (conformance_vintage) {
	md.push(
		`- \`conformance\` (node, coverage-only): ${conformance_vintage.git_commit ?? 'unknown commit'} @ ` +
			...
			(conformance_vintage.stale ? ' — ⚠ **behind the perf siblings**' : '')
	);
}
md.push('');
if (machine) {
...
if (conformance_vintage?.stale) {
	console.error(
		`⚠ compose: report.conformance.node.json is at ${(conformance_vintage.git_commit ?? '?').slice(0, 8)}, ` +
			'behind the perf siblings — the site publishes both from one directory; re-run ' +
			'`deno task bench:conformance` (or `deno task bench` for everything).'
	);
}
if (mixed_machine) {
	console.error(
