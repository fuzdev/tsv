...
- **`terminators`**, by file count over `tests/fixtures` — three groups, none of them a
  ...
  The boundary-whitespace **class** finding this list used to lead with is closed, and so is
  the selector-span residue that followed it: the CSS parser steps the whole
