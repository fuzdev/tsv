...
└─ The parse fact lives only in a form the input can't hold (a leading BOM with nothing
     load-bearing behind it — the format side strips it, so F1 forbids it in input.*) → pin
     a VARIANT: expected_<stem>.json beside <stem><ext> (P4/S24)
```
...
20. **S23**: the `goal` marker is only valid on a `.ts` / `.svelte.ts` fixture. Only the TypeScript-family parse arms read it — Svelte `<script>` hard-wires Module and CSS has no goal — so a marker beside a `.svelte`/`.css` input is inert while reading as a claim the fixture is graded at Script
21. **S24**: an `expected_<stem>.json` variant parse pin requires its sibling `<stem><ext>` variant — the file whose canonical AST it holds (P4) — and that sibling cannot be an `input_invalid_*` file, whose claim is that neither parser produces an AST. `expected_ours.json` / `expected_svelte.json` are not pins (their stems are reserved)
...
- **N12**: `audit_signature_<suffix>.txt` byte-matches prettier's live chain from `unformatted_ours_<suffix>.*` — the **marker of last resort**, auto-generated, for the outputs no single-form marker reaches:
  ...
  - Written only when **both** hold: prettier's first-pass output matches no documented form (when it lands on one, that hop is pinned by N10's own byte comparison and the rest of the chain by whatever it landed on — F4 for `output_prettier`, N1/N9a/N11a for a variant), **and** no single-form marker can express it — either a chain with two or more distinct intermediates (`prettier_intermediate*_*` pins exactly one unstable pass), or a stable first pass **tsv cannot format** (every variant marker asserts something about `ours(V)`, so all are unreachable). In a README-only fixture, a stable first pass tsv *can* format is left unpinned on purpose, so `fixtures:audit` keeps suggesting the more informative `prettier_variant_*` / `variant_*` / `divergent_variant_*`. In a fixture that documents a stable form, N10 rejects that output unpinned. There the updater writes no signature but fails, naming the marker to add (or the tsv bug), and keeps any existing signature (see N10)
  - Its suffixes are what N10 then treats as documented; a signature a single-form marker later covers fails as `VariantChainSignatureSuperseded`, and one whose source became a prettier fixed point fails as `Collapsed` (pin it as a `prettier_variant_*` / `variant_*` instead). A chain prettier now ERRORS partway along is neither — it fails as a walk failure naming the failing pass, and `fixtures:update:formatted` refuses to regenerate (deleting the signature would silently drop the claim). The signature's ABSENCE is policed by N10's blocking arm (as `audit_signature.txt`'s is by F4b), so deleting one fails validation instead of degrading the claim to a note
