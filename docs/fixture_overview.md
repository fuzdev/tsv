...
└─ The parse fact lives only in a form the input can't hold (a leading BOM — the format side
     strips it, so F1 forbids it in input.*) → pin a VARIANT: expected_<stem>.json beside
     <stem><ext> (P4/S24)
```
