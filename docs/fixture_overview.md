...
- **P4**: `convert_ast_json_string` output is byte-identical to the `Value` path (`to_string(convert_ast_json)`) on the input file — guards the compact-wire fast path's eligibility gates and typed offset translation, which expected.json comparisons (always via `convert_ast_json`) don't exercise
- **P5**: typed-walk parity probes — the same byte-identity check on synthesized probe content, so the typed offset-translation walk (`tsv_ts`'s `translate_typed.rs`, which enumerates struct fields manually) is exercised on every fixture's AST shapes, not just the few multibyte standalone-TS inputs. `.ts`/`.svelte.ts` inputs get a multibyte variant (prepended multibyte comment shifts all downstream offsets; byte-0 fixtures — hashbang, BOM — are skipped); `.svelte` inputs have their `<script>` contents extracted and run as standalone TS, as-is when multibyte plus a synthesized variant. A probe that fails to parse is an error, not a skip
