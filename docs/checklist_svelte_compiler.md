...
| `<svelte:element>` with a scoping `<style>` in the component | **Refused**: `<svelte:element> in a component with a scoping <style> (CSS scoping not integrated yet)` — a deferral, not an oracle rejection: the oracle scopes a `<svelte:element>` unconditionally (a type/universal selector matches it however its runtime tag resolves) and tsv's element census does not yet hold it as a scoping leaf, so emitting it in a styled component would under-scope. Refused rather than MISMATCH (CSS integration is the next sub-slice). |
