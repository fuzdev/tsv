...
- a **fallback that reads a `$derived` binding** (`$bindable(d)`) refuses via `read of derived binding {name}` — the fallback is a borrowed `$props()` pattern default (a script position), not a template value position, so the value-walk never rewrites it to `d()`;
