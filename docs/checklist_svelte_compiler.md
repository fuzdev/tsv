...
| ⚠️ **known bug (refusal-contract gap)**: the oracle's phase-2 directive-validity errors — `transition_duplicate` (two same-direction transition-family directives), `transition_conflict` (`in:`/`out:` alongside `transition:`), `animation_invalid_placement` / `animation_missing_key` / `animation_duplicate` (`animate:` outside an immediate keyed-`{#each}` child) — are not yet mirrored: these oracle-rejected combinations currently **compile** instead of refusing | a bug — refusal owed |
