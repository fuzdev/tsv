...
- **Refused**: `destructuring a $state declarator` / `destructuring a $state.snapshot declarator` (a separate temp-destructure lowering, out of scope). A destructured **`$derived` / `$derived.by`** now compiles (see above); its residual `destructuring a $derived declarator` / `$derived.by declarator` refusals cover only the multi-declarator and exotic-key corners
