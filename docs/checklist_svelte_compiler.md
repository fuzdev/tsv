...
- **Comments carry** alongside both `$$slots`/`$$events` injections (rest element, non-destructured): the injected pattern properties are zero-width at the rest element's (or the `props` identifier's) start (`script_props.rs::shorthand_pattern_prop`), so the neighbouring properties' comment windows stay the authored gaps.
...
- **Comments carry alongside a `$$slots` reference**: the injected `const $$slots = $.sanitize_slots($$props)` is zero-width at the component body's start (`transform_server.rs::build_sanitize_slots_decl`), so it opens no comment window over the script.
...
A field whose **whole** argument is a **lone reactive-binding identifier** — a store read `field = $state($count)` or a `$derived` binding `field = $state(d)` (`$state.raw` too) — **refuses** (`class-field $state with a lone store/$derived argument (the oracle keeps it bare)`). The oracle keeps such a lone read **bare** in the unwrapped field (`x = $count` / `x = d`), NOT feeding it through the subscription / derived-call pass — but tsv's store rewrite descends into class bodies unconditionally and would rewrite the kept argument to `$.store_get(…)` / `d()`, a corpus-invisible MISMATCH. A narrow, safe over-refusal keyed on exactly "would a rewrite touch this lone identifier?" (`store_read_base` + `store_names` / `derived_names`) — mirroring the two rewrites' DIFFERING escaped handling: the STORE branch DECODES an escaped `$`-identifier (as `store_rewrite`'s `store_base` does — else an escaped lone store argument would slip the refusal and get subscribed where the oracle keeps it bare), while the DERIVED branch stays plain (an escaped derived read is refused by the guard, never rewritten). A **compound** argument (`$state($count + 1)` → `$.store_get(…) + 1`, `$state(d + 1)` → `d() + 1`) and a **plain-variable** argument (`$state(n)` where `n` is a plain `$state`/prop) still compile at parity.
...
The identifier name is DECODED at every store-detection site — a plain `$name` is its span slice, a unicode-escaped `$`-identifier (`$count` written `$count`) reads its decoded `&'arena str` — so the escaped spelling reaches parity exactly as the plain one (the oracle decodes `node.name` and treats them identically): the template read (`template_value::bare_store_read`), the script read/write/update rewrite (`store_rewrite::store_base` + its `pattern_targets_store` destructuring detector), the `uses_stores` emission gate (`needs_context`), and the class-field lone-argument refusal (`script_rewrite::is_lone_reactive_binding`). A `$name` whose base is NOT a binding is `global_reference_invalid`; the plain form refuses, and the escaped form (whose name the guard's span-identity `dollar_identifier_name` can't read) is a narrow residual it emits verbatim — a separate escaped-identifier gap in the rune guard, not this store lowering.
...
- **Supported (parity)**: an import/export **specifier** whose name side is a string `Literal` emits **verbatim, alias included** — `export { x as 'notdefault' }`, `export { 'a-b' as notdefault } from 'y'`, `export { 'a-b' as 'c-d' } from 'y'`, `import { 'a-b' as loc } from 'y'` all round-trip unchanged. esrap writes the `as` clause whenever the two name sides differ, comparing `Literal` **values** as well as `Identifier` names (its `same_module_name`), so the alias survives and no compiler-side transform is needed. `export * as 'str' from 'y'` keeps its name for the same reason (an `ExportAllDeclaration` carries a single name, never a pair). ⚠️ **Open in `tsv_svelte`'s printer**: the identifier **self-alias** `export { x as x }` / `import { x as x }` (both sides plain identifiers, *equal* name, distinct spans) is a separate gap in the other direction — `same_module_name` reports those EQUAL, so esrap collapses them to `export { x }` / `import { x }` while tsv's printer, keyed on span inequality, prints the alias. A **corpus-absent MISMATCH** (the sibling of "esrap ignores the ESTree shorthand flag"), for the maintainer to close behind the `canonical` flag.
...
| `{ a: a }` / `{ a: a = 1 }` (a same-name longhand `init` property, literal or pattern — valid JS) | collapsed to the shorthand `{ a }` / `{ a = 1 }`: esrap's `Property` visitor prints any non-computed `init` property whose key and value (or the value's `AssignmentPattern.left`) are Identifiers of the same name as its value alone, after Svelte's transforms, so a value those rewrite (a `$derived` read, a store) expands back to longhand. A comment in the dropped `a:` gap refuses through the erased-region window |
...
- **Supported**: comments alongside template blocks (`{#if}`/`{#each}`/`{#await}`/`{#key}`/`{@const}`), a component invocation, `{#snippet}`/`{@render}` (hoisted or body-local), expression-valued attributes (`class={c}`, `style:` / `class:` / `bind:` directives, `{...spread}`), hoisted imports (a comment before/between/after imports relocates down to lead the first surviving statement, as the oracle does), and a **`$derived(e)`/`$derived.by(f)` declarator** (the synthetic `$.derived(…)` steals the replaced init's host span — `build.rs::member_call_at` — and its thunk anchors on the borrowed body — `build.rs::thunk_on_body` — so a following script comment flows to the next statement instead of being swept into the derived slot).
...
- **Refused**: `comment after the last script statement in a template that emits a nested block (the oracle drops it)` — the oracle's printer walks one comment index; opening a block with no source `loc` resets it to the end, discarding every comment not yet written, while opening a block that has a `loc` re-seeks that index absolutely and so can move it backward. A loc-less block therefore annihilates the index and the next loc-bearing one recovers it — which is how the comment survives the component body (that block is assigned the instance script's `loc`, and a context-wrapped component wraps it in a fresh loc-less block, so the wrapper annihilates and the inner block seeks back). A template block gets no such recovery, so the comment vanishes from the oracle's output while tsv keeps it — a DROP, which the parity bar grades. The scan (`script_comments.rs::template_emits_nested_block`) asks only whether a synthetic block exists anywhere — `{#if}`/`{#each}`/`{#await}`/`{#key}`/`{#snippet}`, a special element, or a component with children — not whether one is reached before the comment would flush, so it over-refuses the common case where a loc-bearing head expression (an `{#if}` test) flushes the comment first, and likewise the special elements that emit no block at all (`<svelte:window>`, `<slot>`). The split is keyed to the pinned oracle's `reset_comment_index` behavior (the locked esrap — `LOCKED_TRANSITIVE` in `scripts/check_canonical_pins.ts`) — re-probe it if that pin moves
- **Refused**: `leading comment glued to the <script> line (no newline before it)`
- **Refused**: `multi-line block comment in script (interior-line re-indentation not carried through)` — Svelte's parse strips the comment's start-line indentation from every line of the value (`phases/1-parse/acorn.js`, `get_comment_handlers`) and esrap prints each later line after the emit indent; tsv carries the host text. A `*`-gutter (indentable) comment still carries: `canonicalize_js` rebuilds it from trimmed lines on both sides, so only its first line can differ, and it refuses only when the strip reaches that first line (`/*\t* …` under a tab indent) or the comment holds a `\r` / U+2028 / U+2029 (a line start for the strip's `m` flag, not for esrap's `\n` split). A preserved (non-gutter) comment, or a gutter comment with a blank line, refuses (`script_comments.rs::multiline_comment_carries`)
- **Refused**: `comments with template markup before the script (window ordering)`
- **Refused**: `comment inside a rewritten rune call (a trailing-comma gap or an empty $props() call)` — a comment past a trailing comma in a `$state` / `$state.snapshot` / `$derived` / `$derived.by` call (`$state(1, /* c */)`), or inside an empty `$props(/* c */)` call, whose `$$props` replacement has no comment window: the oracle keeps both, the carry drops them. Every other comment in a rewritten rune call — before the `(`, in a `.raw`/`.by` member gap, anywhere in the argument, a JSDoc cast, a class-field `$state`, a statement `$effect`/`$inspect` — carries: the oracle writes it beside the kept argument (or in the gap a dropped statement leaves), and the carry prints it once. The `$derived(e)` thunk anchors on its borrowed body (`build.rs::thunk_on_body`) so its parameter list claims no comment
- **Refused**: `comment inside a block opened on the line a statement ends (the oracle prints it twice)` — esrap's `body()` flushes, after each statement, every comment starting on that statement's end line and ending before the NEXT statement's end as a trailing comment, so on `let n = 1; if (n) { /* c */ n = 2; }` it writes `/* c */` after `let n = 1;`; opening the `if` block then runs `reset_comment_index`, which seeks back to the already-written comment and writes it again. tsv writes it once. The test (`script_comments.rs::refuse_comment_reflushed_into_block`) is a superset — the innermost block holding the comment, and any statement or class-member end at or before that block's start with no `\n` before the comment — so it only over-refuses (e.g. a comment in a later call argument's block, which esrap's `sequence` bounds by the next argument's start). Instance comments and kept module comments alike (`CommentReflushedIntoBlock`)
- **Comments carry alongside a store reference**: `var $$store_subs;` is zero-width at the component body's start, and each script `$.store_get` / `$.store_set` / `$.update_store` takes the replaced expression's host span with every synthetic leaf zero-width inside it (the `'$name'` key and `-1` spelled through the identifier name channel, since a literal prints its own source slice) — `build.rs::store_get`. `$.store_set`'s store argument sits at the value's start so the call's leading-argument window claims the `=` gap
...
condition, probed against the pinned oracle (the locked esrap — `LOCKED_TRANSITIVE` in `scripts/check_canonical_pins.ts`), is stated precisely. A module
...
mismatch), mirroring the instance-side rules: a preserved multi-line block comment (Svelte strips
and esrap re-indents interior lines; a `*`-gutter one carries), a comment intersecting an erased TypeScript region (emergent stale-span
