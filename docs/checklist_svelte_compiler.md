...
- **Refused**: `rune {name}` — any non-sanctioned rune call (`$effect.tracking`, `$host`, member-form misuse, a rune call in any non-sanctioned position, or a `$bindable` / `$inspect` outside its one sanctioned position — see the `$bindable` and `$inspect` sections above)
