...
and top-level `import`/`export` and `import.meta` are errors (a TypeScript namespace body
keeps its `import`/`export`). Unset, `parse` uses `module`, while
