...
- :nth-child(An+B of S) — Incorrect AST structure; Svelte reads the ` of ` into `Nth.value` (`"2n of "`, from its `REGEX_NTH_OF` terminator) and flattens `S` as sibling simple selectors of the Nth. Per [Selectors 4 §the-nth-child-pseudo](https://drafts.csswg.org/selectors/#the-nth-child-pseudo) the `S` in `:nth-child(An+B [of S]?)` is a nested `<complex-real-selector-list>` scoped to the nth term, so tsv keeps `Nth.value = "2n"` with `S` under a `Nth.selector` field (matcher `nth_of_structure`) — [nth_child_of](../tests/fixtures/css/selectors/pseudo_class/nth_child_of_svelte_prettier_divergence/). A non-ASCII space at the `of` juncture (`of<NBSP>.b`, `2n<NBSP> of`) is the same divergence one character over: Svelte folds the run into the value with the ` of ` (`"2n of<NBSP>"`, its `\s+of\s+` being a JS regex), where tsv's gap absorbs it and the value stays `"2n"` — [nth_nbsp_of](../tests/fixtures/css/selectors/pseudo_class/nth_nbsp_of_svelte_prettier_divergence/); a VT / FF there (`2n + 1<VT>of<VT>.b`) is the same fold with an ASCII member, whose formatting side is the `_prettier_divergence` [nth_control_whitespace](../tests/fixtures/css/selectors/pseudo_class/nth_control_whitespace_svelte_prettier_divergence/). The same nesting applies when `S` is a bare `<number>`/`<an+b>` term (`2n of 123`), which both parsers over-accept as an `Nth` (the `in_pseudo_args` production, parsed like a direct `:is()` arg) — [nth_child_of_number](../tests/fixtures/css/selectors/pseudo_class/nth_child_of_number_svelte_divergence/)
...
The junctures are one entry per position `parseCss`'s two skips occupy: a selector-list
...
`allow_comment_or_whitespace` is a **loop**, so a comment does not end a juncture: whitespace,
comment, whitespace, … is one gap. tsv splits each comment's *disposition* out to its site
(register it, push it as a block child, leave it for the caller), so every such site steps the
boundary run on **both** sides of the comment it consumes — through
`CssParser::skip_boundary_whitespace_registering_comments`, and through the boundary-aware
lookahead (`peek_past_boundary_whitespace`) that predicts the same skip. A lookahead whose
whitespace class is narrower than the skip it predicts is the same defect as one wider than the
lexer's.
...
holds at every CLAIMED juncture. A prelude the parser reads verbatim (`@layer`, `@page`,
`@keyframes`, a condition prelude it cannot structure) keeps its INTERIOR as written, raw
whitespace included, exactly as the member-free spelling does; its two ends — which
`parseCss` trims away — are gaps like any other. This is prettier's answer wherever prettier
keeps the run. Prettier
...
`a /* c */ {`), a `selector()` argument's tail behind a comment, a `@custom-selector` head, among others — and
wherever it drops or relocates the run, tsv keeps the character in place: a content difference tsv will not copy.
...
forward sweep bounded at that run's start takes what a comment or a combinator symbol strands
earlier in it (`a <NBSP>/* c */ b`). Three corollaries the class keeps re-teaching: a gap gate
keyed on COMMENTS blinds the preservation sharing it; a printer scan that locates a part by
stepping trivia owes the boundary class too (one that does not reports the RUN as the part's
start); and a run emitted flush against a NAME glues into it, so every claim whose left
neighbour can be a name puts an ASCII space ahead of the run — flush is safe only against a
`,`, `{`, `)`, `]` or quote. ⚠️ That last one is **five** claims, not one: the attribute
selector's `[name<HERE>]`, a selector list's `,` (`a.x <NBSP>, c`), an explicit combinator's
symbol (`a <NBSP>> b`), a pseudo-argument list's `)` (`:is(a <NBSP>)`), and every interior gap
of the commented attribute rebuild (`[a/* c */ <NBSP>=b]`). Reading it as one was itself a
live bug at the other four, and a bad trade: the drop it replaced still SELECTED, where
`a<NBSP>` matches nothing and is its own fixed point, so no gate can see it.

A fourth corollary runs the other way — a rebuilt head must not keep the run the claim beside
it restores. A printer-facing `str::trim` is Unicode `White_Space`, which agrees with this
class on every member **but `<ZWNBSP>`** (`Cf`, not `White_Space`, yet squarely inside JS
`\s`), so `a { color<ZWNBSP>: red }` and `@container name <ZWNBSP>(…)` kept the character
inside the head *and* emitted it beside — doubling the run on every pass. Both trims ask
`is_boundary_whitespace` now. The lesson generalizes past this pair: a probe that tries one
...
either side of a comment, at a block's tail before its `}`, at every gap of a
`@supports` / `@container` condition prelude, at both ends of every at-rule prelude, at every
structural gap of an `@scope` prelude and in its clauses, at `@custom-selector`'s gaps, at an
`@import` prelude's value gaps, and in a media query list's entries.** A prelude's gaps are
not `allow_whitespace()` junctures to `parseCss`, which reads the prelude raw and trims it;
they are the prelude READER's, which steps a run where the lexer read it as an identifier no
production starts with (or rejected `@scope <NBSP> (.a)` outright), and every such step has
its claim. The one known position that drops the run: the stylesheet's own trailing
whitespace (the outermost gap has no following construct, and a Svelte `<style>` host trims
the island's tail before writing it). Two members of the class
...
- **Current behavior is hard-fail; recovery is the target, not the design.**
  ...
  throw-don't-recover model is inherited from Svelte — but tsv is _stricter_
...
- **each-`as` unwound `end` inside a closing paren** — the same unwind's *offset* half,
  ...
  node the unwind unwraps is parenthesized (`{#each a || (b as A) as item}`,
  `cond ? a : (b as A)`), acorn has already dropped the grouping parens, so the `end` Svelte
  patches onto the head expression stops before the `)`, inside the expression's own
  source. tsv reads the separator directly and ends the head at its `)`. prettier-plugin-svelte
  prints from Svelte's offset and loses the `)` — [conformance_prettier_svelte.md §Svelte:
  each-head parenthesized tail under `lang="ts"`](./conformance_prettier_svelte.md#svelte-each-head-parenthesized-tail-under-langts).
  Pinned by
...
- A `using` / `await using` declaration as the **init of a C-style `for`** — `for (using a = x, b = y; cond; i++)`. A `UsingDeclaration` is a `LexicalDeclaration`, the production `ForStatement`'s init already takes, so the head parses as `let`'s does (declarator list included); the for-in head takes none — tsv rejects it, and acorn does at the pin, though not at `'latest'` (the fixture README carries what that means for its `input_invalid_*` variant). tsc parses it and prettier formats it. acorn reads `using` in a `for` head only from ES2026, one edition past the oracle's `ecmaVersion: 2025` pin — the same acorn accepts it at `'latest'` — so the rejection is the pin, like the two entries above — [for_head_c_style](../tests/fixtures/typescript/typescript_specific/using/for_head_c_style_svelte_divergence/)
- A C-style `for` head's using declaration binding **`of`** — `for (using of = a; ;)`, `for (using of: T = a, b = c; ;)`. The proposal's `[lookahead ≠ using of]` restricts only the for-of `ForDeclaration`, so one token past `of` decides (`=`, `;` or `:` continues a declaration, as tsc reads it) and `for (using of items)` stays a for-of over the identifier `using`. tsc parses it and prettier formats it; acorn rejects it at `'latest'` as well as at the pin, declining `using of` in every `for` head — [for_head_c_style_of_binding](../tests/fixtures/typescript/typescript_specific/using/for_head_c_style_of_binding_svelte_divergence/)
...
- **`export declare` split from its declaration** — `export declare⏎class B {}`, and the same for every head `declare` takes (`function`, `namespace`, `enum`, `const`, `interface`, `type`, `abstract class` — all eight). `declare` carries a `[no LineTerminator here]`, so behind `export` a break leaves `export` with nothing to attach to: **tsc rejects each with TS1128** ("Declaration or statement expected") and prettier rejects them too. acorn instead **welds across the break**, building the very tree it builds for the same-line spelling — one `ExportNamedDeclaration` over a `ClassDeclaration` with `declare: true` — so the line terminator simply vanishes. A tree that silently discards a break both other oracles treat as fatal is worse than no tree, the same call made for the decorated sibling below; the check sits once at the post-`declare` dispatch so the eight heads cannot drift apart — [declare/export_line_break](../tests/fixtures/typescript/typescript_specific/declare/export_line_break_svelte_divergence/). Only the modifier→declaration gap is restricted: `export⏎declare class B {}` stays valid (`export` carries no such restriction), as does the statement-path `declare⏎class B {}`, where `declare` demotes to an expression statement — both pinned in [declare/line_break](../tests/fixtures/typescript/typescript_specific/declare/line_break/). The `export abstract⏎class` and `export declare abstract⏎class` spellings are rejected by acorn too, so they ride as ordinary `input_invalid_*` files rather than divergences. acorn welds the same way one position over, at `export default abstract⏎class Base {}`, where tsc and prettier instead read two statements (`export default abstract;` then `class Base {}`) — the `[no LineTerminator here]` its `async` neighbour already honored. tsv follows tsc and **demotes** rather than rejecting there, since `abstract` is a perfectly good default-exported identifier; so the bare `export default abstract;` parses too, which an unconditional demand for `class` would refuse. The AST difference is reachable only from an input that is nobody's fixed point, so it is pinned as a formatting claim instead — the `unformatted_asi` variant of [abstract/export_default_line_break](../tests/fixtures/typescript/declarations/class/abstract/export_default_line_break/), which both formatters normalize to the two-statement form, with the trees themselves asserted directly in [class_modifier_line_break.rs](../tests/class_modifier_line_break.rs) — a variant tsv parses differently from the canonical parser has no pin shape ([fixture_overview.md §P4](./fixture_overview.md)). **Upstream candidate**: acorn-typescript — `isAbstractClass`'s `this.lookahead().type === tt._class` omits the `hasPrecedingLineBreak` test tsc's `nextTokenIsClassKeywordOnSameLine` makes, where its `async` neighbour in the same `parseExportDefaultDeclaration` already honours the restriction
- **An optional call with type arguments after an argument-less `new`** — `new f?.<T>(x)`. The `?.` can extend neither the callee of `new` nor the argument-less `new f` behind it, so **tsc rejects with TS1209** ("Invalid optional chain from new expression"), as prettier does in a `<script>` body; in a template prettier accepts it and prints `{new f()?.<T>(x)}`, inventing the argument list as acorn does. acorn-typescript accepts: inside a `new` callee its `parseSubscript` stops ahead of a `?.<` instead of refusing it, so the optional call attaches to the finished `new f` outside, and the tree is `new f()?.<T>(x)`'s — an argument list the author never wrote, which tsc calls an error. Without the type arguments (`new f?.(x)`) acorn refuses the same position ("Optional chaining cannot appear in the callee of new expressions"), and tsv rejects both — [new/callee_optional_call_type_args](../tests/fixtures/typescript/expressions/new/callee_optional_call_type_args_svelte_divergence/). Its mirror, type arguments first (`new f<T>?.(x)`, `new a.b<T>?.()`), every parser rejects, so it rides as the `input_invalid_*` files of [new/callee_type_args_follow](../tests/fixtures/typescript/expressions/new/callee_type_args_follow/), beside the valid spellings that settle the `new` first (`new f<T>()?.(x)`, `(new f<T>)?.(x)`) and the line-break reading that binds the list to the callee instead (`new f<T>⏎[x](y)` constructs `f<T>[x]`, printed `new (f<T>)[x](y)`).
...
- **A cast target with a default, in a no-declaration `for`-in/of head** — `for ([(c as T) = 1] of arr)`. The inner `=` converts under *assignment* rules (a cast target is legal there, and its assertion node survives), then the for-head converts the whole pattern again under *binding* rules, where acorn raises "Unexpected type cast in parameter position". tsc accepts with no diagnostic and prettier formats it, so tsv converts the inner `=` under assignment rules in a for-head too — [cast_target_destructure_default_for_head](../tests/fixtures/typescript/expressions/assignment/cast_target_destructure_default_for_head_svelte_divergence/). The **bare** cast target in the same position is the entry below
- **A type-assertion target in a no-declaration `for`-in/of head** — `for (a! of arr)`, `for (a.b! in obj)`, `for ([a!, { b: c! }] of arr)`, and the binary-level `for (a as T of arr)` / `for (a satisfies T in obj)` / `for (<T>a of arr)` / `for ([a as T] of arr)`, a JSDoc cast over any of them included. acorn-typescript converts a for-head target in *binding* mode, where every assertion kind raises "Unexpected type cast in parameter position" — the same head with the `!` one member deeper (`for (a!.b of arr)`) it accepts, so its rule admits the compound and refuses the atom. **tsc accepts every form**, parser and checker: `checkReferenceExpression` skips assertions and parens together and asks only that an identifier or an access expression remain, one rule for the `=` left and the for-head alike, and tsc's own suite asserts `for ((g satisfies number) of [10])` with a clean baseline (`compiler/referenceSatisfiesExpression.ts`); in TypeScript's grammar `a!` is a member-expression postfix (tsc's `parseMemberExpressionRest`), the same production that yields `a.b`. ecma262 makes the for-head's target rule a *Static Semantics: Early Error* (the LHS's `AssignmentTargetType` must be simple), the class tsv defers, and tsv already accepted the same assertions as `=` targets. prettier formats every form, printing the binary-level assertions **without** parens in a for head (it strips `(a as T)` to `a as T`, unlike the `=` target it keeps as `(a as T) = 1`), so the paren spellings ride as `unformatted_*` variants. The line stays where tsc's checker draws it: a target that is not a simple reference under its assertions — a call, `this`, and an **optional chain** (`for (a?.b of arr)`, `for (a?.b! of arr)`, TS2780 / TS2781; acorn rejects it too, `Optional chaining cannot appear in left-hand side`) — is a syntax error on both sides, pinned by the fixture's `input_invalid_*` files, and a binding position (`function f(a!)`, `const [a!] = x`) rejects on both sides as before. The head is a `LeftHandSideExpression` position but not an assignment context, so the deferral the non-simple assignment target and the optional-chain assignment target below take does not reach it: the chain rejects here though tsc's parser accepts it and the wire has a shape for it (a `ChainExpression` as `left`, the one an assignment carries) — [cast_target_for_head](../tests/fixtures/typescript/expressions/assignment/cast_target_for_head_svelte_divergence/). The JSDoc cast over a nested assertion (`for ([/** @type {T} */ (a as U)] of arr)`) has no fixture form — prettier's TypeScript parser strips a JSDoc cast's parens in a for head, so no input holds it as a fixed point — and is pinned by [`tests/nonsimple_assignment_target.rs`](../tests/nonsimple_assignment_target.rs) with the optional-chain seal (`for ((a?.b).c of arr)` is an ordinary member target)
...
- **An `export` or `import` inside a namespace or ambient-module body at `Goal::Script`** — `namespace N { export const a = 1; }` (a decorated `export class` there too), a `module M` body, `declare namespace D { export const c: number; }`, `declare module 'a' { import x from 'b'; }`. The goal gate is a rule about the *file's* top level: a namespace or module body is TypeScript's own module-item context at either goal, and tsc decides whether a file is a module from its top-level statements alone (`isFileProbablyExternalModule`), so a file whose exports all sit inside namespaces is a script, parsed with no diagnostic. tsv's two module-item gates fire at the `Program` top level for that reason, and a namespace body's `export` is no proof the file is a module (the format fallback reads that proof). acorn-typescript's `tsParseModuleBlock` reads the body as top level (`parseStatement(null, true)`), where base acorn's `sourceType` check fires before the plugin's own reading is consulted — the same slip as import-equals above. `import.meta` is not a module item and stays a gate in every position — [namespace_body_module_items](../tests/fixtures/typescript/script_goal/namespace_body_module_items_svelte_divergence/)
...
- A **non-simple assignment target** — a call (`foo() = bar`, `foo() += 1`), a literal (`1 >>= 2`), `this` (`this = x`), a `new` expression (`new a() = 1`), or an instantiation expression (`f<T> += c`, `a.b<T> -= c`; a `+=` / `-=` after the type arguments is its own token, not the `+` / `-` ahead of which tsc and acorn read `<…>` as a comparison chain). The production is `LeftHandSideExpression = AssignmentExpression`; the "is it assignable?" refinement (`AssignmentTargetType`) is an early error layered on top, which tsv defers, so every one parses and prettier formats every one. acorn enforces it (`Assigning to rvalue`). A **parenthesized** left that is not a `LeftHandSideExpression` bare is the same class — `(-a) = 1`, `(a++) += 1`, `(await a) = 1`, `(a + b) = 1`, `(a ?? b) = 1`, `(a ? b : c) = 1`, `(() => b) = 1`, `(yield a) = 1`, `x = (function () {}) = 1`, whole or as a destructuring default's target (`[(-a) = 1] = x`): a `ParenthesizedExpression` is a `PrimaryExpression`, so the production derives it, and tsc's parser accepts every one. tsv parses them and keeps the pair (prettier strips it, a cataloged divergence: [conformance_prettier_ts.md §TypeScript](./conformance_prettier_ts.md#typescript), "Non-LHS assignment target parens") — [operator_target_paren](../tests/fixtures/typescript/expressions/assignment/operator_target_paren_svelte_prettier_divergence/), [assignment_tier_target_paren](../tests/fixtures/typescript/expressions/assignment/assignment_tier_target_paren_svelte_prettier_divergence/), [function_target_paren](../tests/fixtures/typescript/expressions/assignment/function_target_paren_svelte_prettier_divergence/). **Bare, the same operators are a grammar error, not the deferred class**: `-a = 1`, `typeof a = 1`, `++a = 1`, `a++ += 1`, `await a = 1` (so `await using [a] = x`, which reads as `await (using[a]) = x`), `a + b = 1`, `a ?? b = 1` — and a destructuring default's target, since the element is an assignment before any cover reparse (`[-a = 1] = x`, `({ a: -b = 1 } = x)`) — have no `LeftHandSideExpression` on the left, so no production derives them; tsc's parser (TS1005), acorn and prettier reject every one as written, and so does tsv (acorn skips the check for a compound operator inside a paren or a call argument list, and accepts `g(a + b >>= c)` there — the entry below). So does a bare type assertion over a non-simple operand (`-a as T = 1`, `fn() as T = 1`, `<T>fn() = 1`): tsv follows acorn in accepting a bare assertion only over a simple operand (`a as T = 1`, repaired to `(a as T) = 1`), and over any other both oracles reject — the `input_invalid_*` files of [operator_target](../tests/fixtures/typescript/expressions/assignment/operator_target/) and [cast_target](../tests/fixtures/typescript/expressions/assignment/cast_target/). The deferral does **not** reach a no-declaration `for`-in/of head — that is a `LeftHandSideExpression` position but not an assignment context, so a non-simple target there stays a parse error in tsv as in prettier. Nor does it reach the two shapes with no faithful reprint, which tsv rejects as acorn does: a compound operator covering a default (`[a += b] = xs` — the pattern node has no slot for the operator, so converting would delete it) and a parenthesized assignment as the *whole* target (`(a = b) = 1`, `for ((a = b) of xs)` — target conversion turns the parenthesized `=` into an `AssignmentPattern`, a node acorn's grammar has only as a pattern child, so no assignment is left for a kept pair to wrap; a conditional, arrow or `yield` is never converted and survives as itself); both are the fixture's `input_invalid_*` siblings — [nonsimple_target](../tests/fixtures/typescript/expressions/assignment/nonsimple_target_svelte_divergence/)
- A **compound assignment to a binary expression inside a paren or a call argument list** — `g(a + b >>= c)`, `g(a < b >>= c)`, `(a < b >>= c)`, and the glued type-argument spellings that read as one: `g(f<T>>= c)` is `g(f < T >>= c)` and `x = (f<A<B>>=c)` is `x = (f < A < B >>= c)`, since the longest punctuator swallows a `>` glued to the would-be close (tsc's `reScanGreaterToken`), so no list closes. The left is not a `LeftHandSideExpression`, so no production derives the assignment (ecma262 §13.15) — the grammar error of the bare operator target in the entry above, not its deferred early error — and tsc's parser rejects every one (TS1005). acorn-typescript accepts them, building an `AssignmentExpression` over a `BinaryExpression` left: a gap in its own check, not a reading. It validates an assignment's left only outside a paren or a call argument list; inside one the list may still be an arrow's parameters (`maybeInArrowParameters`), so it skips the `checkLValSimple` a compound operator gets and never runs it once the list is not an arrow. The window reaches everything nested in the list until a function body resets it — an object value, an array element, a template hole, a spread, an arrow-parameter default, `async (…)`, `g?.(…)` — while outside it (a bare statement, `x = { k: a + b >>= c }`, an arrow body) acorn rejects the same form, as it rejects the `=` operator inside the window, whose left it converts to a pattern. tsv rejects in every position (`Invalid left-hand side in assignment`), so each fixture is a `tsv_rejects.txt` over-rejection graded against Svelte's live parse — [call_arg_additive](../tests/fixtures/typescript/expressions/assignment/operator_target_compound_paren_list/call_arg_additive_svelte_divergence/), [call_arg_relational](../tests/fixtures/typescript/expressions/assignment/operator_target_compound_paren_list/call_arg_relational_svelte_divergence/), [call_arg_glued_close](../tests/fixtures/typescript/expressions/assignment/operator_target_compound_paren_list/call_arg_glued_close_svelte_divergence/), [paren_relational](../tests/fixtures/typescript/expressions/assignment/operator_target_compound_paren_list/paren_relational_svelte_divergence/), [paren_glued_nested_close](../tests/fixtures/typescript/expressions/assignment/operator_target_compound_paren_list/paren_glued_nested_close_svelte_divergence/)
- A **`>`-led compound assignment to an instantiation expression** — `f<T> >>= c`, `f<T> >>>= c`. tsc's **grammar** takes a type argument list in an expression only where the next token can follow one (`canFollowTypeArgumentsInExpression`), and a `>`-led token never can — the close and the `>` would be ambiguous with a re-scanned `>>` — so there is no instantiation to assign to, and the `<…>` falls back to a comparison with no operand (TS1109). That is a production refusal, not the deferred early error of `f<T> += c` in the non-simple-target entry above, so tsv rejects (`Expected expression, found '>>='`). acorn reads the list and refuses the assignment only by its target check, so the same window as the entry above lets it through: outside a paren or a call argument list both parsers reject (the `input_invalid_*` files of [instantiation_operator_follow](../tests/fixtures/typescript/typescript_specific/generics/instantiation_operator_follow/), which also carry the glued spellings — `f<T>>= c`, `f<A<B>>=c`, `a < b >>= c` — that read as a comparison assigned to), and inside one acorn accepts, building a `TSInstantiationExpression` left — [instantiation_shift_assign_call_arg](../tests/fixtures/typescript/typescript_specific/generics/instantiation_shift_assign_call_arg_svelte_divergence/). A same-line **`/=`** refuses the list by the same rule's other half: tsc takes a list only before a token that cannot start an expression, and `/=` can, as the head of a regular expression literal. So `f<T> /= c` is `f < T >` followed by an unterminated literal (TS1161), which tsv rejects (`Unterminated regular expression literal`) where acorn accepts the instantiation target inside a call argument list — [instantiation_divide_assign_call_arg](../tests/fixtures/typescript/typescript_specific/generics/instantiation_divide_assign_call_arg_svelte_divergence/) — and where the literal does close, `g(f<T> /= c/g)` is the comparison `(f < T) > /= c/g` to tsc and to tsv, while acorn builds `f<T> /= (c / g)` there (outside the window acorn reads the same instantiation and rejects the bare line — `f<T> /= c/g`, `x = f<T> /= c/g`, `Assigning to rvalue` — where tsc and tsv still read the comparison): an AST-shape divergence on a spelling tsv never prints (it keeps the relational-chain pair, `g((f < T) > /= c/g)`, which acorn reads as the comparison too), so it rides as an `unformatted_ours_*` variant of [relational_chain_type_arg_parens_regex_follow](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_regex_follow_prettier_divergence/) and tsv's tree is pinned in `tests/nonsimple_assignment_target.rs`. Past a line break tsc takes the list (`f<T>⏎/= c` is an instantiation assigned to, the deferred class), so the printer, which joins the line, prints `(f<T>) /= c`. Only a spaced lone `>=` is an instantiation follow to acorn-typescript (`f<T> >= c` is `(f<T>) >= c`); tsv keeps that reading and the printer repairs it. Parenthesized, the target is a `LeftHandSideExpression` again — `(f<T>) >>= c` is the deferred class, acorn enforcing its early error outside the window — and tsv keeps the pair, which prettier strips to the bare spelling tsc's grammar refuses ([conformance_prettier_ts.md §TypeScript](./conformance_prettier_ts.md#typescript), "Instantiation expression parens") — [instantiation_target_paren](../tests/fixtures/typescript/expressions/assignment/instantiation_target_paren_svelte_prettier_divergence/)
- An **optional chain as an assignment target** — `a?.b = c`, `a?.[0].c += d`, `a?.b ??= c`, a destructuring element (`[a?.b, ...a?.c] = x`, `({ x: a?.b } = y)`, `({ ...a?.b } = y)`) or default target (`[a?.b = 1] = x`), and the non-null spellings `a?.b! = c` / `a?.b!.c = d`. ecma262 gives `LeftHandSideExpression : OptionalExpression` an `AssignmentTargetType` of *invalid*, and the assignment, logical-assignment and destructuring early errors turn that into a Syntax Error — the same static-semantic class as the non-simple target above, so tsv defers it; **tsc's parser accepts every form** (the rejection is its checker's — TS2779, and TS2778 for the object-rest target) and prettier formats every one. acorn enforces it (`Optional chaining cannot appear in left-hand side`). The wire carries the target as the `ChainExpression` acorn builds for the same chain in a read position — as the whole `left` or as the pattern element. A redundant paren around the chain (`(a?.b) = c`) strips on both formatters. A paren **after** the chain seals it: `(a?.b).c = d` is an ordinary member target that acorn accepts too, so it is a plain fixture, [optional_chain_sealed_target](../tests/fixtures/typescript/expressions/assignment/optional_chain_sealed_target/). The deferral does not reach a no-declaration `for`-in/of head (`for (a?.b of xs)`), which rejects on both sides — see the type-assertion for-head entry above — [optional_chain_target](../tests/fixtures/typescript/expressions/assignment/optional_chain_target_svelte_divergence/)
- A **non-simple update operand** — an optional chain (`a?.b++`, `++a?.b`, `a?.b!--`), a call (`foo()++`, `--foo()`), or a literal (`1++`). The productions are `LeftHandSideExpression ++` / `++ UnaryExpression`; ecma262 §13.4.1 layers "It is an early Syntax Error if the AssignmentTargetType of … is *invalid*" on top, the same static-semantic rule as the two assignment-target entries above, so tsv defers it and the node reprints faithfully (`UpdateExpression.argument` carries any expression). **tsc's parser accepts every form** (its checker rejects: TS2777 / TS2357). acorn enforces it (`Optional chaining cannot appear in left-hand side` / `Assigning to rvalue`), and so does prettier's `typescript` parser, so the fixture is a prettier divergence too, cataloged in [conformance_prettier_ts.md §Prettier rejects valid input](./conformance_prettier_ts.md#prettier-rejects-valid-input) — [update_nonsimple_target](../tests/fixtures/typescript/expressions/unary/update_nonsimple_target_svelte_prettier_divergence/)
...
What separates `let`'s two readings is a lookahead, not a rule: `let` heads a declaration exactly when a binding name, `{` or `[` follows (acorn's `isLet`). The one grammatical bar is `ExpressionStatement`'s `[lookahead ∉ { …, `let` `[` }]`, so `let [` can never begin an expression statement — `let[0] = 1` stays a syntax error (a declaration with an invalid array binding pattern, tsc TS1181) even though `let.x = 1` parses.

The same lookahead settles the two positions where the `let` is not statement-initial:

- a **for-head** carries that same restriction on its init, and `[lookahead ≠ `let` `[`]` on the for-in form, so `for (let in o)`, `for (let; ;)`, `for (let = 3; ;)` and `for (let.x in o)` are expression heads — pinned by [script_goal/let_reference_for_head](../tests/fixtures/typescript/script_goal/let_reference_for_head_prettier_divergence/). The for-of form's own `[lookahead ∉ { `let` }]` is wider and keeps `for (let of x)`, `for (let.x of y)` and `for (let[0] of a)` rejected — only `for ((let) … of x)` says those
- a **single-statement position** — a labelled item, an `if` arm, a loop or `with` body — takes a `Statement`, which a `LexicalDeclaration` is not, so the reference is the only reading and ASI closes it: `L: let ⏎ x = 1;` is `L: let;` then `x = 1;` — pinned by [script_goal/let_reference_statement_position](../tests/fixtures/typescript/script_goal/let_reference_statement_position_prettier_divergence/). `let [` survives the expression bar there and so has no reading at all, which is why `if (a) let [a] = b;` rejects

tsc reads both positions the other way, committing `let` to a declaration on the keyword alone, which is why prettier — whose `typescript` parser is tsc's — is no formatting oracle for either; both fixtures carry that divergence. acorn agrees with tsv on every form at `sourceType: 'script'` but one spelling: a following identifier written with a unicode escape (`L: let ⏎ \u0061 = 1;`) — acorn's `isLet` reads a declaration on the bare `\` before it consults the position, so it rejects, where tsv reads the reference and lets ASI close it exactly as for the unescaped twin (unpinned: the spelling appears in no corpus, and at `'module'` it is subsumed by the reference deferral below). At `'module'` acorn rejects the reference outright, enforcing the strict early error it enforces for `let = 1`.
...
**Class-member `static` across a line break**: the same `[no LineTerminator here]` question one position over, and here acorn errs the *other* way — it imposes a restriction the grammar does not have. ecma262's `ClassElement` productions for a static member (`` `static` MethodDefinition `` and `` `static` FieldDefinition `;` ``, sec-class-definitions) carry no `[no LineTerminator here]`, unlike the `AsyncMethod` beside them (`` `async` [no LineTerminator here] ClassElementName ``), so `class C { static⏎c = 3 }` is the **static field `c`** — one member. **tsc agrees** (one member carrying `StaticKeyword`, empty `parseDiagnostics`) and gives `static` its own arm in `nextTokenCanFollowModifier` for exactly that reason, skipping the `nextTokenIsOnSameLineAndCanFollowModifier` its contextual-modifier siblings take — the `get`/`set` arm beside it is exempt the same way. **prettier agrees** through all three parsers its own `js/classes/multiple-static.js` test lists, whose single snapshot prints the four-line `static static static a() {}` body as `static static;` + `static a() {}`, byte-for-byte tsv's output. **plain acorn agrees too** — it is acorn-*typescript* that regresses it, by folding `static` into the `tsParseModifiers` loop it wrote for the TypeScript modifiers (its own comment: "modifiers not handled by @babel/parser itself. And `static`") and applying that loop's uniform `tsTokenCanFollowModifier` → `!hasPrecedingLineBreak()` guard to it, so it ASI-splits into two members. `static` is the **sole** disagreement in the family: over `readonly`, the three accessibility keywords, `declare`, `override`, `abstract`, `accessor` and `async`, tsv, tsc and acorn all demote across the break. tsv follows the spec, tsc and prettier, so the modifier is deliberately not line-break-guarded in `parse_class_member` where its siblings are. Reachable only from an input that is nobody's fixed point, so it rides the same split as the `export default abstract⏎class` entry above: the formatting claim is the `unformatted_asi` variant of [class/modifier_line_break](../tests/fixtures/typescript/declarations/class/modifier_line_break/), the trees are asserted directly in [class_modifier_line_break.rs](../tests/class_modifier_line_break.rs). **Upstream candidate**: acorn-typescript — `parseClassElement` should keep `static` out of `tsParseModifiers`' allowed list (or exempt it from `tsTokenCanFollowModifier`'s line-break test); plain acorn's own `parseClassElement`, the one it overrides, already reads it right.
...
A canonical-parser rejection is fixturable — `expected_ours.json` + an
`expected_svelte.json` holding `{"error": "failed to parse"}`, in a `_svelte_divergence` dir
(dozens of fixtures do this; [fixture_overview.md](./fixture_overview.md) states it) — so
nothing in this family is documented-only. `import defer`'s comment handling is fixtured as
...
All corrections exist because of upstream bugs. If fixed upstream, tsv would remove the `_svelte_divergence` suffix, delete `expected_ours.json`, and rename `expected_svelte.json` → `expected.json` — or, for a correction reachable only from a form no fixture can hold as `input` ([fixture_overview.md §P4](./fixture_overview.md)), delete the `tests/*.rs` file that pins its trees instead.
...
- **Module-script comment duplicated onto the instance script.** Svelte parses the `<script module>` and instance `<script>` against one shared `root.comments` array, and the instance parse's `add_comments` walk is not given a fresh queue, so every module-script comment (leading *or* trailing) is also shifted into the instance script's first statement (`instance.content.body[0].leadingComments`) — or, when the instance script holds no statement at all, into the Program's own `trailingComments` (the walk's root special case, which takes every leftover comment). Either way the copies form a **prefix** of the array, ahead of the instance's own comments in source order, so each of the instance's own comments reads one whole prefix later than tsv lists it (the corpus matcher `svelte_instance_comment_duplication` admits exactly that shift and nothing else). tsv keeps each module comment only on the module body.
  - [module_comment_instance_duplication_svelte_divergence](../tests/fixtures/svelte/script/module_comment_instance_duplication_svelte_divergence/) — the first-statement shape
  - [module_comment_empty_instance_duplication_svelte_divergence](../tests/fixtures/svelte/script/module_comment_empty_instance_duplication_svelte_divergence/) — the statement-less shape: the module comment lands at `trailingComments[0]` and the instance's own two comments at `[1]` and `[2]`
...
- **Template-expression comment before a parenthesized subexpression.** Svelte's `parse_expression_at` sets acorn's `preserveParens: true`, so a leading comment before a parenthesized subexpression attaches to the synthetic `ParenthesizedExpression`; Svelte's subsequent `remove_parens` discards that wrapper and its `leadingComments`, leaving the comment only in the root `comments` array. tsv (which has no `ParenthesizedExpression` node, matching Svelte's *final* shape) attaches it to the inner expression. This is template-only — a plain `<script>` parse does not set `preserveParens`, so the same comment attaches in both parsers there. The common real-world trigger is a JSDoc cast `/** @type {T} */ (expr)`.
  ...
  - **The TRAILING side of the same wrapper is a match, and was a tsv drop.** acorn's comment window runs to the end of the trailing-comment run its tokenizer scans past *after the parse*, and the parse ended at the wrapper's `)`; the comment then attaches to the inner expression, since `)` is in acorn's own `/^[,) \t]*$/` trailing gap class, and survives `remove_parens` on the node that replaces the wrapper. tsv anchors that scan on the island's own expression — `internal::JsdocCast`'s span covers the `(`…`)` — rather than on the node the writer emits as the root, which for a cast is the *inner* expression and stops short of the `)`. Anchored there the scan died on that `)`, so every comment past it was filtered out of the window before the walk ran and the attachment had nowhere to land. A **bare** grouping paren still loses it (`{(x) /* c */}`): tsv discards those at parse time and keeps no span to anchor on. That shape is not format-stable — both formatters print `{x /* c */}` — so it cannot be fixtured, and is unreachable from formatted code.
    - [template_expr_paren_trailing_comment_svelte_prettier_divergence](../tests/fixtures/svelte/syntax/comments/template_expr_paren_trailing_comment_svelte_prettier_divergence/) — the trailing match at each cast-root position (attribute value, `{#if}` head, `{@html}`) plus a nested cast as the control; its own leading half is the divergence above, and prettier deletes the cast (and the trailing comment with it) wherever the cast is the island root
