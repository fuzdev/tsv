...
- ⚠️ **Two known gaps in that prelude family.** An escape-spelled `url()` name is not recognized, so its content is not opaque (`@supports (a: u\72 l(x1.50))` → `u\72 l(x10.5)`, a different path — prettier preserves it): the fix wants the ident *sequence*'s extent at that position, which is not the run the emitter walks. And `lowercase_simple_feature_expr` alone does not step escapes, so an escaped `:` in a feature name reads as the name→value separator and the name half-lowercases (`(A\:B: 1px)` → `(a\:B: 1px)`; cosmetic, since feature names are ASCII case-insensitive, and prettier splits the ident instead). Both are pinned by unit tests beside the scanners
