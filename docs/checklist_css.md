...
- A function's **name** is postcss-values-parser's **word** — this layer's oracle — not the spec's ident sequence, so `aa/`, `aa!` and `a[bb]c` are names and their arguments normalize (`aa/(.10)` → `aa/(0.1)`), and so is an escape-spelled one, escapes being word content on both readings (`\66 n(.10)` → `\66 n(0.1)`, `u\72 l("a.png")` → `u\72 l('a.png')`). The exclusions are the bytes the oracle reads across (whitespace, `,`, `:`, `@`, `*`, `+`, a quote, `{`/`}`) and the two soundness refusals below. The value parser's search for the opening `(` stays escape-blind and is sound because of the name rule rather than in spite of it: a `(` reached inside an escape leaves the name ending in a dangling `\`, which is not word content, so the value refuses and prints verbatim (`a\(b(c)` — which is also what prettier emits, by unbalancing; where that escaped `(` *balances*, prettier does read a function and tsv's refusal is a cataloged cost). Prettier normalizes every escape spelling *except* the four whose raw payload byte drives its own tokenizer (`\(`, `\)`, `\"`, `\'`), so `a\28 b(.10)` normalizes there and `a\)b(.10)` does not — [conformance_prettier_css.md §CSS: Values](conformance_prettier_css.md#css-values)
- Three value members that are not operators end every token they touch, each on its own measured extent: an **`@`-word** (`a@b` → `a @b`, `@a(2.50)` → `@a (2.5)` — no function name, so the group beside it is a second member; its content runs through `*`, `+`, `-`, `:`, `#`, `.`, a digit and `[`, so `@a*1.50` and `f(@a:1.50)` are verbatim), a **brace** inside a custom property's value (`--x: a{1.50}c(2.50)` → `a{1.5}c(2.5)`; each brace glued to the block's interior always and outward as authored — the **whole-value** block, `--x: { color: red; }`, keeps its own pair as authored; a non-custom property refuses the brace on both sides, per css-syntax-3), and a **welded `+`** (`+a(2.50)` → `+a(2.5)`, `1.5 / +a` → `1.5 / +a`; an operator everywhere else — `f(+a)` → `f(+ a)`, `a +b` → `a + b`, `f(a, +b)` → `f(a, + b)`). A lone `@` keeps its authored gap where prettier welds it (`@ a`), a prettier divergence (see `conformance_prettier_css.md` → CSS: Values, "Empty `@`-word gap")
- An operator the author **spaced** is a member like a glued one — `1.5 / 2.5`, `1.5 * - 2.5` and `--x: a : b` each put it in a whitespace element of its own, and the separator rule reads its kind off that member. With nothing before it (the value's head, a group's head, after a comma, after another operator) it binds to the member after it (`/2.5`, `-@a`, `-(2.5)`, `-[a]`, `-'x'`, `-+a`, `+(2.5)`, and the same one operator in, where the `-` after the head `+` is the operator itself because it opens neither an ident nor a number of its own — `+-'x'`, `+-(2.5)`, `+-/2.5`, `+-*a`, `+-[a]`, `+-#a`, against `+ -a` and `+-1.5`, where it heads one), **unless the glue would merge the two into one token** — css-syntax-3 §4.3.9 / §4.3.10 over the operator byte and the next member's first code points — where the authored gap stands instead (`- a`, `- 2.5`, `- -a`, `- f(2.5)`, `- \61`, `- é`, `+ 2.5`); prettier welds all of those, a divergence (see `conformance_prettier_css.md` → CSS: Values, "Operator head weld"). The refusal withholds only glue the rule would introduce — an authored `1.5+2.5` is spelled back
- A **top-level `:`** is a value operator in a custom property's value and nowhere else: `--x: a: 1.50` → `a: 1.5` (no space before, one after, both sides normalizing, across whitespace and comma element boundaries alike), while a plain property's `b: c:1.50` stays exactly as authored — prettier's parser throws there, so no form has an oracle. A `url()` argument and an `@`-word keep their `:` as content; a `:` with nothing after it ends the value where prettier strands a space (see `conformance_prettier_css.md` → CSS: Values, "Colon with an empty tail")
