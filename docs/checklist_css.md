...
- A number **merges** with the identifier before it, so a prelude copies the pair verbatim: `x1.50` is the ident `x1` then `.50`, and giving the number its canonical leading zero handed the `0` to the ident (`x10` then `.5` — two different tokens). Prettier states the same rule textually (`(WORD_PART)?(NUMBER)(UNIT)?` returned unchanged when the word part matched). A signed number never merges, so `x+1.50` still normalizes, and neither do `$`/`@`, which are `<delim-token>`s rather than ident code points. ⚠️ **Still open**: a number after a *number* (`1a` + `.50`) or a `#`-token (`#1` + `.50`) merges the same way, but prettier's two prelude paths disagree about them — `@supports` prints value nodes (`1a.50`, `#1.50`, `1.5.50`, `1.5abc`) where `@media` runs the regex (`1a0.5`, `#1.5`, `1.50.5`, `1.50abc`) — so each is its own oracle question and tsv matches neither everywhere. Same shape: `@import`'s media condition under-normalizes against both (`1.50abc`, `1.50PX` unchanged)
