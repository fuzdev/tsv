...
The `u16` is a cached visual width with two sentinel values (`TEXT_WIDTH_HAS_NEWLINE`, `TEXT_WIDTH_NOT_COMPUTED`). Caching is selective: newline-free non-ASCII `SourceSpan` text precomputes its width — it needs grapheme segmentation, which costs far more than the ASCII path, and `fits()` may measure the same text repeatedly — while ASCII `SourceSpan` and all `Static` text stays uncached: `visual_width()`'s ASCII fast path makes re-measuring cheaper than eagerly measuring text that may never reach a `fits()` check. `Pooled` text always precomputes (rare — ~1.4% of text nodes — and the eager width means neither `fits()` nor any other width query ever needs the pool borrowed; only the render loop reads the bytes, through one pool borrow hoisted per render). `Symbol` defers both resolution and width to print time.
