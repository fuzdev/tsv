...
**Lexing — spans, not strings.** Tokens store `u32` byte offsets (`start`, `end`) into the source, never slices or copies — `Token` is a 16-byte POD the byte cursor (`bytes: &[u8]` + `position`) emits and the parser unpacks in registers. The exceptions are deliberate: a string literal allocates its decoded value only when it actually contains escape sequences, held **out-of-band on the lexer** (`Lexer::decoded`, drained via `take_decoded`) so the per-token `Token` stays pointer-free; and each comment allocates its content once.
