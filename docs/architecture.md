...
requirements (`needs_parens.rs`), the bracketed list body (`layout.rs`), comment
printing helpers, and shared analysis utilities.
...
(`=`, `:`, `=>`, `as`, `satisfies`, `extends`, type-parameter `=`, and the Svelte
`{const …}` tag's `=`) is centralized in `tsv_lang::doc::after_operator`, which
exposes the two distinct shapes Prettier uses — and they are **not** interchangeable:
...
**Lexing — spans, not strings.** Tokens store `u32` byte offsets (`start`, `end`) into the source, never slices or copies — `Token` is a 16-byte POD the byte cursor (`bytes: &[u8]` + a byte position) emits. The exception is deliberate: a string literal's decoded value is materialized only when it actually contains escape sequences, decoded into a **reused scratch buffer held out-of-band on the lexer** (`Lexer::decode_scratch`, borrowed via `decoded_str` and copied into the AST arena at receipt) so no per-literal `String` allocates and the per-token `Token` stays pointer-free. Comments are spans too — the token carries a `content_start` and the `Comment` node a `content_span`, recovered from source on demand and never copied. On the parser's hot path (`advance`, plus tsv_css's `peek_kind`) the TS and CSS lexers write the token in place into the parser's slot (`next_token_into`) rather than returning it, since a by-value `Result<Token, ParseError>` return goes through a stack slot the parser would reload and re-scatter; every other caller takes it by value (`next_token`).
