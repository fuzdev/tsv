...
- Arrow body stripped parens (prettier 3.9) → Parens stripped, block comment trailing the body / line comment floated past `;`; tsv preserves the parens to keep the comment in place — [body_paren_comment](../tests/fixtures/typescript/expressions/arrows/body_paren_comment_prettier_divergence/)
...
**Arrow body stripped parens**: `() => (x /* c */)` → Prettier 3.9 strips the parens and trails the block comment on the body (`() => x /* c */`); a line comment is floated past `;` (`() =>\n\tx; // c`). For curried arrows, every level's parens are stripped and the comment trails (`(a) => (b) => z /* c */`). tsv preserves parens to keep comments in place: `() => (x /* c */)`, `(a) => ((b) => z /* c */)`. Same approach as unary expression fix. Each formatter normalizes to its own form (prettier strips parens, tsv preserves them).
