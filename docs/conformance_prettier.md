...
- Frozen `for` init clause holding a bare `in` — emits the slice without the `[~In]` pair, giving a four-clause header (`for (⏎// prettier-ignore⏎ccc = 'aaa' in bbb;⏎;⏎) {}`) that no parser accepts; prettier is idempotent on it only because prettier-plugin-svelte swallows its own TypeScript parser's `SyntaxError` and passes the script through verbatim — [init_clause_prettier_ignore_bare_in](../tests/fixtures/typescript/statements/for/init_clause_prettier_ignore_bare_in_prettier_divergence/)
- Arrow-body leftmost decorated class — drops the pair a concise body needs around a leftmost `@dec class {}` (`() => (@dec class {}).bbb` → `() => @dec⏎class {}.bbb`), at every shape the leftmost rule reaches → output no parser accepts; prettier is idempotent on it only because prettier-plugin-svelte swallows its own TypeScript parser's `SyntaxError` and passes the script through verbatim — [body_leftmost_decorated_class](../tests/fixtures/typescript/expressions/arrow/body_leftmost_decorated_class_prettier_divergence/)
