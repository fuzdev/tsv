...
- Import source/attrs `;`, own-line line comment trailing a `; // 1` → Promoted to lead the next statement (tsv keeps it with the import) — [source_trailing_comment](../tests/fixtures/typescript/modules/imports/source_trailing_comment_prettier_divergence/)
...
**Import source to `;` (own-line line comment)**: a same-line block comment after an import's source / `with {…}` attributes, and a same-line trailing line comment, both trail **after** the `;` in both formatters (`import { a } from './a'; /* c */`, `import { b } from './b'; // 1`) — tsv adopts prettier 3.9 here. The narrow divergence is an **own-line** line comment that *follows* such a trailing line comment (`; // 1` then `// 2` on the next line): prettier promotes `// 2` to lead the *following* statement (a blank line falls before it), while tsv keeps `// 2` attached to the import it was written under (the blank falls after it). `input.svelte` is dual-stable; the `unformatted_ours_*` variants pin the split. (`import =` / `export =` keep a same-line trailing **block** before the `;` in both formatters — operand-attached — while a line comment floats after; those are regular fixtures, not divergences.)
