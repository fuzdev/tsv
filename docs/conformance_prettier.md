...
- Unary parenthesized binary operand trailing block comment (prettier 3.9) → Redundant parens stripped, comment pulled against the operand (`!(x + y /* c */)`); tsv keeps the parens to preserve the comment's association (assignment operand keeps parens in both) — [operand_paren_comment](../tests/fixtures/typescript/expressions/unary/operand_paren_comment_prettier_divergence/)
