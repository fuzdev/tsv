...
**Caveat — this is a semantic divergence, not a cosmetic one.** In checkJs / JSDoc-typed code the parens are part of the cast: `/** @type {T} */ (expr)` asserts the type, whereas `/** @type {T} */ expr` does *not* (TypeScript requires the parentheses for a JSDoc cast). Stripping them therefore drops the assertion. tsv strips to match prettier's standalone TS/JS behavior, which is itself cast-unaware — but prettier-plugin-svelte, the `.svelte` formatter, deliberately *preserves* them, so in `.svelte` the strip both diverges from the canonical Svelte formatter and changes the meaning of a typed component. Preserving the cast parens (matching the plugin) is the semantically faithful behavior.
