...
**Wide inline child own-line**: When an inline child (component or HTML element) is too wide to share a line with the preceding text, Prettier hugs it onto the text line (101+) and breaks it internally — attributes wrap and the closing `>` dangles (the inline content hug). tsv keeps printWidth a hard limit, so the whole child drops to its own line and the preceding word stays hugged on the text line. The break sits at the collapsible space before the child, so it holds regardless of tag-name length (`inline_component_wide_longname_long`), for repeated children in one run (`inline_component_wide_multi_long`), for a multi-attribute component whose own attributes could break (`inline_component_wide_multiattr_long` — it still drops whole, attributes intact), and for special elements (`inline_svelte_element_wide_long`); HTML inline elements produce the identical shape (`inline_element_wide_long`). These fixtures also pin idempotence — both the compact authored form and Prettier's hugged form must normalize to the own-line form in one pass.

**Wide inline content + trailing text**: The mirror of the above for an element whose own _content_ (not its attributes) overflows, followed by trailing text — `<strong>…90 chars…</strong> tail`. Prettier keeps the content on a single over-width line and lets `>…content…</tag` exceed printWidth (the inline content hug again); tsv wraps the content _inside_ the element, so every line stays ≤100 (`inline_wide_content_trailing_long`, covering both `<strong>` with no attributes and `<a>` with a short one). tsv puts the trailing text on its **own line** after the dangled closing `>` — uniformly, regardless of authoring; it does **not** hug trailing text onto a wrapped element's last line. The fill renderer applies one rule — break the separator after a fill item that wrapped at line start — to inline content and CSS value lists alike (no position-aware "hug after a break" special case). Prettier's trailing-text placement is instead **authoring-dependent**: fed a compact `…</tag> tail` source it hugs `> tail` (`prettier_variant_hug`), fed the canonical form (text already on its own line) it breaks — so the fixture's `output_prettier` agrees on the tail and the **content wrap is the sole divergence**. The two `unformatted_ours_*` variants pin tsv's idempotence: single-line and multiline-authored forms both normalize to the wrapped, tail-on-its-own-line form in one pass. A non-terminal text run (followed by another inline element) likewise keeps its own line (`inline_wide_content_text_sibling_long`).
