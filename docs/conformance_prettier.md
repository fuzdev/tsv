...
- CSS URL-shaped custom-property value (`--a: https://a.fuz.dev/b`) — reads the `//` as an inline comment and moves the URL's tail out past the declaration's `;`; the output is not CSS and prettier's own parser throws on it (`Unknown word //a.fuz.dev/b`), so no fixed point exists — [colon_custom_property_url](../tests/fixtures/css/values/operators/colon_custom_property_url_prettier_divergence/)
...
- CSS `-` at an operand position before a byte postcss's word scan swallows but no token production opens (`+-[a]`, `+-#a`, `+-~a`; the full fifteen-byte set is enumerated in the CSS catalog's entry) — not idempotent on its own pass-1 output: from a spaced authoring pass 1 glues the head `-` onto the member (`+- [a]` → `+-[a]`), and pass 2 re-reads the glued text as postcss's single word `-[a]` and spaces the `+` off it (`+ -[a]`, prettier's fixed point). tsv reads the `-` as css-syntax-3 does at that position and holds pass 1's form — [hyphen_word_extent](../tests/fixtures/css/values/operators/hyphen_word_extent_prettier_divergence/)
...
- `style:` value whitespace — collapses every whitespace run in what is a CSS property value, so `'Font   Name'` is a different font-family name; where the attribute list breaks the collapsed run becomes a **line break**, and a raw newline inside a CSS string is a `<bad-string-token>` (Syntax 3 §4.3.5), so the declaration is dropped at runtime. Idempotent on its own output, so nothing reveals it — [value_whitespace](../tests/fixtures/svelte/directives/style/value_whitespace_prettier_divergence/)
...
- Whitespace written only in front of a hoisted section (`<script>`, `<style>`, `<svelte:options>`) glued to what follows, or only inside its travelling comment run (`x <script>…</script>y`, `x<!-- c -->⏎<script>…</script>y`) — deletes the rendered space (`xy`), a content change; idempotent on its own output, so nothing reveals it — [lifted_run_text_whitespace](../tests/fixtures/svelte/script/ordering/lifted_run_text_whitespace_prettier_divergence/), [lifted_run_space_join](../tests/fixtures/svelte/script/ordering/lifted_run_space_join_prettier_divergence/), [lifted_run_comment_whitespace](../tests/fixtures/svelte/script/ordering/lifted_run_comment_whitespace_prettier_divergence/), [lifted_run_blank_inside](../tests/fixtures/svelte/script/ordering/lifted_run_blank_inside_prettier_divergence/)
...
- TS instantiation parens (`(x ? y : z)<T>`) — strips parens, semantic change — [instantiation_parens](../tests/fixtures/typescript/typescript_specific/assertions/instantiation_parens_prettier_divergence/); the class-operand / `export default` case (`export default (class {}<T>)`) is worse still (strips → a class _declaration_) — [export_default_instantiation](../tests/fixtures/typescript/modules/exports/default_wrappable_leftmost_operators/instantiation_prettier_divergence/); the pair AROUND one (`(fn<T>) + 1`, `(fn<T>) >= 1`, `(fn<T>)!`) — strips it too, and WHICH oracle refuses the stripped form splits the class three ways: `fn<T> + 1` re-parses everywhere as the comparison chain `fn < T > +1` (semantic change, printed that way on prettier's own next pass); `fn<T> > 1`, `>> 1`, `>>> 1` and `fn<T>!` no parser accepts, so that output fails to re-parse outright; and `fn<T> < 1` / `fn<T> >= 1` only **tsc** rejects (TS1005 `'>' expected`, TS1109 `Expression expected`) — acorn-typescript, tsv's parse oracle and the parser Svelte itself uses, reads both as instantiation expressions, so prettier throws on its own output (its TypeScript parser is tsc's) where tsv accepts it and puts the pair back. `fn<T> << 1` is the mirror, tsc's instantiation and acorn's reject — [instantiation_paren_follow](../tests/fixtures/typescript/typescript_specific/generics/instantiation_paren_follow_prettier_divergence/); the same pair ahead of a second type argument list (`new (f<T>)<U>(x)`, `(f<T>)<U>(x)`) — stripped, `f<T><U>(x)` is tsc's comparison `f < T > <U>(x)` and `new a.b<T><U>()` does not parse — [instantiation_paren_type_args_follow](../tests/fixtures/typescript/typescript_specific/generics/instantiation_paren_type_args_follow_prettier_divergence/); and the bare `new f<T><U>(x)` / `f<T><U>(x)`, which acorn-typescript (and so Svelte's compile) reads as two type argument lists, prettier rewrites to tsc's comparison, `new f() < T > <U>x` / `f < T > <U>x`, where tsv prints them as written — [callee_type_args_second_list](../tests/fixtures/typescript/expressions/new/callee_type_args_second_list_prettier_divergence/)
- Relational chain printed into a type-argument list — the dual of the entry above, at the same `canFollowTypeArgumentsInExpression` seam: a comparison chain whose `<`…`>` region would parse as a type-argument list re-lexes as an instantiation the moment the printer touches a line break near it. Removing one does it (`fn<A⏎[T]>(t, u)` is the chain `(fn < A[T]) > (t, u)` only because the newline holds `[T]` to an index — folded, the output is a `CallExpression`), and so does adding one (a break after the `>` settles the list — from an object's own forced break, or from **width alone** on plain identifiers, where prettier's own second pass writes the loss out as `aaa<bbb>;` plus `ccc;`). Which object shapes end worse still is stated in the object fixture's README. tsv keeps the pair around the `>`'s left operand, at every follower past the `>`: tsv's own parse commits the list past a break ahead of any expression, so the follower protects nothing (the catalog entry states which followers commit, for which parser) — [relational_chain_type_arg_parens](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_prettier_divergence/), [relational_chain_type_arg_parens_object_long](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_object_long_prettier_divergence/), [relational_chain_type_arg_parens_long](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_long_prettier_divergence/), [relational_chain_type_arg_parens_close_follow](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_close_follow_prettier_divergence/), [relational_chain_type_arg_parens_kept_shell](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_kept_shell_prettier_divergence/), [relational_chain_type_arg_parens_kept_shell_long](../tests/fixtures/typescript/expressions/binary/relational_chain_type_arg_parens_kept_shell_long_prettier_divergence/). A `>` the region reaches BEFORE its own close is the same seam with a layout answer: tsc's list parse runs from the `<` with error recovery to the first token the type grammar cannot take, so a break after an inner `>` commits the list (`x <⏎(aaa… >⏎bbb…)`, and with no shell at all `fn(x < q, aaa… >⏎bbb…)`, which acorn-typescript and tsv reject too); prettier takes that break by width and throws on its own output, tsv breaks ahead of the `>` — [relational_region_inner_gt_break_long](../tests/fixtures/typescript/expressions/binary/relational_region_inner_gt_break_long_prettier_divergence/)
...
- Multiline block comment before a non-null `!`, or before a type's `[no LineTerminator here]` token (a conditional's `extends`, a tuple `?`, an array `[]`, an indexed `[`, an arrow's return-type `=>`) — strips the grouping parens that held it (`(x /* m1⏎m2 */)!` → `x /* m1⏎m2 */!`, `(B /* m1⏎m2 */) extends C` → `B /* m1⏎m2 */ extends C`), putting a line break in a `[no LineTerminator here]` gap → output fails to re-parse; the array / indexed forms at the top of a type alias re-parse as a *different* program instead (`type Q = R;` plus an array expression). The same three failures arrive by a second route, with no paren to strip: the keyword→value **relocation** carries a multi-line block leading the value back ACROSS the keyword, and lands the break in the same forbidden gap — a conditional's `extends` (`A extends /* a⏎b */ /* c */⏎B` → `A /* a⏎b */ extends /* c */⏎B`, which no parser accepts) ([extends_paren_mixed_trailing_line_comment](../tests/fixtures/typescript/types/conditional/extends_paren_mixed_trailing_line_comment_prettier_divergence/)), a predicate's `is` (→ `x /* a⏎b */ /* c */ is A`, a type predicate no longer in return-type position) ([predicate_is_mixed_trailing_comment](../tests/fixtures/typescript/types/predicate_is_mixed_trailing_comment_prettier_divergence/)), and an indexed access's `[` (→ `type J = A /* a⏎b */ /* c */[K]`, which prettier's OWN next pass then writes out as the different program `type J = A;` plus `[K];` — pinned by that fixture's `audit_signature.txt`) ([indexed_access_line_comment](../tests/fixtures/typescript/types/indexed_access_line_comment_prettier_divergence/)) — [operand_paren_multiline_block_comment](../tests/fixtures/typescript/expressions/non_null/operand_paren_multiline_block_comment_prettier_divergence/), [retained_paren_shell_trailing_multiline_block_comment](../tests/fixtures/typescript/types/retained_paren_shell_trailing_multiline_block_comment_prettier_divergence/), [return_type_function_multiline_block_comment](../tests/fixtures/typescript/expressions/arrow/return_type_function_multiline_block_comment_prettier_divergence/)
- Comment forcing a break after `yield` — drops the grouping parens that made the break legal (`yield (⏎/* c */⏎x ?? y⏎)` → `yield /* c */⏎x ?? y;`), so ASI ends the `yield` at the newline and the operand becomes a separate statement: the `yield` **silently loses its argument**, a semantic change, and prettier's own next pass writes the split out (`yield; /* c */`). All three break-forcing spellings — a `//`, an own-line comment, and a block whose own text spans a line — and prettier gets the same rule right at `return`/`throw`, where `returnArgumentHasLeadingComment` runs; it simply never reaches `YieldExpression` — [yield_hanging_comment_parens](../tests/fixtures/typescript/expressions/await_yield/yield_hanging_comment_parens_prettier_divergence/), [yield_hanging_comment_parens_left_side](../tests/fixtures/typescript/expressions/await_yield/yield_hanging_comment_parens_left_side_prettier_divergence/)
...
- Frozen `for` init clause holding a bare `in` — emits the slice without the `[~In]` pair, giving a four-clause header (`for (⏎// prettier-ignore⏎ccc = 'aaa' in bbb;⏎;⏎) {}`) that no parser accepts; prettier's own second pass then throws on it (the Svelte plugin re-parses the script block and its TypeScript parser rejects), which F4b tolerates — no `audit_signature.txt` can represent a truncated chain — [init_clause_prettier_ignore_bare_in](../tests/fixtures/typescript/statements/for/init_clause_prettier_ignore_bare_in_prettier_divergence/)
- Arrow-body leftmost decorated class — drops the pair a concise body needs around a leftmost `@dec class {}` (`() => (@dec class {}).bbb` → `() => @dec⏎class {}.bbb`), at every shape the leftmost rule reaches → output no parser accepts; prettier's own second pass then throws on it (the Svelte plugin re-parses the script block and its TypeScript parser rejects), which F4b tolerates — no `audit_signature.txt` can represent a truncated chain — [body_leftmost_decorated_class](../tests/fixtures/typescript/expressions/arrow/body_leftmost_decorated_class_prettier_divergence/)
- Exponentiation type-assertion operand (`(<T>x) ** 2`) — strips the pair; tsc rejects the bare `<T>x ** 2` (so prettier throws on its own output) and acorn-typescript reads it as the different tree `<T>(x ** 2)` — [exponentiation_type_assertion](../tests/fixtures/typescript/expressions/exponentiation_type_assertion_prettier_divergence/)
- Two `<` printed as one `<<` where tsc never splits it (`<<T>() => R>x`, `typeof f<<T>() => void>`, `class B implements I<<T>() => U> {}`) — glues an assertion's or a list's `<` onto a generic function type's, from a spaced, parenthesized or broken authoring alike; tsc reads a shift there, so the output fails to re-parse and prettier's own second pass throws (tsc's printer, oxfmt, biome, dprint and swc all share the bug). tsv keeps a space between the two — [shift_left_type_assertion](../tests/fixtures/typescript/expressions/binary/shift_left_type_assertion_prettier_divergence/), [shift_left_typeof_query](../tests/fixtures/typescript/expressions/binary/shift_left_typeof_query_prettier_divergence/), [shift_left_heritage](../tests/fixtures/typescript/expressions/binary/shift_left_heritage_prettier_divergence/), [shift_left_no_split_long](../tests/fixtures/typescript/expressions/binary/shift_left_no_split_long_prettier_divergence/)
- Statement-head cast operand (`(namespace) as T;`, `(accessor) as T;`, `(readonly) satisfies T;`, `for ((using) as T; ;)`) — strips the pair, and tsc reads each bare line as a declaration head → output fails to re-parse (tsv's own parser included, for `namespace`); an `async` cast operand (`a = (async) as T`) likewise, at every position, where acorn-typescript reads an async arrow head instead — [statement_cast_paren](../tests/fixtures/typescript/syntax/contextual_keywords/statement_cast_paren_prettier_divergence/), [async_cast_paren](../tests/fixtures/typescript/syntax/contextual_keywords/async_cast_paren_prettier_divergence/)
- `as` / `satisfies` keyword→type gap, a `//` TAIL on a run led by a multi-line block (`x as /* c6⏎d6 */ // c7⏎H`) — **welds the line comment into the block's first line** (`x as /* c6 // c7⏎d6 */ H`), so `// c7` stops being a comment and becomes text of the block: a **content loss**, and idempotent on its own output, so nothing reveals it. Specific to these two keywords — at `keyof`, `await`, `new`, a `case` test and `export default` the same run keeps the `//` separate — [as_satisfies_value_glued_run_broke_after_multiline_block_comment](../tests/fixtures/typescript/expressions/as_satisfies_value_glued_run_broke_after_multiline_block_comment_prettier_divergence/)
...
there too, and the value keeps its own layout below the comment. The **inverse** run reads the
same way — a multi-line block the author GLUED ahead of a single-line one and then broke after
THAT (`= /* c1⏎d1 */ /* c2 */⏎v`) — because the rule is the RUN's, not one comment's: any
multi-line block in a broke-after run cannot print flat, so every separator the author broke
opens and the value drops below the run's closing line, with an author blank there surviving as
above. A per-comment reading answers no at both halves — the first is glued, the second
single-line — and welds the value onto the run's line. Evidence at every head gap, bare and
inside a redundant paren shell that strips into one:
[head_paren_shell_glued_run_broke_after_multiline_block_comment](../tests/fixtures/typescript/types/head_paren_shell_glued_run_broke_after_multiline_block_comment/),
a MATCH fixture — the form is prettier's own fixed point. At a keyword→value gap that
