...
To turn a span-only wire back into a loc-bearing one, `reconstruct_locations(ast, source)` adds `loc` to every node — and the Svelte `name_loc` — (mutating in place; `structuredClone` first to keep the input) — **exact for TypeScript** (each node's `loc` value equals acorn's; the key is appended last, so an object consumer matches but a re-serialized tree won't byte-match the wire's key order), **approximate for Svelte** (it skips only Svelte's `<script>`/`<style>` tag-position and destructure position quirks; `name_loc` and the in-tag comment `character` field are exact; and it **throws** rather than guess on a source holding a lone `\r`/U+2028/U+2029 — parse those with `loc`), a no-op for CSS. For sparse lookups, `create_locator(source, opts?)` reuses one line table across `loc_of(node)` / `reconstruct(ast)` calls (for `.svelte`, pass the span-only tree as `{ast}`, which a Svelte `loc_of` requires); a bare `loc_of(node, source, opts?)` is also exported.
