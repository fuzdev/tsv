...
- `goal` (TypeScript only, default `'module'`) — the parse goal: at `'script'`, `await` is an ordinary identifier, `import`/`export`/`import.meta` are syntax errors, and the code is sloppy unless its own `"use strict"` prologue makes it strict (so `with` and legacy octal literals/escapes parse; a module is always strict). `parse_svelte` and `parse_css` **throw** on the key rather than ignoring it (Svelte's `<script>` is always a module, CSS has no goal), so code forwarding one options bag to whichever parser should spell the inapplicable goal as `undefined` — every supported key reads `undefined` as its default.
