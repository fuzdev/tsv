...
use bumpalo::Bump;
use tsv_check::{bind_lib, check_program_with_lib, FileId, LibBase, SourceUnit};
...
fn var_eval_conflicts_with_lib_function_eval() {
    let es5 = bind_lib("lib.es5.d.ts", "declare function eval(x: string): any;").expect("lib parses");
    ...
    assert!(result.diagnostics.iter().any(|d| d.file == Some(FileId(1)) && d.code == 2300));
}
...
fn class_promise_conflicts_across_lib_files() {
    ...
    let promise =
        bind_lib("lib.es2015.promise.d.ts", "declare var Promise: PromiseConstructor;").expect("parses");
    let base = LibBase::build(&[&es5, &iterable, &promise]);

    let arena = Bump::new();
    let units = [SourceUnit::new("promiseDefinitionTest.ts", "class Promise<T> {}")];
    let result = check_program_with_lib(&units, Some(&base), &arena);
...
fn interface_augmentation_of_lib_is_silent() {
    ...
    let arena = Bump::new();
    let units = [SourceUnit::new("t.ts", "interface Array<T> { extra(): void; }")];
    let result = check_program_with_lib(&units, Some(&base), &arena);
