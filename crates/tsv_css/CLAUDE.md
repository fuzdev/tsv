...
- **Printer is organized by CSS spec hierarchy** (`rules.rs` → `declarations.rs` → `values.rs`, plus `atrules.rs` and `selectors.rs`), not by node kind. `selectors.rs` is shared between rules and at-rules, and rule and at-rule block bodies iterate their children through one shared `print_css_block_children` routine in `mod.rs` (blank-line preservation, inline trailing comments, format-ignore — one place, no drift); the `value_normalization/` module normalizes value text to prettier's form (numbers, hex colors, whitespace).
