...
- `printing` (`printing.rs`, pub) — String literal formatting, same-line detection, visual width. The line-break table (`build_line_breaks_into`, one entry per terminator's LAST byte) answers the three line questions two ways: the `*_fast` forms binary-search it, and the `*_scan` forms (`is_same_line_scan` / `has_newline_between_scan` / `has_blank_line_between_scan`) read the source bytes instead — one word of the host, since the first `\n` after a position is a few bytes away where the search is `log2(lines)` dependent steps — falling back to the search past a 64-byte cap. The scan is exact only over a table that IS the set of `\n` positions, which the builder reports as its return value (**LF-only**: every recorded byte is a `\n`; a `\r\n` qualifies, a bare `\r` or a U+2028/2029 does not); table and verdict travel together as `LineTable { breaks, lf_only }` (`LineTable::EMPTY` is the erased layout table — an empty table is authoritative, since the canonical reprint empties the layout table to erase authoring intent), which `tsv_ts::PrinterInputs::line_table` carries. ⚠️ The shape is measured, not stylistic — read the comments on `LINE_SCAN_CAP`, `next_lf_in`, the `*_table` fallbacks and the `_capped` layer before touching it: the three-needle exact walker at the ask cost +1.0% of a TypeScript format run; the scan converts only when it INLINES, which the cold-outlined fallbacks are what allow; a `const CAP` generic in place of the runtime cap gave back 0.077 points; and the six hand-written word-loop copies in this module (`next_lf_in`, `next_lf`, `next_width_relevant_in`, `next_width_relevant_or_single_quote_in`, `next_non_printable_ascii`, `exact_line_terminator_candidate`) stay separate on purpose — folding them onto one closure-parameterized walker was measured at +0.17% on the CSS cell and gave back the whole L86 win on the TypeScript one. Each copy carries its own `const _` class proof, which is the thing that must not be lost. The exhaustive test beside the forms grades every byte position of every terminator shape at every cap, and a two-sided `debug_assert_eq!` in each form grades every ask of every fixture against the table — no corpus holds a document where the two could differ (a bare `\r` is folded away before any printer runs; a U+2028 appears in none), so keep both green
...
// per document — two from the comment list, the table's `lf_only` verdict by the
// builder's own walk (`build_line_breaks_into` returns it) — never per embedded island.
let inputs = PrinterInputs {
	source,
	comments,
	line_table: LineTable { breaks: &line_breaks, lf_only },
	has_owned_comments,
	has_format_ignore,
};
