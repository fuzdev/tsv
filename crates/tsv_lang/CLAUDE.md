...
// The per-document environment (tsv_ts::PrinterInputs); the two flags are computed
// once per document from the comment list, never per embedded island.
let inputs = PrinterInputs { source, comments, line_breaks, has_owned_comments, has_format_ignore };
