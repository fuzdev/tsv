...
- `comment` (`comment.rs`, private) — Comment type, classification, O(log n) range lookup, and `CommentFreeWindow` — the comment-free window a printer keeps over its array: every search refreshes it, each printer's inline gate reads it ahead of the search, `next_comment_start` answers an unbounded walk, and a Svelte printer hands it to the island TS printers it constructs (`take_from`)
...
- `whitespace` (`whitespace.rs`, private) — `is_js_whitespace`, the ECMAScript `\s` CharSet, shared because two language crates need it and neither can reach the other (`tsv_svelte`'s `is_svelte_ws` is this set; so is the class `parseCss` skips, and `tsv_css` is a *dependency* of `tsv_svelte`). One definition, one exhaustive per-code-point test against Svelte's own hand-written enumeration. Its three trims — `trim_start_js_whitespace` / `trim_end_js_whitespace` / `trim_js_whitespace` — are the class's `trim_*_matches` answered on bytes: `str::trim_ascii*` over the five ASCII members the two classes share, then the byte it stopped on decides whether the searcher still runs (`<VT>`, the member `is_ascii_whitespace` lacks, or a non-ASCII byte — out of line and cold). One inline body per direction, deliberately: one body over two runtime direction flags came out outlined and paid the searcher's price call for call, where the inline form asks ~20 instructions per one-space trim against the searcher's ~50 — at seams asked once per comment line (the directive recognizer, `is_indentable_block_comment`, `tsv_ts`'s comment renderer: 135K a pass on a TypeScript corpus), a fifth of a percent of the run; `tsv_css`'s wire writer trims every declaration's property and value through the same three bodies, where the space after the colon is the one byte trimmed. ⚠️ Its module doc is the **workspace-wide whitespace index** — every class in tsv, which oracle each answers to, the reads that deliberately answer to none, and the four crates the `char::is_whitespace` / `str::trim` trap has been found in. Read it before adding or changing any whitespace predicate, in any crate: the recurring bug is a site's crate being taken as evidence about which class it wants
- `json_writer` (`json_writer.rs`, private, **`json` feature**) — `JsonWriter`: the byte-buffer + scalar-emitter primitive the three language crates' wire-JSON writers (`ast/convert/write/`) build on. It lives here, not in a language crate, so `tsv_svelte` can compose `tsv_ts` (embedded `{expr}` / `<script>`) and `tsv_css` (embedded `<style>`) emission into **one** shared buffer by passing `&mut JsonWriter` across crate boundaries; each language crate keeps its own node emitters. Escape/format parity is a contract: static structure and tokens are written verbatim (debug-asserted escape-free), dynamic strings are escaped by hand **byte-identical** to `serde_json`'s string serialization, non-integral `f64` delegates to `serde_json` so ryu formatting is exactly its, and integers are hand-formatted (two-digit-pair, several per node). `string` has two arms behind a **SWAR escape prescan** (`needs_escape`, over the `swar` kernels): identifier names, string-literal bodies and comment text are overwhelmingly escape-free, so a clean word-at-a-time answer turns the emission into `token`'s quote-blit-quote. The remainder is finished with an **overlapping final word** rather than a byte loop whenever the slice is at least 8 bytes, and a slice shorter than a word (over half of all calls) is **gathered** into one word (two overlapping 4-byte loads, or first/middle/last byte) — both sound only because the answer is a boolean over the *union* of the bytes tested, so re-testing a byte cannot change it (a position-returning scan like `location`'s could not do this). Anything that flags goes to `string_escaped`, the hand escaper, which is **not** the rare arm (16% of the strings and 30% of the bytes over real Svelte components — template whitespace like `"\n\t\t"`): it writes into a fixed-width window of the buffer (the `start_end` shape: extend, write through a register-held slice, truncate), moving clean bytes a word at a time with `escape_lanes` and stopping at the **lowest** flagged lane (the one the kernel guarantees genuine), then escaping byte by byte while escapes cluster; a string shorter than a word takes a byte loop into a narrower window. The short forms are `serde_json`'s seven (`\"` `\\` `\b` `\t` `\n` `\f` `\r`) and every other control byte is `\u00XX` in **lowercase** hex, as `serde_json` writes it. ⚠️ The prescan's predicate must stay a superset of the escaper's set (`0x00..=0x1F`, `"`, `\` — **not** `DEL`, not any non-ASCII byte), and like the integer emitters **no corpus can grade either arm**: a mis-scan or a wrong escape only surfaces on an input that actually carries the byte, and several of the boundary bytes never appear in real source. The oracle is `string_matches_serde_json`, which grades both arms against `serde_json` itself over an alphabet of every escaped byte and its neighbours, **at every offset across the 8-byte stride and every length to 40**, runs of escapes at every offset, and long strings that cross the escaper's window restarts — the axes a word-at-a-time scan fails on, and the ones a corpus samples arbitrarily; the escaper's window widths are exact, and `escape_windows_hold_their_widest_step` is the sweep that drives a step to the last byte of each (a window one byte short panics there). `string_pair` is `string` for a node that carries one string as two consecutive field values (a Svelte `Text`'s `raw` and `data`, the same bytes whenever the decode is identity): it escapes once and copies the first field's *emitted* bytes for the second, byte-identical by construction and graded against two `string` calls over the same case set; it is `inline(always)` so its constant middle fragment stays a fixed-width move (outlined, that fragment was a libc `memcpy` call). ⚠️ **Integer emission is arithmetic, and no corpus can grade arithmetic** — a wrong digit width writes a wrong offset that still parses as JSON, so the fixture suite, a byte-diff over thousands of files, and every audit gate can all stay green through the bug; the oracles live beside the code and must stay green. Three emission paths share one arithmetic core (`digit_word`): the direct `u32`/`u64`/`usize` appends, the **`start`/`end` pair** (`start_end`, and `start_end_field` / `start_end_object` with the `,"start":` / `{"start":` key — both integers and the `,"end":` key between them written into one fixed-width window of the buffer, then trimmed; graded against `format!` over every digit-width edge pair by `start_end_family_matches_format`), and the **staged run** (`stage_begin` / `stage_raw` / `stage_short` / `stage_u32` / `stage_usize` / `stage_flush`) that assembles a whole node header in a writer-owned scratch and appends it once. Before touching any of the three, read `stage_begin`'s doc, which carries the measured rationale — including the one a reader is most likely to get backwards, that a run's **static** fragments are copied twice (scratch, then flush) so the trade is *appends removed* against *static bytes in the run*, **not** run width, and a wide mostly-static burst can be a net loss — and `digit_word`'s for the codegen constraints (register-only digit generation, one magnitude test that yields both the arm and its width, `inline(always)` on the staged emitters) that any rewrite must preserve. Within a run, `stage_raw` takes a constant fragment (a fixed-width move) and `stage_short` a static one chosen at run time (the node type), whose runtime length would otherwise lower `copy_from_slice` to a libc `memcpy` call: two overlapping fixed-width copies by length class, the same two 16-byte halves libc's AVX `memmove` uses for a 16–31-byte copy, so only the call goes (a wide inline blit is the opposite trade, SSE2 against AVX, and measured slower)
- `sizing` (`sizing.rs`, private) — `estimated_json_capacity` / `estimated_ast_arena_capacity` — pre-size heuristics for the wire-JSON output buffer and the parse-time bump arena — plus the crate-internal `estimated_line_starts_capacity`, the capacity every `location` constructor that scans for lines seeds its line-start table at (a deliberately short bytes-per-line, so the table rarely grows at all)
- `output` (`output.rs`, private) — `OutputBuffer` — string building with column tracking
- `swar` (`swar.rs`, `pub mod` for three items) — the word-at-a-time byte-search kernels `location`'s line/multibyte scans, `json_writer`'s escape prescan and escaper, and the language lexers' token-body scans share: `splat` (the broadcast needle), `zero_lanes` (has-zero), `lanes_less_than` (has-less; the floor test of the host-range scan and of the escape prescan), `high_bit_lanes`, all `pub(crate)`. One module rather than one copy per caller because their correctness argument is subtle and identical — a SWAR subtract borrows **across** lanes, so a lane's flag is not independently trustworthy. What holds is that **the lowest set lane is always genuine** (a spurious flag needs a genuine one below it, and a genuine lane always flags itself), which gives position callers `trailing_zeros` and boolean callers `mask != 0`. Read the per-kernel guarantee before adding a caller; the lowest-lane property itself is graded by exhaustive tests beside the code. **`next_byte_of`, `next_byte_below_or_of` and `has_ascii_uppercase` are the three public items.** The second is `next_byte_of`'s **host-range** form — the whole document plus `[from, end)`, three classes in one word read (below a floor, non-ASCII, one of `N` needles), a lane past `end` ignored — for a "does this short slice hold any byte of a class" question: `tsv_css`'s identifier values are 10.6 bytes on average and 42% under a word, and over the bare slice such a question can only be a byte at a time. The second is the one class scan — "does this slice hold an `A`..=`Z` byte?", `uppercase_lanes` a word at a time, exact per lane (no borrow: the low-seven-bit word plus two constants, masked by the original high bits), its last word the slice's LAST eight bytes overlapping the one before so a nine-byte CSS property name is two word tests and not a seven-byte compare chain. The first is, the shape every lexer scan over a token body is: `tsv_ts`'s string / template / line- and block-comment runs and `tsv_css`'s comment run all reach it, because spelled as the compare chain it reads as, LLVM emits `2 + 2N` instructions and `N + 2` branches per byte and **does not vectorize** — the caller's resume at a hit (an escape step, a `*` that opens no `*/`) makes the stride data-dependent. ⚠️ It has a **density axis**: the splats are per call and the word is per eight bytes, so break-even is ~3–4 content bytes and a construct that is routinely empty loses. Numbers, and the loose-class-plus-exact-fallback shape a non-byte-set caller uses (`tsv_ts`'s `<LS>` / `<PS>`), are on the function
- `hash` (`hash.rs`, private) — `FxHasher` + the `FxHashMap` / `FxHashSet` / `FxBuildHasher` aliases: a dep-free multiply-xor hasher for the side tables. The printer/writer tables are integer-keyed (the doc arena's `ChainShareStore` hashed tier, `tsv_svelte`'s root-inline-run set) and reach the hasher only through its integer methods; every table in the experimental `tsv_check` uses it too — address map, symbol tables and flow-label scratch on integer keys, atom interner and merge globals on **name (`str`) keys**, which route through the byte path. Replacing SipHash there is **byte-identical by construction only while every consumer stays order-free** — they use `get`/`insert`/`contains`/`clear` and never let iteration order reach output, and the two `tsv_check` sites that do iterate sort first. A consumer that iterates one of these and emits in that order would make the hasher observable; that is the constraint to preserve, not the hash quality
...
let inputs = PrinterInputs {
	...
	has_owned_comments,
	has_format_ignore,
	comment_free_window: None, // an island printer takes the embedding printer's window here
};
