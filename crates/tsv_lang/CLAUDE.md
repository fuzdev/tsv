...
- `printing` (`printing.rs`, pub) — String literal formatting, same-line detection, visual width. The line-break table (`build_line_breaks_into`, one entry per terminator's LAST byte) answers the three line questions two ways: the `*_fast` forms binary-search it, and the `*_scan` forms (`is_same_line_scan` / `has_newline_between_scan` / `has_blank_line_between_scan`) read the source bytes instead — one word of the host, since the first `\n` after a position is a few bytes away where the search is `log2(lines)` dependent steps — falling back to the search past a 64-byte cap. The scan is exact only over a table that IS the set of `\n` positions, which the builder reports as its return value (**LF-only**: every recorded byte is a `\n`; a `\r\n` qualifies, a bare `\r` or a U+2028/2029 does not) and the printers thread as `tsv_ts::PrinterInputs::line_breaks_lf_only`; an empty table is authoritative (the canonical reprint empties the layout table to erase authoring intent). ⚠️ The shape is measured, not stylistic — read the comments on `LINE_SCAN_CAP`, `next_lf_in` and the `*_table` fallbacks before touching it: the three-needle exact walker at the ask cost +1.0% of a TypeScript format run, and the scan converts only when it INLINES, which the cold-outlined fallbacks are what allow. The exhaustive test beside the forms grades every byte position of every terminator shape at every cap, and a two-sided `debug_assert_eq!` in each form grades every ask of every fixture against the table — no corpus holds a document where the two could differ (a bare `\r` is folded away before any printer runs; a U+2028 appears in none), so keep both green
...
// The per-document environment (tsv_ts::PrinterInputs); the three flags are computed
// once per document — two from the comment list, `line_breaks_lf_only` by the table
// builder's own walk — never per embedded island.
