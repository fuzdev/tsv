...
/// `[$($k),*]` as its own arm.
///
/// **Every arm is the same expression, and that is the point.** The render
/// loop's output write moves a *mean of 4.69 bytes*, with 5% of calls moving
/// zero. `String::push_str` lowers a runtime-length `copy_nonoverlapping` to an
/// indirect `memcpy@plt`, so those calls cost more than the bytes they carry.
/// Inside an arm that has matched `len == k`, LLVM knows the count is `k` and
/// stores it inline instead — nothing at all for the empty write.
///
...
/// way. Collapsing the render write's nine — measured at **that site alone**,
/// with the only other caller already retired — costs `instructions:u`
/// **+0.97…+1.08%** across five corpora (per-side spread ≤0.004%).
...
/// two call sites and affords nine arms for +1,872 B. `DocArena::alloc_children`
