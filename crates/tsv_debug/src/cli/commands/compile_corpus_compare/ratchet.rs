...
/// The ratchet over this gate's colocated snapshot, carrying its header + re-pin hint. The
/// snapshot sits beside the code that owns it, as with `gap_audit_known.txt` /
/// `blank_audit_known.txt`; only the path is compile-time, the file is read at runtime.
pub(super) fn ratchet() -> Ratchet {
    Ratchet::colocated("compile_validation_known.txt", SNAPSHOT_HEADER, REPIN_HINT)
}
...
pub(super) fn grade_and_report(
    ...
    println!("\nVALIDATION RATCHET — {}", ratchet().path().display());
    if diff.holds() {
        println!(
...
pub(super) fn update(groups: &[GroupInfo], outcomes: &[FileOutcome]) -> Result<(), CliError> {
    let previous: BTreeSet<RatchetKey> = if ratchet().path().exists() {
