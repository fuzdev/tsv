...
const SNAPSHOT_HEADER: &str = "# Generated by `deno task gaps:audit:update` — do NOT hand-edit.\n\
     ...
     # formatter drop or double-print one (DROPPED / DOUBLE-PRINTED), makes a `//`\n\
     # swallow following content on its output line (SWALLOW — real content loss the\n\
     # print-once ledger is blind to, since the comment IS printed once), or leaves an\n\
     # output that no longer parses (UNREPARSEABLE — a dead document, which no gate that\n\
     # formats files as authored can reach). The gate fails\n\
     # on a line that is NOT here (a new kind of loss) and on a line here that no longer\n\
     # fires (a stale entry — delete it when you fix one). Counts are deliberately not\n\
     # pinned: they churn with every ordinary fixture PR, and a gate that fails per added\n\
     # fixture gets turned off. The PAYLOAD set is pinned, though: a shape that starts\n\
     # dropping a comment kind it used to survive is a new bug on a new ownership path.\n\
     #\n\
...
/// [`scan_audit`](crate::cli::commands::scan_audit)'s hand-curated `ALLOW`: at several
/// hundred shapes a per-entry rationale is not a thing a human can keep honest, so it
/// deliberately carries none. It is a ratchet, not a sanction — every line is a **known
/// bug**, and the file shrinking is the goal.
///
/// Colocated with this module so it travels with the code that owns it — see
/// [`Ratchet::colocated`](crate::audit::ratchet::Ratchet::colocated), and
/// [`audit::ratchet`](crate::audit::ratchet) for why the file is read at runtime rather than
/// `include_str!`d.
pub(super) fn ratchet() -> Ratchet {
    Ratchet::colocated("gap_audit_known.txt", SNAPSHOT_HEADER, REPIN_HINT)
}
...
/// in one place.
///
/// There is **no report-only carve-out**: every kind but `PANIC` is graded, [`Kind::Swallow`]
/// included. It was staged report-only while the swallow check was armed on `tsv_ts`'s
/// emitters alone — freezing a shape set that only half the printers could produce would have
/// pinned an artifact of the instrument. With every `tsv_svelte` comment emitter tagged too,
/// the arm is whole and the class is pinned like any other known bug.
...
/// How many of `shapes` are `kind`. Three kinds get their own accounting on top of the
/// ratchet's whole-set verdict: [`Kind::Swallow`] and [`Kind::Unreparseable`] are pinned like
/// every other known bug but named on their own lines (the content-loss half and the
/// dead-output half of what a holding run holds), and [`Kind::Panic`] is the set
/// [`is_pinnable`] keeps out of the snapshot, so it needs its own count on every exit path
/// (the ratchet's `GateDiff::unpinnable` is the abstract count; this is the concrete panic set
/// gap reports with examples).
pub(super) fn count_kind(shapes: &BTreeMap<(Kind, String), ShapeAgg>, kind: Kind) -> usize {
    shapes.keys().filter(|(k, _)| *k == kind).count()
}

/// Whether a shape is something the snapshot may pin — everything but a [`Kind::Panic`].
///
/// A panic is not a "known bug" to ratchet alongside the losses. The invariant it breaks is
...
mod tests {
    ...
    /// A SWALLOW is pinned like a drop — the class carries no carve-out.
    ///
    /// The corpus cannot grade the *absence* of one: a filter putting swallows back on a
    /// report-only footing would simply write a smaller snapshot, and `--update` would
    /// re-pin the reduced set on the very next run, so the gate would stay green through
    /// the regression. This asserts the key reaches the file at all.
    #[test]
    fn a_swallow_is_pinned_like_a_drop() {
        let mut shapes: BTreeMap<(Kind, String), ShapeAgg> = BTreeMap::new();
        shapes.insert((Kind::Dropped, "import⟨⟩.".to_string()), mk_agg(&["block"]));
        shapes.insert((Kind::Swallow, "IDENT⟨⟩;".to_string()), mk_agg(&["line"]));

        let r = Ratchet::new(PathBuf::from("/unused"), SNAPSHOT_HEADER, REPIN_HINT);
        let found = snapshot_keys(&shapes);
        assert_eq!(found.len(), 2, "the swallow is a key the ratchet sees");
        assert!(
            found.iter().all(SnapshotKey::is_pinnable),
            "and a pinnable one — a swallow must not fail the gate the way a panic does"
        );
        assert!(r.render(&found).contains("SWALLOW\tIDENT⟨⟩;\tline\n"));
        assert_eq!(count_kind(&shapes, Kind::Swallow), 1);
    }

    /// An UNREPARSEABLE is pinned like a drop too — same reasoning as the swallow: the file
    /// must be able to hold the class, or `--update` would re-pin a smaller set and the gate
    /// would stay green through a regression.
    #[test]
    fn an_unreparseable_is_pinned_like_a_drop() {
        let mut shapes: BTreeMap<(Kind, String), ShapeAgg> = BTreeMap::new();
        shapes.insert((Kind::Dropped, "import⟨⟩.".to_string()), mk_agg(&["block"]));
        shapes.insert(
            (Kind::Unreparseable, "yield (⟨⟩x".to_string()),
            mk_agg(&["multiline"]),
        );
        ...
        assert_eq!(
            found.len(),
            2,
            "the unreparseable is a key the ratchet sees"
        );
        assert!(found.iter().all(SnapshotKey::is_pinnable));
        assert!(
            r.render(&found)
                .contains("UNREPARSEABLE\tyield (⟨⟩x\tmultiline\n")
        );
        assert_eq!(count_kind(&shapes, Kind::Unreparseable), 1);
        assert_eq!(count_kind(&shapes, Kind::Swallow), 0);
    }
    ...
    /// **`Kind`-enum** order — all `DROPPED`, then all `DOUBLE-PRINTED`, then all `SWALLOW`.
    /// That is NOT label-string order, which would put `DOUBLE-PRINTED` first (`'O' < 'R'`).
    /// Two facts nothing else pins carry it: (1) `KnownKey.kind` is the [`Kind`] **enum**
    ...
    /// surface as a whole-file reorder the next time someone runs `--update`. The line text
    ...
    fn render_orders_by_kind_enum_not_label() {
        ...
        // The SWALLOW block is the file's tail, so an added swallow lands at the end rather
        // than interleaving with the drops it has nothing to do with.
        shapes.insert((Kind::Swallow, "A⟨⟩;".to_string()), mk_agg(&["line"]));
        ...
        assert_eq!(
            ...
            "snapshot renders in Kind-enum order (DROPPED, DOUBLE-PRINTED, SWALLOW), not \
