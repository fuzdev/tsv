...
const SNAPSHOT_HEADER: &str = "# Generated by `deno task gaps:audit:update` — do NOT hand-edit.\n\
     ...
     # formatter drop or double-print one (DROPPED / DOUBLE-PRINTED), or makes a `//`\n\
     # swallow following content on its output line (SWALLOW — real content loss the\n\
     # print-once ledger is blind to, since the comment IS printed once). The gate fails\n\
...
/// How many of `shapes` are the [`Kind::Swallow`] class — pinned like every other known bug,
/// but counted on its own so a run can name the content-loss half of what it holds.
pub(super) fn count_swallows(shapes: &BTreeMap<(Kind, String), ShapeAgg>) -> usize {
    shapes.keys().filter(|(k, _)| *k == Kind::Swallow).count()
}
...
mod tests {
    ...
    fn a_swallow_is_pinned_like_a_drop() {
        ...
        assert_eq!(count_swallows(&shapes), 1);
    }
