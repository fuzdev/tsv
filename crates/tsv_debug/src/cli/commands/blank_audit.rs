...
//! 3. **structural reparse** — `format(injected)` must reparse to the same document: it must
//!    parse at all (`Unreparseable`), keep its node population — no element, block, statement
//!    or word of template text dropped, duplicated or re-typed (`NodeLoss`, the gated subset
//!    of a structural change) — and keep its skeleton (`StructuralDivergence`, **report-only**,
//!    see below).
...
//!
//! Invariants 1–4 are the shared [`f1_check`] (its reparse compare through [`compare_reparsed`]);
//! 5 is the ledger; 6 is a region-scoped output scan.
//!
...
use crate::audit::properties::{
    F1Outcome, Formatted, Pristine, ReparseCompare, Utf16ToByte, compare_reparsed, f1_check,
    ledger_format, pristine_format, source_has_ignore_directive, tsv_parse_to_value,
};
...
enum BlankKind {
    ...
    Unreparseable,
    /// Output reparses but a conserved node (or a word of template text) was dropped,
    /// duplicated or re-typed — the gated subset of a structural divergence.
    NodeLoss,
...
const SNAPSHOT_HEADER: &str = "# Generated by `deno task blanks:audit:update` — do NOT hand-edit.\n\
     ...
     # fixed point, fail to reparse, drop or duplicate a node, corrupt a leaf, or\n\
     # drop/double-print a comment. The gate fails on a line that is NOT here (a new kind\n\
     # of break), on a line here that no longer fires (a stale entry — delete it when you\n\
     # fix one), and on any PANIC.\n\
     #\n\
...
/// 2, 3, 4, 6 — the same checks [`f1_check`] runs (through the shared [`compare_reparsed`]
/// core), but reusing the ledger's already-computed `output` (= `format_source(injected)`) so no
/// second format of the injected input is paid.
///
...
fn grade_changed(
    ...
    // Invariants 3 (structure — population, then skeleton) and 4 (leaf), then 2 (idempotency):
    // exclusive, in `f1_check`'s priority order, through the same compare core.
    match compare_reparsed(wire_in, wire_out, RENDER, false) {
        ReparseCompare::NodeLoss(_) => kinds.push(BlankKind::NodeLoss),
        ReparseCompare::Divergent(_) => kinds.push(BlankKind::StructuralDivergence),
        ReparseCompare::LeafCorruption(_) => kinds.push(BlankKind::LeafCorruption),
