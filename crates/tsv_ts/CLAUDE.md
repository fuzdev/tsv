...
- `convert_ast(program, source) -> public::Program` / `convert_ast_json(...) -> serde_json::Value` / `convert_ast_json_string(...) -> String` (all gated on `convert` feature). The string variant is the compact-wire hot path (FFI/WASM/CLI non-pretty): it serializes the typed public AST directly, never materializing the intermediate `Value`, and **fuses byte→UTF-16 offset translation into conversion** — `convert_program` threads a `tsv_lang::LocationMapper` (tracker + `ByteToCharMap`) and emits final char-space `start`/`end`/`loc` directly, so no post-conversion translation walk runs (per-language pipeline shapes: [docs/architecture.md §Closed Scope, Open Convention](../../docs/architecture.md#closed-scope-open-convention)). `convert_ast` and the `Value` path pass `LocationMapper::identity` (exact byte-space passthrough); `convert_ast_json` then translates via the `Value` walk in `ast/convert/mod.rs` — the independent oracle the fused path is gated against: the fixture suite's string-path identity check and typed-walk parity probes (synthesized multibyte variants plus extracted `<script>` contents, so every fixture's AST shapes are covered) and `json_profile`'s corpus comparison. Output is byte-identical to serializing `convert_ast_json`'s `Value`. The typed translation walk (`ast/convert/translate_typed.rs`) remains for `tsv_svelte`'s hybrid walk, which requires byte-space conversion output (comment-attach windows, `inject_loc_character`) and translates afterward.
...
- `ast::convert::translate_expression_byte_to_char_offsets_typed` — the expression-level sibling of the `Program`-level typed offset-translation walk, for hosts that embed typed `Expression`s in their own public AST (`tsv_svelte`'s typed walk calls it per template expression); `ast::convert::translate_column` is likewise `pub` so the delta-preserving column math exists once (`tsv_svelte` reuses it for `name_loc` positions)
