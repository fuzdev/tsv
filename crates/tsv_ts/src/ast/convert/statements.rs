...
fn convert_attributes<'src>(
    ...
    match attributes {
        Some(attrs) => Some(
            attrs
                .iter()
                .map(|a| convert_import_attribute(a, source, loc, interner))
                .collect(),
        ),
    ...
    match stmt {
        internal::Statement::ExpressionStatement(expr_stmt) => {
            ...
            public::Statement::ExpressionStatement(public::ExpressionStatement {
                ...
                start: loc.pos(expr_stmt.span.start),
                end: loc.pos(expr_stmt.span.end),
                loc: create_location(expr_stmt.span, loc),
                expression: convert_expression(&expr_stmt.expression, source, loc, interner),
                directive,
            })
        }
        internal::Statement::VariableDeclaration(var_decl) => {
            public::Statement::VariableDeclaration(convert_variable_declaration(
                var_decl, source, loc, interner,
            ))
        }
        ...
        internal::Statement::ExportNamedDeclaration(export_decl) => {
            ...
            let attributes =
                convert_attributes(export_decl.attributes, source, loc, interner, schema);
            let export_start = if let Some(internal::Statement::ClassDeclaration(class)) =
                export_decl.declaration
            {
                if class.decorators.is_some() {
                    find_export_start(source, export_decl.span.start)
                } else {
            ...
            public::Statement::ExportNamedDeclaration(public::ExportNamedDeclaration {
                ...
                start: loc.pos(export_start),
                end: loc.pos(export_decl.span.end),
                loc: create_location(export_span, loc),
                export_kind,
                declaration: export_decl
                    .declaration
                    .as_ref()
                    .map(|d| Box::new(convert_statement(d, source, loc, interner, schema))),
                specifiers: export_decl
                    .specifiers
                    .iter()
                    .map(|s| convert_export_specifier(s, source, loc, interner, schema))
                    .collect(),
                ...
                source: export_decl
                    .source
                    .as_ref()
                    .map(|s| convert_literal(s, source, loc)),
                attributes,
            })
        }
        ...
        internal::Statement::ExportAllDeclaration(export_decl) => {
            ...
            public::Statement::ExportAllDeclaration(public::ExportAllDeclaration {
                ...
                start: loc.pos(export_decl.span.start),
                end: loc.pos(export_decl.span.end),
                loc: create_location(export_decl.span, loc),
                export_kind,
                exported: export_decl
                    .exported
                    .as_ref()
                    .map(|name| convert_module_export_name(name, source, loc, interner)),
                source: convert_literal(&export_decl.source, source, loc),
                attributes,
            })
        }
        internal::Statement::TSExportAssignment(export_assign) => {
            public::Statement::TSExportAssignment(public::TSExportAssignment {
                ...
                start: loc.pos(export_assign.span.start),
                end: loc.pos(export_assign.span.end),
                loc: create_location(export_assign.span, loc),
                expression: convert_expression(&export_assign.expression, source, loc, interner),
            })
        }
        internal::Statement::TSNamespaceExportDeclaration(ns_export) => {
            public::Statement::TSNamespaceExportDeclaration(public::TSNamespaceExportDeclaration {
                ...
                start: loc.pos(ns_export.span.start),
                end: loc.pos(ns_export.span.end),
                loc: create_location(ns_export.span, loc),
                id: convert_identifier(&ns_export.id, source, loc, interner),
            })
        }
        internal::Statement::ImportDeclaration(import_decl) => {
            ...
            let attributes =
                convert_attributes(import_decl.attributes, source, loc, interner, schema);
            public::Statement::ImportDeclaration(public::ImportDeclaration {
                node_type: "ImportDeclaration",
                start: loc.pos(import_decl.span.start),
                end: loc.pos(import_decl.span.end),
                loc: create_location(import_decl.span, loc),
                import_kind,
                phase: import_decl.phase.as_str(),
                specifiers: import_decl
                    .specifiers
                    .iter()
                    .map(|s| convert_import_specifier(s, source, loc, interner, schema))
                    .collect(),
                source: convert_literal(&import_decl.source, source, loc),
                attributes,
            })
        }
        internal::Statement::TSImportEqualsDeclaration(import_eq) => {
            public::Statement::TSImportEqualsDeclaration(convert_import_equals_declaration(
                import_eq, source, loc, interner,
            ))
        }
        // Control flow statements
        internal::Statement::IfStatement(if_stmt) => {
            public::Statement::IfStatement(convert_if_statement(if_stmt, source, loc, interner))
        }
        internal::Statement::ForStatement(for_stmt) => {
            public::Statement::ForStatement(convert_for_statement(for_stmt, source, loc, interner))
        }
        internal::Statement::ForInStatement(for_in) => public::Statement::ForInStatement(
            convert_for_in_statement(for_in, source, loc, interner),
        ),
        internal::Statement::ForOfStatement(for_of) => public::Statement::ForOfStatement(
            convert_for_of_statement(for_of, source, loc, interner),
        ),
        internal::Statement::WhileStatement(while_stmt) => public::Statement::WhileStatement(
            convert_while_statement(while_stmt, source, loc, interner),
        ),
        internal::Statement::DoWhileStatement(do_while) => public::Statement::DoWhileStatement(
            convert_do_while_statement(do_while, source, loc, interner),
        ),
        internal::Statement::SwitchStatement(switch_stmt) => public::Statement::SwitchStatement(
            convert_switch_statement(switch_stmt, source, loc, interner),
        ),
        internal::Statement::TryStatement(try_stmt) => {
            public::Statement::TryStatement(convert_try_statement(try_stmt, source, loc, interner))
        }
        internal::Statement::ThrowStatement(throw_stmt) => public::Statement::ThrowStatement(
            convert_throw_statement(throw_stmt, source, loc, interner),
        ),
        internal::Statement::BreakStatement(break_stmt) => public::Statement::BreakStatement(
            convert_break_statement(break_stmt, source, loc, interner),
        ),
        ...
        internal::Statement::LabeledStatement(labeled) => public::Statement::LabeledStatement(
            convert_labeled_statement(labeled, source, loc, interner),
        ),
        internal::Statement::EmptyStatement(empty) => {
            public::Statement::EmptyStatement(public::EmptyStatement {
                node_type: "EmptyStatement",
                start: loc.pos(empty.span.start),
                end: loc.pos(empty.span.end),
                loc: create_location(empty.span, loc),
            })
        }
        internal::Statement::DebuggerStatement(dbg) => {
            public::Statement::DebuggerStatement(public::DebuggerStatement {
                ...
                start: loc.pos(dbg.span.start),
                end: loc.pos(dbg.span.end),
                loc: create_location(dbg.span, loc),
            })
        }
        ...
        internal::Statement::TSDeclareFunction(func) => public::Statement::TSDeclareFunction(
            super::convert_declare_function(func, source, loc, interner),
        ),
        internal::Statement::TSEnumDeclaration(enum_decl) => public::Statement::TSEnumDeclaration(
            super::convert_enum_declaration(enum_decl, source, loc, interner),
        ),
...
pub(in crate::ast) fn convert_module_declaration<'src>(
    ...
    public::TSModuleDeclaration {
        ...
        start: loc.pos(decl.span.start),
        end: loc.pos(decl.span.end),
        loc: create_location(decl.span, loc),
        id: convert_module_name(&decl.id, source, loc, interner),
        body: decl
            .body
            .as_ref()
            .map(|b| convert_module_declaration_body(b, source, loc, interner)),
...
fn convert_module_name<'src>(
    ...
    match name {
        internal::TSModuleName::Identifier(id) => {
            public::TSModuleName::Identifier(convert_identifier(id, source, loc, interner))
        }
        internal::TSModuleName::Literal(lit) => {
            public::TSModuleName::Literal(convert_literal(lit, source, loc))
        }
    }
}
    ...
    match body {
        ...
        internal::TSModuleDeclarationBody::TSModuleDeclaration(nested) => {
            public::TSModuleDeclarationBody::TSModuleDeclaration(Box::new(
                convert_module_declaration(nested, source, loc, interner),
            ))
        }
    }
}
...
pub fn convert_variable_declaration<'src>(
    ...
    public::VariableDeclaration {
        ...
        start: loc.pos(var_decl.span.start),
        end: loc.pos(var_decl.span.end),
        loc: create_location(var_decl.span, loc),
        declarations: var_decl
            .declarations
            .iter()
            .map(|d| convert_variable_declarator(d, source, loc, interner))
            .collect(),
...
pub(in crate::ast) fn convert_variable_declarator<'src>(
    ...
    public::VariableDeclarator {
        ...
        start: loc.pos(declarator.span.start),
        end: loc.pos(declarator.span.end),
        loc: create_location(declarator.span, loc),
        // id can be Identifier, ArrayPattern, or ObjectPattern
        id: convert_expression(&declarator.id, source, loc, interner),
...
fn convert_import_equals_declaration<'src>(
    ...
    public::TSImportEqualsDeclaration {
        ...
        id: convert_identifier(&decl.id, source, loc, interner),
        module_reference: convert_module_reference(&decl.module_reference, source, loc, interner),
    }
}
    ...
    match module_ref {
        internal::TSModuleReference::ExternalModuleReference(ext_ref) => {
            public::TSModuleReference::ExternalModuleReference(public::TSExternalModuleReference {
                ...
                start: loc.pos(ext_ref.span.start),
                end: loc.pos(ext_ref.span.end),
                loc: create_location(ext_ref.span, loc),
                expression: convert_literal(&ext_ref.expression, source, loc),
            })
        }
