...
pub(in crate::printer) struct UnionValueDoc {
    ...
    pub run_handed: bool,
    /// Whether the SEAM keeps its operator or delimiter glued to the union — `: { … } |
    /// null`, `Foo<{ … } | null>` — the answer every seam's own layout must agree with.
    /// Holds when `doc` prints hugged AND the seam's gap holds no block comment at all:
    /// the union's own join declines only for a block GLUED to its first member
    /// ([`Printer::union_prints_hugged_with`]), but a block the author separated from
    /// the member (`: /* c */⏎{ … } | null`) must still take the seam's non-hug layout,
    /// whose emitter keeps the run on its own line — the hug arm would glue it onto the
    /// member, which the next pass reads as the glued spelling and declines (F1). Under
    /// the non-hug layout the run leads a union that still hugs below it, the one form
    /// both spellings reach. A SINGLE-member union is its member, so only a separated
    /// block declines there ([`Printer::union_seam_hugs`]).
    pub hugged: bool,
}
...
enum UnionLeadingGap {
    /// An enclosing SEAM whose gap runs from `gap_start` (just past its operator or
    /// opening delimiter) to the first member: the value seams (the alias `=`, the
    /// annotation `:`, the function-type `=>`, the mapped-type value), the containers (a
    /// type argument's `<`, a tuple's or indexed access's `[`, a paren shell's `(`), and
    /// the keywords (`as` / `satisfies`, a type parameter's `extends` / `=`, a
    /// conditional's `extends` / `?` / `:`, a mapped `in`). A block comment GLUED to the
    /// first member — ending on the member's line, whether the seam broke ahead of it or
    /// not, and whether handed in from the seam's gap ([`Printer::build_union_value_doc`],
    /// `handed`) or authored around the union's leading pipe (`| /* c */ { … }`,
    /// `/* c */ | { … }`) — declines the hug ([`Printer::union_prints_hugged_with`]):
    /// prettier's own rule, the comment binds to the first member and
    /// `types.some((t) => hasComment(t))` bails. A block the author separated from the
    /// member (`/* c */⏎{ … }`) binds to the union instead and leaves the hug intact; the
    /// seam's non-hug layout then carries it on its own line ([`UnionValueDoc::hugged`]).
    /// One rule at every seam, so each spelling is one fixed point
    /// (`union_hug_gap_block_comment` and its `_container` / `_cast` / `_keyword`
    /// siblings).
    Seam { gap_start: u32, handed: Option<u32> },
    /// A PARENT starts where the union does — the type predicate's annotation at `is`,
    /// a conditional's CHECK type: prettier binds a block glued ahead of the union to
    /// that parent, not to the member, so the union answers the hug from its own span
    /// alone and keeps hugging behind it (`union_hug_gap_block_comment`'s `is` control,
    /// the `_keyword` sibling's check type).
    Parent,
}
...
/// Which own-line test a member's stripped-shell leading run is graded by — prettier's own
/// asymmetry, and a fact about WHICH of its handlers answers the position rather than a
/// choice of ours ([`Printer::member_shell_run_keeps_its_line`]).
#[derive(Clone, Copy, PartialEq)]
enum ShellRunOwnLine {
    /// `handleUnionTypeComments` — it binds every in-union comment to the PRECEDING member
    /// and prints an own-line one above the `| `, and its own-line test reads BACKWARDS
    /// only (`hasNewline(text, locStart(comment), { backwards: true })`,
    /// [`Printer::is_own_line_comment`]). A union's LATER member, the only position with a
    /// preceding member to bind to.
    PrecedingMember,
    /// `printLeadingComment`, whose hardline wants a newline on BOTH sides
    /// ([`Printer::block_comment_isolated_own_line`]). Every INTERSECTION member — no
    /// union handler fires there — and a union's FIRST member, which has no preceding
    /// member for the handler to bind to, so it falls to the leading rule like any other
    /// value. Measured at all three: `B | (⏎/* c */ C)` hoists, while `B & (⏎/* c */ C)`
    /// and `(⏎/* c */ C) | B` collapse, each agreeing with its own shell-free authoring.
    LeadingComment,
}

/// An intersection member's own stripped-shell leading run, at the seam where the MEMBER
/// GAP owns it rather than the shell ([`Printer::intersection_member_shell_run`]).
///
/// Two sinks, because the run straddles the boundary separator, and `claim` is the region
/// both halves came out of — what the shell's own emitters stand down under
/// ([`Printer::with_claimed_shell_leading_run`]).
///
/// ⚠️ **No loop reads these fields.** Three parts, each of which is a DROP, a REORDER or a
/// DOUBLE-PRINT if taken alone, so they are placed by exactly two seams that pair them with
/// the thing that makes each safe: [`Printer::push_member_shell_run_boundary`] (the pull
/// and the hardline it obliges) and [`Printer::push_member_shell_run_body`] (the run and
/// the claim). The compact and forced-multiline loops are twins and both take the same two
/// calls — the shape [`Printer::push_hoisted_member_doc`] is built around, for the same
/// reason it gives.
struct MemberShellRun {
    /// A `//` the author left on the BOUNDARY's own line, inside a shell that strips: the
    /// `&` is what that line becomes, so the comment keeps it. `None` at every other
    /// spelling, including a shell whose own `(` opens a line of its own — there the
    /// comment's line is the MEMBER's, and pulling it up would be a relocation.
    boundary_line: Option<DocId>,
    /// The rest of the run, at the member's own gap.
    rest: DocBuf,
    /// The whole region — the claim the member doc is built under.
    claim: Span,
}

/// What the per-boundary loop has already resolved about ONE continuation member, before
/// it can choose that member's separator ([`Printer::intersection_member_facts`]).
///
/// Bundled because the two loops are TWINS — the compact, width-decided one and the
/// forced-multiline one — and each of these is a fact a boundary rule reads. A fact
/// resolved at one loop and not the other is how a single authoring reaches two fixed
/// points depending on whether some unrelated gap happened to carry an isolated comment,
/// which is the standing hazard this module keeps paying for. Resolved once, for both.
///
/// Each is also an answer the loop needs for its OWN decision, so threading them to the
/// body builder rather than letting it re-derive them saves a second byte scan and a
/// second comment search over a range the loop has just walked.
struct IntersectionMemberFacts {
    /// `list_member_frozen` — Rule A, also feeding the loop's must-break tracking.
    frozen: bool,
    /// The `&` this member follows. `None` both where the whole intersection is
    /// comment-free — nothing bounded by it can have anything to place — and where the
    /// byte is not there to find.
    separator: Option<u32>,
    /// The member's own stripped-shell leading run, where the MEMBER GAP owns it
    /// ([`MemberShellRun`]).
    shell_run: Option<MemberShellRun>,
}
...
impl<'a> Printer<'a> {
    ...
    /// comment that KEEPS ITS OWN LINE ([`Self::member_shell_run_keeps_its_line`]): a gap
    /// whose every block is glued (`(/* b */ b)`), or a comment-free one, keeps its block
    /// inline and is already idempotent, so it returns empty (the general member arm).
    /// Peels every redundant nesting layer (`((// c⏎ b))` → `b`) to match the detection
    /// window. The narrow shared [`Self::stripped_paren_leading_line_comments`]
    ...
    fn stripped_redundant_paren_member_leading_run(&self, t: &TSType<'_>) -> CommentVec<'_> {
        ...
        let Some(region) =
            self.member_shell_own_line_run_region(t, ShellRunOwnLine::PrecedingMember)
        else {
            return smallvec![];
        };
        self.comments_to_emit_between(region.start, region.end)
            .collect()
    }

    /// The member's own stripped shell's leading REGION, where its run holds a comment
    /// that keeps a line of its own — **one span for the two things that must agree**: the
    /// window the run is collected from, and the claim the shell then stands down under
    /// ([`Printer::shell_leading_run_claimed`]).
    ///
    /// It opens at the shell's own `(` rather than one byte past it, because the claim is
    /// matched by containment from that byte; no comment can begin there, so the collected
    /// run is the same either way. It ends past a transparent composite's dropped operator
    /// where [`Self::stripped_shell_hoist_composite_head`] says so — and that widening is
    /// the whole reason this is one function: read as two, the window and the trigger drift
    /// and one source-contiguous run splits across two emitters (the ⚠️ on that method).
    ///
    /// Both of the union's two answers for such a run are built from this one region: a
    /// LATER member hoists the comments above its `| `
    /// ([`Self::stripped_redundant_paren_member_leading_run`]), a FIRST member has no `| `
    /// above it and claims the region instead, so the loop's own `first_leading` emitter
    /// prints the run after the pipe at the member offset — which is where the shell-free
    /// authoring of the same comment lands, and what prettier emits. The INTERSECTION's
    /// single answer is built from it too ([`Self::intersection_member_shell_run`]), which
    /// is what makes `own_line` a parameter rather than a constant: the three positions
    /// differ only in which of prettier's handlers grades the run ([`ShellRunOwnLine`]).
    fn member_shell_own_line_run_region(
        &self,
        t: &TSType<'_>,
        own_line: ShellRunOwnLine,
    ) -> Option<Span> {
        let shell = redundant_member_shell(self, t)?;
        if self.paren_shell_retains_for_trailing_run(shell) {
            return None;
        }
        let (leading_gap, _) = paren_shell_gaps(shell);
        let end = self
            .stripped_shell_hoist_composite_head(t, own_line)
            .unwrap_or(leading_gap.end);
        let run: CommentVec<'_> = self
            .comments_to_emit_between(shell.span.start, end)
            .collect();
        self.member_shell_run_keeps_its_line(&run, end, own_line)
            .then(|| Span::new(shell.span.start, end))
    }

    /// Whether a run inside a member's STRIPPED shell holds a comment that must keep a
    /// line of its own — a `//`, or a block the author gave a line of its own, which is
    /// **two different tests, one per seam**, because prettier's own answer is.
    ///
    /// Both are the enclosing member gap's to place, for the same reason: the shell is
    /// gone, so the line the author gave the comment is the member gap's line, and it is
    /// exactly the line the shell-free authoring of that comment gets
    /// (`B |⏎/* c */⏎C` — prettier agrees). Left to the shell's own
    /// [`ShellPair::Stripped`] collapse, which spaces every block unconditionally, an
    /// isolated block came back GLUED after the `| `; the reparse then reads it as an
    /// inline block, nothing forces the member break any more, and the whole union
    /// printed flat on pass 2. Own-line-ness is authoring signal
    /// ([`conformance_prettier.md`](../../../../../docs/conformance_prettier.md)
    /// §Comment Position Philosophy) and the one break-forcing property a layout can
    /// destroy, so the emitter that can keep it has to.
    ///
    /// ⚠️ **The UNION's test is one-sided and the INTERSECTION's two-sided**, and that is
    /// prettier's asymmetry rather than a choice: `handleUnionTypeComments` fires for a
    /// union alone, binding every in-union comment to the preceding member and printing an
    /// own-line one above the `| ` — and its own-line test reads BACKWARDS only
    /// (`hasNewline(text, locStart(comment), { backwards: true })`,
    /// [`Printer::is_own_line_comment`]). No handler fires for an intersection, so its
    /// members fall to `printLeadingComment`, whose hardline needs a newline on BOTH sides
    /// ([`Printer::block_comment_isolated_own_line`]). Measured at both seams:
    /// `B | (⏎/* c */ C)` hoists and `B & (⏎/* c */ C)` collapses, each agreeing with its
    /// own shell-free authoring.
    ///
    /// Asked per comment against its OWN next neighbour, the discipline
    /// [`Printer::block_comment_isolated_own_line`] states: at the intersection seam
    /// `/* c1 */ /* c2 */` written together on one own line is a pair the author glued,
    /// and neither half is isolated. The verdict is the RUN's, though — one such comment
    /// takes the whole run out of the shell, because the run is one contiguous region and
    /// splitting it would leave two emitters at two indents
    /// ([`comments.md`](../../../../../docs/comments.md): a gap's claim is a partition).
    fn member_shell_run_keeps_its_line(
        &self,
        run: &[&Comment],
        run_end: u32,
        own_line: ShellRunOwnLine,
    ) -> bool {
        run.iter().enumerate().any(|(i, comment)| match own_line {
            ShellRunOwnLine::PrecedingMember => self.is_own_line_comment(comment),
            ShellRunOwnLine::LeadingComment => {
                let next = run.get(i + 1).map_or(run_end, |c| c.span.start);
                !comment.is_block || self.block_comment_isolated_own_line(comment, next)
            }
        })
    }

    /// An intersection member's own stripped-shell leading run, where it KEEPS A LINE OF
    /// ITS OWN ([`Self::member_shell_own_line_run_region`]) — the union hoist's twin, one
    /// member over, and the same statement: *a leading run inside a stripped shell must
    /// lay out exactly as the identical run with the shell removed.*
    ///
    /// With the pair gone the run is the MEMBER GAP's, so it takes that gap's emitter —
    /// [`Printer::push_leading_comment_run`], prettier's `printLeadingComment` read
    /// straight. Left to the shell's own [`ShellPair::Stripped`] collapse, which spaces
    /// every block unconditionally, an own-line block came back GLUED after the `&`
    /// (`B & (⏎/* c */⏎C)` → `B & /* c */ C`, where the shell-free authoring — and
    /// prettier — keep the author's line). Own-line-ness is the one break-forcing property
    /// a layout can destroy, so the emitter that can keep it has to.
    ///
    /// ⚠️ **The gap's emitter, not a hardline of the shell's.** Reading
    /// [`Printer::block_comment_isolated_own_line`] at the shell instead — the emitter's
    /// own `Kept` rule, applied to a stripped pair — is not idempotent: it asks about BOTH
    /// sides of the comment, and the layout it produces does not reproduce its own trigger
    /// (a run's first comment lands own-line with the second still glued to it, so pass 2
    /// reads neither as isolated and collapses the whole run). The forward-gap rule this
    /// emitter uses reads only the gap AFTER each comment, which every output does
    /// reproduce.
    ///
    /// `separator` is the `&` this member follows, and it answers the run's FIRST comment
    /// alone: a `//` the author wrote on that operator's line trails it in the output too,
    /// because the `(` between them does not print. Asked of the `&` rather than of the
    /// `(` — [`Printer::split_open_delimiter_glued_run`]'s question, which is the right
    /// one only while the delimiter survives: a shell whose `(` opens a line of its own
    /// (`B &⏎( // c⏎C)`) has that comment on the MEMBER's line, and pulling it onto the
    /// `&` is a relocation the paren-free authoring never produces. Declined outright when
    /// the pre-member gap holds a comment of its own — that run prints after the
    /// separator, so pulling only the shell's half past it would REORDER the two.
    ///
    /// Declines a FROZEN member (its shell rides inside the verbatim slice) and one whose
    /// transparent composite head already claimed the region
    /// ([`Self::later_member_transparent_head_line_comment_claim`]), so the two answers
    /// cannot both fire on one run.
    ///
    /// ⚠️ **That second decline is load-bearing, and the two seams are NOT redundant even
    /// though they now share an emitter.** They draw different WINDOWS over the same
    /// member: the transparent-head claim is the leading-EDGE descent's
    /// ([`Printer::leading_edge_shell_line_comment_claim`], which reaches a shell one link
    /// INSIDE the member), this one is the member's OWN shell widened past a dropped
    /// operator. Dropping the decline so this seam answered both was measured: three
    /// fixtures move, two of them existing divergences
    /// (`composite_head_line_comment_under_shell_run`,
    /// `intersection_member_single_member_head_line_comment`). One region, one claim —
    /// the wider descent answers first, exactly as
    /// [`Self::union_member_leading_claim`] resolves the same order for the union.
    fn intersection_member_shell_run(
        &self,
        t: &TSType<'_>,
        separator: Option<u32>,
        frozen: bool,
    ) -> Option<MemberShellRun> {
        if frozen
            || self
                .later_member_transparent_head_line_comment_claim(t)
                .is_some()
        {
            return None;
        }
        let claim = self.member_shell_own_line_run_region(t, ShellRunOwnLine::LeadingComment)?;
        let d = self.d();
        let mut boundary_line = None;
        let mut resume = claim.start;
        if let Some(sep) = separator
            && !self.has_comments_to_emit_between(sep + 1, claim.start)
            && let Some(first) = self.comments_to_emit_between(claim.start, claim.end).next()
            && !first.is_block
            && self.is_same_line(sep, first.span.start)
        {
            let mut parts = DocBuf::new();
            // A `line_suffix`, never an inline emission: the PREVIOUS member's own lifted
            // trailing run is already pending in the buffer, and an inline `//` here
            // renders ahead of it — so the flush lands the earlier comment BEHIND this one
            // on the same output line, welding the two into one (`B // inj⏎& ( // c1⏎C)` →
            // `B & // c1 // inj`, the author's first comment swallowed as text inside the
            // second). Queued, the two drain in source order and the run separator breaks
            // between them ([`Printer::build_trailing_comment_doc`], the same shape the
            // forced-multiline loop's after-`&` emission takes, for the same reason).
            parts.push(self.build_trailing_comment_doc(first));
            self.push_delimiter_glued_blank(&mut parts, first.span.end, claim.end);
            boundary_line = Some(d.concat(&parts));
            resume = first.span.end;
        }
        let mut rest = DocBuf::new();
        self.push_leading_comment_run(
            &mut rest,
            self.comments_to_emit_between(resume, claim.end),
            claim.end,
    ...
    /// Place a member's stripped-shell run's BOUNDARY half and answer the separator — the
    /// two halves of ONE act, which is why no caller gets to take them apart.
    ///
    /// A `//` the author left on the operator's line is pushed BEFORE the separator, so it
    /// keeps that line; the separator then has to be a `hardline`, because the member
    /// cannot follow a `//` on it. Every break the run carries of its own sits PAST this
    /// separator, so an ordinary `line` here would be left to a group that may still fit
    /// flat — and the comment would SWALLOW the member. A loop that pulled without
    /// hardlining is not a layout difference, it is lost code (`gaps:audit` found exactly
    /// that shape).
    ///
    /// `ordinary` is the loop's own separator, evaluated only when nothing was pulled —
    /// the compact loop's arms the group that decides them, so building it eagerly would
    /// mark a group needed on a boundary that no longer has a breakable line.
    fn push_member_shell_run_boundary(
        &self,
        parts: &mut DocBuf,
        shell_run: Option<&MemberShellRun>,
        ordinary: impl FnOnce() -> DocId,
    ) -> DocId {
        match shell_run.and_then(|run| run.boundary_line) {
            Some(pulled) => {
                parts.push(pulled);
                self.d().hardline()
            }
    ...
    /// Place a member's stripped-shell run's MEMBER-GAP half and answer the claim — the
    /// pair every caller takes, for the reason [`Printer::leading_edge_claim_and_start`]
    /// gives for its own: emitting the run without the claim prints it TWICE (the shell's
    /// emitters are still live), and claiming without emitting it DROPS it
    /// ([`comments.md`](../../../../docs/comments.md) hazards 1 and 3). Taken separately
    /// they are two chances to disagree; taken here, neither is expressible.
    ///
    /// Pushed AFTER the gap's own leading run, which is where source order puts it: the
    /// gap's comments sit between the `&` and the shell's `(`, this run just inside it.
    fn push_member_shell_run_body(
        &self,
        parts: &mut DocBuf,
        shell_run: Option<&MemberShellRun>,
    ) -> Option<Span> {
        let run = shell_run?;
        parts.extend(run.rest.iter().copied());
        Some(run.claim)
    }

    /// Resolve the three per-member facts both intersection loops need before choosing a
    /// separator ([`IntersectionMemberFacts`]) — one resolver, so the twins cannot answer
    /// one member two ways.
    ///
    /// `has_comments` is the intersection-wide zero-comment gate, and it guards BOTH
    /// searches: with nothing anywhere in the span, no gap this member owns can hold
    /// anything, so neither the `&` byte scan nor the shell-run search can find one. That
    /// matters because this runs for every member of every intersection, nearly all of
    /// them comment-free. The forced-multiline caller passes `true` rather than its own
    /// answer: that layout is reached only through
    /// [`Self::intersection_needs_line_comment_layout`], both of whose disjuncts require a
    /// comment inside the span, so the gate is satisfied by construction there.
    ///
    /// ⚠️ `freeze_first` is the CALLER's, and the two loops genuinely differ: the
    /// forced-multiline one passes its own, while the compact one passes `false` because it
    /// answers the first member's freeze separately (`first_frozen`, built into
    /// `first_parts` before the loop). Threaded rather than re-derived so that difference
    /// stays visible at the call sites instead of hiding inside this seam.
    fn intersection_member_facts(
        &self,
        ...
        let separator = has_comments
            ...
            .flatten();
        let shell_run = has_comments
            .then(|| self.intersection_member_shell_run(member, separator, frozen))
            .flatten();
        IntersectionMemberFacts {
            frozen,
            separator,
    ...
    /// Who owns a union member's leading comment REGION, as the pair every gap takes: the
    /// claim to silence the member's shell under, and the window end the gap's own run
    /// emitter scans to ([`Printer::leading_edge_claim_and_start`]).
    ///
    /// The leading-EDGE descent answers first — where it claims, its region is the wider
    /// one (it reaches past a transparent composite's dropped operator) and it is the
    /// region the member's own emitters already stand down on. Only where it declines,
    /// which for a shell that IS the member it always does (every list gap has its own
    /// answer for that shape, [`Printer::leading_edge_shell_claim`]), does the FIRST
    /// member's own shell get to claim
    /// ([`Self::member_shell_own_line_run_region`]). A LATER member never needs it: its
    /// run is hoisted above the `| ` instead, which is what `run_owned_above` says.
    fn union_member_leading_claim(
        &self,
        run_owned_above: bool,
        is_first: bool,
        t: &TSType<'_>,
    ) -> (Option<Span>, u32) {
        match self.leading_edge_claim_and_start(run_owned_above, t) {
            (None, end) if is_first && !run_owned_above => self
                .member_shell_own_line_run_region(t, ShellRunOwnLine::LeadingComment)
                .map_or((None, end), |claim| (Some(claim), claim.end)),
            resolved => resolved,
        }
    }
    ...
    /// (`composite_head_line_comment_under_shell_run`).
    ///
    /// Two conditions narrow it, and both are load-bearing. A head gap whose every comment
    /// is GLUED needs no widening: the collapse prints such a comment where this gap would,
    /// so the two answers already coincide and widening would only move it. And the region
    /// itself stops where [`Printer::shell_printed_head_start`] stops — in particular at a
    /// sole member carrying a run of its OWN, which the composite composes with its head
    /// gap and hoists as one run; taking half of that stranded the rest after the `| `.
    ///
    /// ⚠️ **The narrowing asks the same question the hoist does**
    /// ([`Self::member_shell_run_keeps_its_line`]), and it has to: the shell's run and the
    /// head gap's are ONE run in source, so a window narrower than the hoist's trigger
    /// splits that run across two emitters. Read as "a `//` in the head gap" while the
    /// hoist itself only fired on a `//`, the two agreed by construction; once an ISOLATED
    /// own-line block hoists too, the block spelling of the same authoring
    /// (`B | (⏎/* c1 */⏎|⏎/* d1 */⏎C)`) hoisted `/* c1 */` alone and left `/* d1 */` to the
    /// composite — two emitters, two indents, and a fixed point neither pass agreed on.
    fn stripped_shell_hoist_composite_head(
        &self,
        t: &TSType<'_>,
        own_line: ShellRunOwnLine,
    ) -> Option<u32> {
        let inner = unwrap_parenthesized(t);
        self.shell_head_past_operator(t).filter(|&end| {
            let head: CommentVec<'_> = self
                .comments_to_emit_between(inner.span().start, end)
                .collect();
            self.member_shell_run_keeps_its_line(&head, end, own_line)
        })
    }
    ...
    ///
    /// ⚠️ **The blank between two comments is the STRICT question**
    /// ([`Printer::push_blank_preserving_hardline`], the emitter every other leading run
    /// already goes through). A bare newline COUNT reads a TRANSPARENT composite's dropped
    /// operator as a blank line — this run's window reaches past one
    /// ([`Self::stripped_shell_hoist_composite_head`]), so `(⏎/* c */⏎|⏎/* d */⏎C)` has two
    /// newlines between the comments with a `|` on the line between, and the `|` does not
    /// print. A FABRICATED blank (`comments.md` hazard 5), and its own fixed point: the
    /// emitted blank reads back as the author's, so no gate reaches it — only a prettier
    /// `compare` does (`union_member_shell_dropped_operator_blank`).
    fn push_union_member_leading_run(&self, parts: &mut DocBuf, run: &[&Comment]) {
        ...
        for (j, comment) in run.iter().enumerate() {
            ...
            self.push_blank_preserving_hardline(parts, comment.span.end, next.span.start);
        }
    }
    ...
    /// in the output.
    /// [`Self::union_member_has_leading_comments`] with the FIRST member's run start as the
    /// layout that emits it knows it (`run_start`, [`Self::build_union_type_doc_inner`]):
    /// a run handed in from the enclosing seam sits ahead of `union.span.start`, outside
    /// the window that predicate reads, yet it prints after the first member's `| `
    /// exactly as an in-span run does — and the reparse reads it back as the member's
    /// (`| /* a */ T`), so every layout question keyed on "the first member carries a
    /// leading run" must count it, or the answer flips between passes.
    fn union_member_has_leading_run(
        &self,
        union: &TSUnionType<'_>,
        i: usize,
        run_start: u32,
    ...
    fn push_transparent_head_member_doc(
        ...
        // The MEMBER GAP's own emitter, for the reason
        // [`Self::intersection_member_shell_run`] gives: the shell is gone, so this run is
        // the gap's, and the shell's own [`ShellPair::Stripped`] collapse answers neither
        // of the two questions the gap has. It spaces every block, so a block the author
        // gave its own line after the dropped operator came back GLUED to the member
        // (`N & (⏎// x⏎|⏎/* d */⏎O)` → `/* d */ O`); and its blank test is a newline COUNT,
        // which reads the dropped operator's own line as an author blank and FABRICATES
        // one (`comments.md` hazard 5 — its own fixed point, so only a prettier `compare`
        // finds it). Both are `push_leading_comment_run`'s already
        // (`union_member_shell_dropped_operator_blank`).
        self.push_leading_comment_run(
            parts,
            self.comments_to_emit_between(claim.start, claim.end),
            claim.end,
            LeadingGlue::Adjacent,
        );
    ...
    ///   run ([`Printer::shell_leading_run_claimed`]). ⚠️ **That stand-down is now the
    ///   common case, not the exception**: the member gap claims every run that keeps a
    ///   line of its own ([`Self::intersection_member_shell_run`]), so what reaches this
    ///   emission is exactly the runs the [`ShellPair::Stripped`] collapse is right for —
    ///   a block the author glued. The doc here used to read "a `//`-leading shell is
    ///   claimed upstream and a block run never is", which was true only while no member
    ///   seam claimed a block;
    ...
    fn intersection_boundary_opens_for_shell_run(
        ...
        // ⚠️ **The member gap's OWN run answers first, from the resolved value rather than
        // from a predicate of its own** — it is emitted with real hardlines at the member
        // gap ([`Self::push_member_shell_run_body`]), so a boundary that hugs past it puts
        // those breaks OUTSIDE the continuation `indent` and the member lands at column
        // zero (`{ a: 1 } & /* c */⏎D`, against the shell-free authoring's indented form,
        // which is also prettier's).
        //
        // The predicate below cannot stand in for it: its window is the shell's own
        // leading gap, where the CLAIM's reaches past a transparent composite's dropped
        // operator ([`Self::member_shell_own_line_run_region`]). A run that sits entirely
        // in that widened part (`{ a: 1 } & (⏎|⏎/* c */⏎D)`) is invisible here, and every
        // object-free boundary only hid it by breaking anyway on the neither-is-object
        // arm. One trigger, one window
        // ([`Self::stripped_shell_hoist_composite_head`]'s ⚠️ states the same rule for the
        // union's).
        shell_run.is_some()
            || (run_indent_follows_boundary
                && self.intersection_boundary_leading_run_ends_line(prev))
    ...
    pub(in crate::printer) fn build_union_type_doc(&self, union: &TSUnionType<'_>) -> DocId {
        self.build_union_type_doc_inner(union, UnionLeadingGap::Parent)
    }

    /// Build a union VALUE doc for an enclosing seam whose gap opens at `gap_start` —
    /// just past its operator (`=` / `:` / `=>` / `as` / `extends` / `?` / `in`) or its
    /// opening delimiter (`<` / `[` / `(`) — handing the gap's glued leading block run into the union when
    ...
    /// whether the SEAM keeps its operator glued ([`UnionValueDoc::hugged`]), the one
    /// answer the seam's own layout must agree with — read here, beside the doc, so a
    /// seam cannot pair the doc with a re-derivation: a block ahead of the first member
    /// declines it ([`UnionLeadingGap::Seam`]), handed in or authored after the pipe,
    ...
    pub(in crate::printer) fn build_union_value_doc(
        ...
        let gap = UnionLeadingGap::Seam { gap_start, handed };
        UnionValueDoc {
            doc: self.build_union_type_doc_inner(union, gap),
            run_handed: handed.is_some(),
            hugged: self.union_seam_hugs(gap_start, union),
        }
    }

    /// Whether the union's own join HUGS under a seam whose gap opens at `gap_start` —
    /// [`Self::union_prints_hugged_with`]'s answer alone, without the seam-level "no block
    /// in the gap" clause of [`Self::union_seam_hugs`]: `true` for a block the author
    /// separated from the first member, where the seam takes its non-hug layout and the
    /// union hugs below the run. The one ask a seam's own run emitter makes.
    pub(in crate::printer) fn union_hugs_behind_seam_run(
        &self,
        gap_start: u32,
        union: &TSUnionType<'_>,
    ) -> bool {
        self.union_prints_hugged_with(
            union,
            UnionLeadingGap::Seam {
                gap_start,
                handed: None,
            },
        )
    ...
    /// [`UnionValueDoc::hugged`] without building the doc — the seam-level hug answer,
    /// for a gate that chooses its path before it builds (a type argument's inline arm,
    /// a cast's or bound's hanging tail): the union prints hugged
    /// ([`Self::union_prints_hugged_with`]) and the seam's gap `[gap_start, first
    /// member)` holds no block comment SEPARATED from what follows it. Equal to the answer
    /// [`Self::build_union_value_doc`] pairs with its doc, by construction.
    pub(in crate::printer) fn union_seam_hugs(
        &self,
        gap_start: u32,
        union: &TSUnionType<'_>,
    ) -> bool {
        ...
        // Only a block the author SEPARATED from the union's head declines the seam's
        // glue — the hug arm would glue it, which the next pass reads as the glued
        // spelling. A glued block that leaves the hug standing (a multi-line block, the
        // sole member's leading comment) rides the hug arm glued, as prettier prints it
        // (`: /* c⏎d */ { … } | null`).
        self.union_hugs_behind_seam_run(gap_start, union)
            && !self
                .comments_on_page_between(gap_start, first.span().start)
                .any(|c| c.is_block && !self.comment_hugs_next(c))
    }

    /// Whether `comment` lies wholly AHEAD of the union's span — before its authored
    /// leading pipe, or before the first member when none is authored — i.e. in the
    /// enclosing seam's gap rather than the union's own. The one distinction prettier's
    /// binding draws for a multi-line block: outside the span it is the union's, inside
    /// it the first member's.
    fn union_comment_precedes_head(&self, union: &TSUnionType<'_>, comment: &Comment) -> bool {
        comment.span.end <= union.span.start
    }
    ...
    /// - the run is not GLUED to what follows it — the union's head token: an
    ///   AUTHORED leading pipe (`/* c */ | { … }`, where the handed run lands after
    ///   the synthesized pipe beside the in-span gap's own, the two emitted as ONE run
    ///   from the handed start), else the member, or an owned comment leading it (the
    ///   physical next, not the emit-set's view): a broke-after run is the value seam's
    ///   own break-materialization question, not this binding one;
    ...
    pub(in crate::printer) fn union_external_leading_run_start(
        &self,
        gap_start: u32,
        union: &TSUnionType<'_>,
    ) -> Option<u32> {
        ...
        // The caller's gap ends at the union's head: the authored pipe, else the member.
        let head_start = union.span.start;
        let comments: CommentVec<'_> = self
            .comments_to_emit_between(gap_start, head_start)
            .collect();
        let (first, last) = (comments.first()?, comments.last()?);
        if !comments.iter().all(|c| c.is_block)
            || !self.is_same_line(last.span.end, self.blank_scan_end_after(last, head_start))
            || self
    ...
    fn build_union_type_doc_inner(&self, union: &TSUnionType<'_>, gap: UnionLeadingGap) -> DocId {
        ...
        let external_run_start = match gap {
            UnionLeadingGap::Seam { handed, .. } => handed,
            UnionLeadingGap::Parent => None,
        };
        if union.types.is_empty() {
            return d.empty();
        }
        let first_member_start = union.types[0].span().start;
        // Where the first member's leading run STARTS: the handed run's start when the
        // enclosing seam handed one in ([`Self::union_external_leading_run_start`]), else
        // the union's own head. One run from there to the member, whichever layout emits
        // it — a handed run and the in-span gap's own (`/* c1 */ | /* c2 */ {`) are one
        // run, so no layout prints the head twice (docs/comments.md hazard 3).
        let run_start = external_run_start.unwrap_or(union.span.start);
        ...
        // doc renders it.
        //
        // Under a seam a block GLUED ahead of the first member (`: /* c */ { … } | null`,
        ...
        // handed run lands. The seam reads its own glue answer off `UnionValueDoc::hugged`.
        if self.union_prints_hugged_with(union, gap) {
            let mut parts = DocBuf::new();
            // Leading block comments before the first type (`| /* c */ A` — between the
            // authored `|` and the first member; a line comment there declines the hug
            // above). Through the shared leading-run emitter, so a block the author BROKE
            // AFTER (`| /* c */⏎{ … }` — the seam's rule keeps the hug for it) takes its
            // soft `line` and stays on its own line when the enclosing seam hangs; glued
            // onto the member instead, it came back as the glued spelling, which the next
            // pass binds to the member and declines (`blanks:audit`, `*/⟨⟩{`).
            if has_leading_run {
                self.push_leading_comment_run(
                    &mut parts,
                    self.comments_to_emit_between(run_start, first_member_start),
                    first_member_start,
        ...
        if has_comments && !head_region_claimed {
            ...
            //
            // ⚠️ The member's own STRIPPED shell is asked through the hoist's own
            // resolver ([`Self::member_shell_own_line_run_region`]), not through the
            // `//`-keyed predicate beside it: the two keyed the same question while both
            // read [`Printer::block_comment_isolated_own_line`], and the union's own-line
            // test is one-sided ([`Self::member_shell_run_keeps_its_line`]), so a block
            // the author opened a line for and then glued to its member
            // (`B | (⏎/* c */ C)`) routes to the width path under the narrower reading
            // and the hoist below never runs — the collapse the shell-free authoring of
            // the same comment does not take. Routing and emission ask one predicate,
            // which is the discipline this module keeps paying for.
            let has_paren_inner_leading_line_comments = union.types.len() > 1
                && union.types.iter().enumerate().any(|(i, t)| {
                    self.union_member_paren_leading_line_comment(t)
                        || (i > 0
                            && self
                                .member_shell_own_line_run_region(
                                    t,
                                    ShellRunOwnLine::PrecedingMember,
                                )
                                .is_some())
                });
            ...
            {
                return self.build_union_type_doc_with_line_comments(union, run_start);
            }
        }
        ...
        for (i, t) in union.types.iter().enumerate() {
            ...
            if frozen {
                ...
                parts.push(if has_comments {
                    ...
                    let has_leading = self.union_member_has_leading_run(union, i, run_start);
    ...
    ///
    /// `gap_start` is the keyword's end: the keyword→union gap is this seam's, and its
    /// block run rides INSIDE the hang — a run glued to the first member is handed into
    /// the union ([`Self::build_union_value_doc`], where it declines the hug and lands
    /// after the synthesized pipe), any other run leads the union through the value-gap
    /// emitter, whose separators keep a broke-after or own-line block on its own line
    /// when the hang breaks. Either way the seam's glue answer
    /// ([`UnionValueDoc::hugged`]) decides the hang, so a block in the gap never welds
    /// `/* c */ | {` onto the keyword.
    pub(in crate::printer) fn build_union_hanging_indent_doc(
        ...
        // The seam's glue answer, not the bare syntactic `union_hug_shape`: this must
        // agree with the layout `build_union_value_doc` actually takes. A comment can
        // make it decline the hug and expand, and then the keyword has to break like any
        // other non-hugging union — asking the syntactic form alone keeps `as ` glued
        // while the members explode below it.
        let UnionValueDoc {
            doc: union_doc,
            run_handed,
            hugged,
        } = self.build_union_value_doc(gap_start, union);
        if hugged {
            return None;
        }
        ...
        let hung = if run_handed {
            ...
            self.prepend_rhs_comments(union_doc, gap_start, union.span.start)
        };
        Some(hang_after_operator(self.d(), hung))
    }
    ...
    /// ```
    ///
    /// `run_start` is where the first member's leading run begins — the union's own head,
    /// or a handed run's start ahead of it ([`Self::build_union_type_doc_inner`]).
    fn build_union_type_doc_with_line_comments(
        &self,
        union: &TSUnionType<'_>,
        run_start: u32,
    ) -> DocId {
        ...
        for (i, t) in union.types.iter().enumerate() {
            ...
            let (edge_claim, gap_end) = self.union_member_leading_claim(run_owned_above, i == 0, t);
            ...
            let mut first_leading = if i == 0 {
                self.build_leading_comments_multiline(run_start, gap_end, None)
            } else {
                DocBuf::new()
            };
            ...
            if i > 0 {
                    ...
                    find_separator_position(self.source, prev_type_end, type_start, b'|')
                {
                    ...
                    if self.union_member_has_leading_run(union, i - 1, run_start) {
                    ...
                    //
                    // Relocated above the `| `, the run is read back on the next pass as
                    // the PREVIOUS member's trailing run (`T /* b */⏎⏎/* c */⏎| U`), which
                    // the gap emitter above aligns under that member's own offset when it
                    // carries a leading run — prettier's `hasComment(node, Leading)`
                    // split. So the same offset is taken here, on the same question, or
                    // the two passes disagree by two columns (the `|⟨⟩␣` blank-audit
                    // shape, reached once a seam-handed run prints after the first `| `).
                    // `align` offsets the lines opened INSIDE it: the break opening the
                    // run goes in, the run's closing break — which opens the `| ` line,
                    // flush — comes back out.
                    let mut relocated = DocBuf::new();
                    relocated.push(d.hardline());
                    self.push_union_member_leading_run(&mut relocated, own_line);
                    self.push_union_member_leading_run(&mut relocated, &stripped_paren_leading);
                    let closing = relocated.pop();
                    if self.union_member_has_leading_run(union, i - 1, run_start) {
                        parts.push(d.align(2, d.concat(&relocated)));
                    } else {
            ...
            if frozen_member {
                ...
                let hoist_claim = self
                    .stripped_shell_hoist_composite_head(t, ShellRunOwnLine::PrecedingMember)
            ...
            } else if let Some((p, inner_union)) =
                // Asked through [`Self::paren_union_line_comment_member`], the accessor the
                // intersection's three readers ask — this is the fourth. Its first question
                // is `member_parens`, which reads [`Printer::printed_operand`] and so sees
                // THROUGH a transparent one-member composite: a pair around `| C` is not
                // required, so it does not survive, and the keep-inside layout is not this
                // member's. Spelled here as its own triple, the arm answered a shape
                // `member_pair_layout` routes to `MemberPairLayout::None`, retaining a pair
                // that strips and printing the shell's run INSIDE it — beside the copy the
                // widened `edge_claim` had already given the gap emitter above. Every pass
                // re-read the retained pair and added another copy, so a `//` in the
                // composite's head gap grew the run WITHOUT BOUND.
                //
                // ⚠️ Both halves of the accessor read THROUGH the author's extra layers,
                // because the ROUTER above does
                // ([`Self::union_member_paren_leading_line_comment`]) and a member it routed
                // here must find an emitter. A direct-child match plus the shallow one-paren
                // window missed `X | ((// c⏎A | B))`: the run fell to the default arm, whose
                // `ShellLeadingRun::Upstream` licence is granted on an upstream emitter
                // EXISTING — this arm — so nothing printed it and the comment was DROPPED.
                // The pair the shells collapse into is the outermost, and
                // `build_parenthesized_union_doc`'s window already spans from it to the
                // union, so any nesting depth emits the whole run.
    ...
    ///
    /// `gap_start` is the byte after the list's `<`: the seam's glue answer
    /// ([`Self::union_seam_hugs`]) — a block in the `<`→argument gap declines the hug at
    /// the seam, the union taking the list's group path with the run handed in.
    pub(crate) fn type_arg_union_prints_hugged(&self, gap_start: u32, ty: &TSType<'_>) -> bool {
        // `union_seam_hugs` subsumes `union_hug_shape`, so the shape is not re-tested
        // here — the brace clause is the only thing this position adds.
        matches!(unwrap_parenthesized(ty), TSType::Union(u)
            if self.union_seam_hugs(gap_start, u) && union_has_brace_member(u))
    }

    /// The run a seam's list emitter must LEAVE to a union item: `Some(run start)` when
    /// `ty` is a bare union whose gap from `gap_start` holds a glued block run
    /// [`Self::build_union_value_doc`] will hand into it (the same test that call makes,
    /// [`Self::union_external_leading_run_start`], so the claim and the handoff cannot
    /// disagree); `None` for any other type, whose gap the emitter prints whole.
    pub(in crate::printer) fn union_seam_run_handoff(
        &self,
        gap_start: u32,
        ty: &TSType<'_>,
    ) -> Option<u32> {
        match ty {
            TSType::Union(u) => self.union_external_leading_run_start(gap_start, u),
            _ => None,
        }
    }
    ...
    ///   comment to the member, unless a parent starts where the union does (the
    ///   predicate's annotation at `is`, a conditional's check), and the union expands
    ///   one-member-per-line with the comment after the pipe. Every enclosing seam asks
    ///   it ([`UnionLeadingGap::Seam`] — the value seams, the containers, the keywords;
    ///   `union_hug_gap_block_comment` and its `_container` / `_cast` / `_keyword`
    ///   siblings) and hands the run into the union; the two parent positions ask the
    ///   bare span ([`UnionLeadingGap::Parent`]) and keep hugging behind it. A block
    ///   the author separated from the member by a newline is not glued: it binds to
    ///   the union (prettier's own-line binding) and the hug stands. "Glued" is the
    ///   comment's own forward glue ([`Self::comment_hugs_next`]) against whatever follows
    ///   it — the member, an authored leading pipe (prettier's union span starts there,
    ///   so `/* c */ | {` binds like `| /* c */ {`), or the next comment. Two glued shapes
    ///   still keep the hug: a MULTI-LINE block ahead of the span, which prettier binds to
    ///   the union (inside the span it is the member's), and the sole member of a
    ///   single-member union, which is its member (`union_hug_gap_block_comment_multiline`,
    ///   `_leading_pipe`);
    ...
    pub(crate) fn union_prints_hugged(&self, union: &TSUnionType<'_>) -> bool {
        self.union_prints_hugged_with(union, UnionLeadingGap::Parent)
    }
    ...
    /// off the union alone. The seams ask it through [`Self::build_union_value_doc`] /
    /// [`Self::union_seam_hugs`], beside the doc it governs; the two parent positions ask
    /// [`Self::union_prints_hugged`] (`Parent`).
    fn union_prints_hugged_with(&self, union: &TSUnionType<'_>, gap: UnionLeadingGap) -> bool {
        ...
        // Under a seam, a block GLUED to the first member — the comment nearest the
        // member, between the operator and the member, ending on the member's line —
        // declines (see the doc above), wherever it sits: in the seam's own gap, outside
        // the span, or around an authored pipe. Asked before the in-span fast path, which
        // cannot see the seam's gap; one range from the operator, so a handed run and the
        // in-span spellings are one question. A block the author separated from the
        // member by a newline binds to the union instead (prettier's own-line binding)
        // and keeps the hug — the seam's non-hug layout carries it on its own line
        // (`UnionValueDoc::hugged`).
        // "Glued" is the comment's own forward glue (`comment_hugs_next`, prettier's
        // `hasNewline` after the comment): what follows it may be the member, an authored
        // leading pipe, or another comment — prettier's union span starts at that pipe, so
        // `/* c */ | {` and `/* c */ |⏎{` bind to the member like `| /* c */ {`. Two shapes
        // keep the hug: a SINGLE-member union (it is its member — prettier drops the node —
        // whose leading comment blocks no hug), and a MULTI-LINE block outside the span
        // (`: /* c⏎d */ { … } | null`), which prettier binds to the union; inside the span
        // (after an authored pipe) a multi-line block is the member's like any other.
        if let UnionLeadingGap::Seam { gap_start, .. } = gap
            && union.types.len() > 1
            && self
                ...
                .is_some_and(|c| {
                    c.is_block
                        ...
                        && !(c.multiline && self.union_comment_precedes_head(union, c))
                })
        {
            return false;
        }
        ...
        !self.union_has_comments_between_members(union)
            ...
            // (the hug path could not print it); a block there is the seam's question
            // above, and hugs at a parent position.
    ...
    pub(in crate::printer) fn build_intersection_type_doc(
        ...
        if has_comments && let Some(first_member) = intersection.types.first() {
            ...
            if !head.run.is_empty() {
                ...
                // panic, it fired on none of the 4,725 fixtures, on none of `gap_audit`'s
                // 5.2M injections, and on none of a 7,488-cell composite-under-shell-run
                // sweep. It stays because the claim is matched by CONTAINMENT, so a wider
                // claim from some other seam is the residue that argument does not cover.
                // ⚠️ The union-FIRST-member double-print this comment once named as the
                // change that would make it fire is CLOSED, and it did not: that arm was
                // routed through [`Self::paren_union_line_comment_member`] rather than by
                // relaxing the resolver's stop, so the stop still holds and the belt is
                // still a belt. Relaxing the stop itself remains the candidate.
    ...
    fn build_intersection_compact_doc(
        ...
        for i in 1..intersection.types.len() {
            ...
            let sep =
                self.push_member_shell_run_boundary(&mut member, facts.shell_run.as_ref(), || {
                    if breaks {
    ...
    fn build_intersection_type_doc_with_line_comments(
        ...
        for i in 1..types.len() {
            ...
            // Everything the boundary rules below read about this member, resolved once by
            // the seam the compact twin also calls ([`Self::intersection_member_facts`]).
            // `true`: this layout is reached only because a comment is in play, so the
            // whole-intersection gate is satisfied by construction.
            let facts = self.intersection_member_facts(intersection, i, true, freeze_first);
            let amp = facts.separator;
            ...
            //
            // The member's own stripped-shell leading run, where the MEMBER GAP owns it
            // ([`Self::intersection_member_shell_run`]) — the compact loop's twin question,
            // asked at the same point, so one authoring cannot reach two fixed points
            // depending on which loop some other gap routed it to.
            let sep =
                self.push_member_shell_run_boundary(&mut unit, facts.shell_run.as_ref(), || {
                    if should_break {
                        d.hardline()
                    } else {
                        d.text(" ")
            ...
            let shell_claim = self.push_member_shell_run_body(&mut unit, facts.shell_run.as_ref());
            ...
            if facts.frozen {
                ...
                .flatten()
            {
                member_held = self.with_claimed_shell_leading_run(shell_claim, || {
                    Some(self.push_hoisted_member_doc(
                        &mut unit,
                        cur,
                        inner,
                        self.build_intersection_line_comment_member_doc(inner, member_parens),
                ...
                unit.push(self.with_claimed_shell_leading_run(shell_claim, || {
                    self.build_intersection_line_comment_member_doc(cur, member_parens)
                }));
            }
            if is_last {
    ...
    /// comments/`&` separator.
    ///
    /// `has_comments` is the caller's whole-intersection window answer: `false` proves both
    /// gaps around this member are bare, so neither is searched. `separator` is the `&`
    /// this member follows, found once by that same caller behind the same gate and shared
    /// with the shell-run resolver ([`Self::intersection_member_shell_run`]) — `None` both
    /// where there is nothing to bound and where the byte is not there to find, and either
    /// way the leading-gap run has no window and is skipped. `frozen` is the caller's
    /// `list_member_frozen` answer (also feeding its must-break tracking), threaded so the
    /// question is asked once per member. All three are the loop's answers, threaded rather
    /// than re-derived: a builder that re-scanned for its own `&` was a second byte walk
    /// over the range the loop had just walked.
    ///
    ...
    fn build_intersection_member_body_doc(
        ...
        if let Some(sep_pos) = facts.separator {
            let (run, breaks) =
        ...
        // The member's own stripped-shell run, past the half the separator above already
        // took ([`Self::intersection_member_shell_run`]). Emitted here — after the
        // pre-member gap's own run, which is where source order puts it — and the member
        // is then built with the whole region CLAIMED, so the shell's own emitters stand
        // down ([`comments.md`](../../../../docs/comments.md) hazard 3).
        let shell_claim = self.push_member_shell_run_body(&mut parts, facts.shell_run.as_ref());
        ...
        if facts.frozen {
            ...
            .flatten()
        {
            held_run = self.with_claimed_shell_leading_run(shell_claim, || {
                Some(self.push_hoisted_member_doc(
                    &mut parts,
                    t,
                    inner,
            ...
            parts.push(self.with_claimed_shell_leading_run(shell_claim, || {
                self.build_intersection_member_type_doc(t, member_parens)
            }));
        }
