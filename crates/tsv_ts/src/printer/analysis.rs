...
/// (ObjectPattern or ArrayPattern) as its value. The object EXPRESSION has no
/// counterpart — it expands on its own authored newline and on whatever its
/// contents force, never on a nesting count.
///
