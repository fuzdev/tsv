...
use super::statement_tail::StatementTerminator;
...
impl<'a> Printer<'a> {
    ...
    /// All three callers — an enum member's initializer, `export default` and `export =` —
    /// name [`Self::build_expression_doc`] today. They agree because the binaryish default
    /// answers all three, not because the seam decides for them: the parameter is what
    /// keeps a future caller from having the plain builder substituted silently, which is
    /// the bug the paragraph above warns about.
    #[inline]
    ...
    ///
    /// Only a kind that OWNS a terminator ([`StatementTerminator`]) trims one, which is what
    ...
    fn frozen_statement_slice(&self, stmt: &internal::Statement<'_>) -> (Span, bool) {
        let span = stmt.span();
        let terminator = Self::statement_terminator(stmt);
        if terminator == StatementTerminator::Never {
            return (span, false);
        }
        ...
        if content_end == span.end {
            ...
            return (span, terminator == StatementTerminator::Always);
        }
