...
Ships the `tsv` binary (defined in `Cargo.toml` `[[bin]]`). Depends on `tsv_ts`, `tsv_css`, and `tsv_svelte` (each with `convert` features) plus `argh` — nothing else. `tsv_debug` depends on this crate and reuses its `cli::` library surface for its own command set; keep that surface stable.
...
- `tsv format <paths...|--content|--stdin> [--parser <type>] [--check] [--jobs <n>]` — formats paths in place (writes only when output differs); `--content`/`--stdin` print to stdout
...
- `cli::discover::discover_files()` — file/dir expansion with the formattable-extension filter and default dir excludes; returns `Discovered { files, errors }` (non-fatal traversal errors), `Err` per unresolvable arg
