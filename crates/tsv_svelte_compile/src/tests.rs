...
fn compile_class_directive_ordering() {
    ...
    let js =
        compile_js("<script>let x = $state(true);</script>\n<div id=\"a\" class:x={x} title=\"b\">t</div>");
    assert!(
...
#[test]
fn compile_class_directive_with_style_directive_refuses() {
    // `class:` alongside a still-refused `style:` directive refuses via the
    // sibling (a temporary over-refusal until the `style:` slice lands).
    assert_unsupported(
        "<script>let x = $state(true);</script>\n<div class:active={x} style:color={x}>t</div>",
        "non-plain attribute (directive/spread)",
    );
}
