...
fn compile_shadowing_js_local_assignment_compiles() {
    ...
    assert_compiles("<script>const x = 1; function f(x) { x = 2; return x; }</script>");
    ...
    assert_compiles("<script>import {a} from './m.js'; const g = (a) => { a = 1; };</script>");
    // A plain nested block `let`.
    assert_compiles("<script>const b = 1; function f() { { let b = 0; b = 2; } }</script>");
    // A `catch` parameter, and a `for` head binding.
    assert_compiles(
        "<script>const e = 1; function f() { try { g(); } catch (e) { e = null; } }</script>",
    );
    assert_compiles(
        "<script>const i = 1; function f() { for (let i = 0; i < 2; i++) {} }</script>",
    );
}
