...
impl Refusal {
    ...
    pub fn bucket_key(&self) -> Cow<'static, str> {
        match self {
            ...
            Self::ScriptInvalidAttributeValue => Cow::Borrowed(
                "<script module> attribute with a value (must be boolean — the oracle rejects it)",
            ),
            Self::ScriptReservedAttribute { .. } => {
                Cow::Borrowed("reserved <script> attribute {name} (the oracle rejects it)")
            }
            ...
            Self::PropsInvalidPattern => Cow::Borrowed(
    ...
    pub fn every_variant() -> Vec<Self> {
        vec![
            ...
            Self::ScriptInvalidAttributeValue,
            Self::ScriptReservedAttribute {
                name: "{name}".to_string(),
            },
...
mod tests {
    ...
    fn deliberate_fences_are_the_legacy_syntax_only() {
        ...
        assert!(!Refusal::CssNestedRule.is_deliberate_fence());
    }
}
