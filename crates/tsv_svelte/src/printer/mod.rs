...
impl<'a> Printer<'a> {
    ...
    /// gap known to hold no comment ([`Self::gap_known_comment_free`]), else the outlined
    /// search — the same inline gate over one `#[inline(never)]` body the `tsv_ts` printer
    /// spells (see `CommentFreeWindow`).
    #[inline]
