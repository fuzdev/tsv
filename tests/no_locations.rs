...
fn assert_ts(src: &str) {
    ...
    let mut full = tsv_debug::json::wire_value(&tsv_ts::convert_ast_json_bytes(&ast, src));
...
fn assert_svelte(src: &str) {
    ...
    let mut full = tsv_debug::json::wire_value(&tsv_svelte::convert_ast_json_bytes(&ast, src));
    strip_locations(&mut full);
    let no_loc: Value =
