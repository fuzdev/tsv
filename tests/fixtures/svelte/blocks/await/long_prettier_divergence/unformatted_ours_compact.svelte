<div>
	...
	{#await getData(aaaaaaaaaaaaaaaaaaa,bbbbbbbbbbbbbbbbbbb,ccccccccccccccccccc,ddddddddddddddddddd) then r}{r}{/await}

	<!-- member-call (.filter) with arrow body: call breaks open, ) dedents, } dangles (divergence) -->
	{#await a.filter((item)=>item.a&&item.b&&item.c&&item.d&&item.e&&item.f&&item.g&&item.h&&item.i) then item}{item}{/await}

	<!-- binary chain expression: wraps, } dangles (divergence) -->
	{#await aaaaaaaaaaaaaaaa&&bbbbbbbbbbbbbbbb&&cccccccccccccccc&&dddddddddddddddd&&eee then r}{r}{/await}
	...
	{#await a.filter((item)=>item.x).filter((item)=>item.yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy) then r}{r}{/await}

	<!-- 3+ group member chain: always wraps regardless of width (divergence) -->
	{#await a.filter((item)=>item.x).map((item)=>item.y).filter((item)=>item.z) then r}{r}{/await}
	...
	{#await getData(aaaaaaaaaaaaaaaaaaa,bbbbbbbbbbbbbbbbbbb)}loading{:then value}{value}{:catch error}{error}{/await}
</div>
