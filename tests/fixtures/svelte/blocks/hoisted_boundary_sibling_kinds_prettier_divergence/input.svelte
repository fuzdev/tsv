...
<div><span>inline1</span>{@debug expr}</div>
<div><Comp />{@debug expr}</div>
<div>{expr}{@debug expr}</div>
<div>{@html expr}{@debug expr}</div>
<div>{@render fn()}{@debug expr}</div>

<!-- leading edge -->
<div>{@debug expr}<span>inline1</span></div>
<div>{@debug expr}{expr}</div>
...
{#each items as item}{expr}{@debug expr}{/each}

<!-- the text sibling this extends -->
<div>text1{@debug expr}</div>
