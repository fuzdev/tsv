<div>
	...
	{#key a.filter((item)=>item.x).filter((item)=>item.yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy)}{x}{/key}

	<!-- 3+ group member chain: always wraps regardless of width (divergence) -->
	{#key a.filter((item)=>item.x).map((item)=>item.y).filter((item)=>item.z)}{x}{/key}
</div>
