<div>
	...
	{#key fn00000(aaaaaaaaaaaaaaaaaaa, bbbbbbbbbbbbbbbbbbb, ccccccccccccccccccc, dddddddddddddddddddd)}
		{x}
	{/key}

	<!-- member-call (.filter) with arrow body: call breaks open, ) dedents, } dangles (divergence) -->
	{#key a.filter((item) => item.a && item.b && item.c && item.d && item.e && item.f && item.g && item.h && item.i)}
		{x}
	{/key}

	<!-- binary chain expression: wraps, } dangles (divergence) -->
	{#key aaaaaaaaaaaaaaaa && bbbbbbbbbbbbbbbb && cccccccccccccccc && dddddddddddddddd && eeeeeeeeeeeeeeee && ffff}
		{x}
	{/key}
