<div>
	...
	{#each fn00000(aaaaaaaaaaaaaaaaaaa, bbbbbbbbbbbbbbbbbbb, ccccccccccccccccccc, ddddddddddddddddddd) as item}
		{item}
	{/each}

	<!-- member-call (.filter) with arrow body: call breaks open, ) dedents, } dangles (divergence) -->
	{#each a.filter((item) => item.a && item.b && item.c && item.d && item.e && item.f && item.g && item.h && item.i) as item}
		{item}
	{/each}

	<!-- binary chain expression: wraps, } dangles (divergence) -->
	{#each aaaaaaaaaaaaaaaa && bbbbbbbbbbbbbbbb && cccccccccccccccc && dddddddddddddddd && eee as item}
		{item}
	{/each}
	...
	{#each a.filter((item) => item.x).filter((item) => item.yyyyyyyyyyyyyyyyyyyy) as item}
		{item}
	{/each}
	...
	{#each a
		.filter((item) => item.x)
		.filter((item) => item.yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy) as item}
		{item}
	{/each}
	...
	{#each fn00000(aaaaaaaaaaaaaaaaaaa, bbbbbbbbbbbbbbbbbbb, cccccccccc) as item}
		{item}
	{:else}
		fallback
	{/each}
