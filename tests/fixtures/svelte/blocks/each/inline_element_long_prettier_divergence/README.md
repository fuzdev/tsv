...
A `{#each …}` whose head exceeds printWidth, placed inside an **inline** element
(`<span>`). tsv applies the same layout as at block level — the head wraps, its `}`
dangles, and the body + `{/each}` drop to their own lines — while the element hugs the outer boundary
...
- [each/long](../long_prettier_divergence/) — the same divergence standalone (block level)
