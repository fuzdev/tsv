...
splitting its attributes, so an over-wide component goes to its own line. The boundary before the
component is a collapsible space, so the word before it stays on the text line. See
