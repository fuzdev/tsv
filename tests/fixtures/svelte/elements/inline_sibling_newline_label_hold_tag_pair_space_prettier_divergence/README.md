...
one of them (`output_prettier.svelte`). The control is a wordless pair (`{expr1} {expr2}`, no
...
only whether the run holds a word at all — is there a fill for this separator to sit in —
...
layout keyed on nothing the author wrote. Only a run with no word in it keeps the bare `line`
