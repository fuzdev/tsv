# inline_attrs_multiline_content_prettier_divergence
...
reflows it. Here the text after a void `<input>` rejoins its line. The separator's **presence** still decides layout (a glued boundary is never
split, since breaking there would inject a rendered space); only space↔newline is reshaped.

This fixture is scoped to the diverging case alone; the multiline-attribute cases it used to bundle stay a
plain (non-divergence) fixture in [inline_attrs_multiline_content](../inline_attrs_multiline_content/).
