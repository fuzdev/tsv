# root_text_newline_inline_prettier_divergence
...
reflows it. Here a root-level text node and the `<span>` after it join one line. The separator's **presence** still decides layout (a glued boundary is never
