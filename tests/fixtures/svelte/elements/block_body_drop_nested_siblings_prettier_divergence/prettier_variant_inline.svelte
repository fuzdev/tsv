...
<nav>
	<a href={root}>Home</a>{#each path_pieces as piece (piece)}<a
			href={base + piece.path}
			class={{ selected: piece.selected }}>{piece.name}</a
		>{/each}
</nav>
...
<p>
	<span class="label">Status</span><span class="value"
		>{#if loading}<Spinner size={spinner_size} variant={spinner_variant} />{:else}<Badge
				tone={badge_tone}>{badge_text}</Badge
			>{/if}</span
	>
</p>
