...
<Comp
	><span>text</span>{#await promise}<style>
			.class {
				color: red;
			}
		</style>{/await}</Comp
...
<a href="value"
	><span>text</span>{#await promise}<Comp
			prop1={value1}
			prop2={value2}
			prop3={value3}
			prop4={value4}
...
<Comp
	>text <span>text</span>{#await promise}text{:then value}<pre>text1
text2</pre>{/await}</Comp
>

<!-- a {:catch} branch forces it open (a nested <style>) -->
<span
	><span>text</span>{#await promise}text{:catch error}<style>
			.class {
				color: red;
			}
		</style>{/await}</span
...
<Comp
	><span>text</span>{#snippet fn()}<style>
			.class {
				color: red;
			}
		</style>{/snippet}tail</Comp
>

<!-- a glued run: only the run's last closing > dangles -->
<Comp
	><span>inline1</span><span>inline2</span>{#await promise}<style>
			.class {
				color: red;
			}
		</style>{/await}</Comp
