...
<pre><Comp prop={value} onhover={(node)=>{handle(node);}}/></pre>

<!-- Nested through code > if > ul: depth accumulates, attrs at 5 tabs, `/>` at 4 -->
<pre>
	<code>
		{#if cond}
			<ul>
				<Comp prop1={value1} prop2={value2} onhover={(node)=>{if(node===null){handle(node);}}}/>
			</ul>
		{:else}
			<p>text</p>
		{/if}
