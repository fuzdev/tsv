# adjacent_if_block_render_tag_long_prettier_divergence

A `{@render}` tag glued to an `{#if}` block (`{@render menu(…)}{#if …}…{/if}`, no
whitespace between) whose construct exceeds printWidth — the same class as
