...
An **own-line JSDoc cast comment in a class member's computed key** — a property and a
method:
...
(`input.svelte`, `[/** @type {A} */ (k1)] = 1;`), matching what a plain block comment in
the same gap already does.
...
pass emits the broken mid-line form for both members
(`prettier_intermediate_to_variant_own_line.svelte`), and its second pass reflows the
**property** key to the glued form while holding the **method** key broken — a mixed
stable form pinned as `prettier_variant_break.svelte`, which tsv normalizes whole to
`input.svelte`. `unformatted_ours_own_line.svelte` and `unformatted_ours_break.svelte`
are the own-line and mid-line authorings: tsv normalizes both to `input.svelte` in one
pass.
