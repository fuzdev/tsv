<script lang="ts">
	...
	const b1 = fn<0.5>(t, u);
	const b2 = fn<0.5 | 0.25>(t);
	const b3 = fn<-0.5>(t);
	const b4 = fn<A[0.5]>(t, u);
	const b5 = fn<keyof 0.5>(t, u);
	...
	const c1 = x < 0.5 ? q : r > `t`;
	const c2 = x < 0.5 + 1 > (t, u);
	const c3 = x < (0.5).toString() > (t, u);
	const c4 = x < a[0.5 + 1] > (t, u);
</script>
