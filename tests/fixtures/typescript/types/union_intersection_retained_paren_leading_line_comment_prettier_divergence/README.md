...
the comment was dropped entirely — a content-loss SAFETY bug: `build_parenthesized_union_doc`
only emits block comments in the paren gaps, and the first-member path has no
previous member to relocate the line comment onto.)
