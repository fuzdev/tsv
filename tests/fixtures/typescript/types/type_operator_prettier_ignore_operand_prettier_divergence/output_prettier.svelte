<script lang="ts">
	...
	type A = keyof // prettier-ignore
	{  a: 1  };

	type B = readonly // prettier-ignore
	{  b: 2  }[];

	type C = readonly // prettier-ignore
	[  1,  2  ];
	...
	type G = typeof // prettier-ignore
	h . i;

	type J = typeof // prettier-ignore
	k/* l */ <{ m: 3 }>;
	...
	type AA = typeof // prettier-ignore
	bb<Cccccccccccccccccccc, Dddddddddddddddddddd, Eeeeeeeeeeeeeeeeeeee, Ffffffffffffffffffff, Gggg>;

	// `infer` is the same gap once more. Its child is the type PARAMETER, so a constraint
	// rides inside the freeze.
	type T = U extends infer // prettier-ignore
	V extends {  w: 8  }
		? 1
		: 2;
