<script lang="ts">
	...
	type LongNestedInfer<T> =
		T extends Promise<infer OuterLongType>
			? OuterLongType extends Promise<infer InnerLongType>
				? InnerLongType
				: OuterLongType
			: T;

	// Function params with infer - params break to multiple lines
	type LongConstraintInfer<T> = T extends (
		first: infer FirstParam,
		...rest: infer RestParams
	) => infer ReturnType
		? [FirstParam, RestParams, ReturnType]
		: never;

	// Chained conditionals in false branch - each level wraps
	type ChainedInfer<T> =
		T extends Promise<infer LongOuterResult>
			? LongOuterResult
			: T extends Array<infer LongArrayElement>
				? LongArrayElement
				: T;

	// Deeply nested (3 levels) - each conditional wraps
	type DeeplyNestedInfer<T> =
		T extends Promise<infer LongOuter>
			? LongOuter extends Array<infer LongMiddle>
				? LongMiddle extends object
					? LongMiddle
					: never
				: LongOuter
			: T;
	...
	type BothBranchesInfer<T> =
		T extends Promise<infer LongResultType>
			? LongResultType extends Error
				? LongResultType
				: LongResultType
			: T extends Array<infer LongElementType>
				? LongElementType
				: never;
</script>
