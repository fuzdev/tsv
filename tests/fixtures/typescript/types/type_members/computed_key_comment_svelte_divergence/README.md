# Parser divergence: comment duplication in a type-member computed method's `[ … ]` key

A comment inside a type-member computed method's `[ … ]` key brackets falls in a region acorn-typescript re-parses — its backtrack-and-reparse
