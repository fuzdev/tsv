...
For a comment **before the key** and **after the key** (both interface and
type-literal contexts), acorn-typescript's backtrack-and-reparse duplicates the
comment in the root `comments` array; our parser keeps a single entry. The AST
is semantically equivalent (`ast_diff` confirms match) and the after-colon and
before-`]` positions match canonical exactly — the difference is only the
duplicate count for those two in-bracket positions.
