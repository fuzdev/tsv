...
tsv preserves that placement rather than collapsing it onto the keyword line. A
comment that is *already* on the keyword line (`U = // c\n…`) tsv emits inline via
`line_suffix` (zero width, so a long trailing comment never forces a preceding
