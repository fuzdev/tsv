...
**tsv**: keeps the member (and the block comment's continuation lines) flush
under the `|`, using whole-tab indentation only:
...
`tabs + 2 spaces` under `--use-tabs`, so the member and the comment continuation
lines sit two columns past the `|`:
...
emitting prettier's `align(2)` sub-tab offset. At `tabWidth = 2` the visual
result is equivalent; only the representation differs. The fixture also covers a
