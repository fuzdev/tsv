...
A comment inside a mapped type's `{ [K in … ] }` header (before the `in`) falls
in a region acorn-typescript re-parses — its backtrack-and-reparse fires the
`onComment` callback twice, so the comment is duplicated in the root `comments`
array. Our parser keeps a single entry (`expected_ours.json` vs
...
`[K in …]` line (`BInline`), while an own-line leading comment (`A`/`B`) keeps its
own line. See
