...
**Prettier 3.9**: also breaks the `[K in T]` brackets but additionally drops
`K in T` to its own indented line, leaving `[` alone:
...
the key-side counterpart of [mapped_value_line_comment](../mapped_value_line_comment_prettier_divergence/)
(the comment after `:`).
