<script>
	...
	fn1(bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb, (a) => (b) => ({
		c: 1,
		d: 2
	}));

	// 101 - an array terminal hugs the same way, with no parens added around the array
	fn1(bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb, (a) => (b) => [
		c,
		d,
		e
	]);
	...
	fn1('first', (argument1) => (argument2) => (argument3) => (argument4) => (argument5aaaaaaaa) => ({
		c: 1
	}));
	...
	fn1(
		'first',
		(argument1) => (argument2) => (argument3) => (argument4) => (argument5aaaaaaaaa) => ({ c: 1 })
	);

	// a destructured head, which forces the chain layout open elsewhere, still hugs here
	fn1('first', ({ a }) => ({ b }) => ({
		c: 1,
		d: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
	}));
	...
	fn1(
		...
		({ aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa }) =>
			({ bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb }) => ({ c: 1 })
	);
</script>
