<script>
	...
	obj.method((a) => (b) => ({
		c: 1,
		d: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
	}));
	...
	obj.method((argument1) => (argument2) => (argument3) => (argument4) => (argument5aaaaaaaaaa) => ({
		c: 1
	}));

	// one char over (101) - the argument breaks out and the chain progressive-indents
	obj.method(
		(argument1) => (argument2) => (argument3) => (argument4) => (argument5aaaaaaaaaaa) => ({ c: 1 })
	);
