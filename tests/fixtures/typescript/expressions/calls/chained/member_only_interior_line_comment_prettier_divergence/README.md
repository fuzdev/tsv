...
- **prettier**: hoists the own-line comment onto its own line after `=`
...
into the line below — which would merge two comments onto one line
(`foo.bar.baz; // c2 // c1`) and lose information. tsv breaks the chain instead,
applying the [comment-position
