# value_block_comment_break_prettier_divergence

A block comment glued to an object property's `:`, with the value authored on the next line.
...
**Prettier** reflows too, but hoists the comment onto its own line above the property:
...
The **own-line** authoring (`k:⏎/* c */⏎1`) is pinned separately as `variant_own_line.svelte` —
also dual-stable, and a *different* fixed point from input: with a newline before it the comment
leads its own line, so nothing glues it to the `:` and both formatters break after the `:` and
keep it there. Carrying the whole authoring axis (glued / own-line) is deliberate: the two take
