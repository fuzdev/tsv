...
A comment between the `async` keyword and the generator `*` of an object-literal
method is preserved at the author's position. Prettier relocates it to after the
`*` (before the name). Same divergence as the class-method form
(`../../../statements/class/async_star_comment_prettier_divergence/`); both call
sites share the delimiter scan that finds the generator `*`.
