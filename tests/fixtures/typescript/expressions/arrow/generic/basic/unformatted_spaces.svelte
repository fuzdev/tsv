<script lang="ts">
  ...
  const   withBoth   =   < T   
  extends   string   =   'default' > ( x :  T ) :  T   =>   x ;
  ...
  T ,  
  U   extends   T , 
   V   =   number > ( x :  T ,  y :  U ,  z :  V )   =>   [ 
    x ,  y ,  z ] ;

  // Complex constraint (object type)
  const   objectConstraint   =   < T   extends   { foo :  string } > ( 
    x :  T ) :  T   =>   x ;

  // Complex constraint (function type)
  const   fnConstraint   =   < T   extends   ( x :  string )   =>   number > ( f
   :  T ) :  T   =>   f ;

  // Generic arrow with conditional type in default
  const   conditionalDefault   =   < T ,  U   extends   
  T   =   T   extends   string   ?   T   :   never > ( x :  T )   =>   x ;
  ...
  const   complexArg   =   withConstraint
   <
   'literal' > ;
  ...
  const   nestedInst   =   ( 
    < 
  T  
   extends   string > ( x :  T ) :  T   =>   x 
  ) < 
  string
   > ;

  // Instantiation in conditional
  const   conditional   =   true   ?   withConstraint 
