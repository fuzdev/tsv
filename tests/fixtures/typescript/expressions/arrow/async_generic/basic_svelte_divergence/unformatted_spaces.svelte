<script lang="ts">
	...
	const   multipleComplex   =   async   	< T   extends   object ,   U   =  
	 T > ( )   
	 =>   { } ;
	...
	const   unionConstraint   =   async   < T   extends   string   |  
	 number > ( )   =>   { } ;

	// Intersection type constraint
	const   intersectionConstraint   =   async   < T   extends   A  
	 &   B > ( )   =>   { } ;

	// Nested generics in constraint
	const   nestedGenerics   =   async   < T   extends   Array < 
	U > ,   U > ( )   =>   { } ;

	// Template literal type constraint
	const   templateLiteral   =   async   < T   extends   `prefix_${ 
	string }` > ( )   =>   { } ;

	// keyof constraint
	const   keyofConstraint   =   async   < T   extends 
	  object ,   U   extends   keyof   T > ( )   =>   { } ;
	...
	const   withOptional   =   async   < T , > ( x 
	? :   T ) :   Promise < T   |   undefined >   =>   x ;

	// With rest parameter
	const   withRest   =   async   < T , > ( ...   args :  
	 T [ ] ) :   Promise < T [ ] >   =>   args ;

	// Array return type
	const   arrayReturn   =   async   < T ,
	 > ( ) :   Promise < T [ ] >   =>   [ ] ;

	// Tuple return type
	const   tupleReturn   =   async   < T ,   U > ( x 
	:   T ,   y :   U ) :   Promise < [ T ,   U ] >   =>   [ x ,   y ] ;
