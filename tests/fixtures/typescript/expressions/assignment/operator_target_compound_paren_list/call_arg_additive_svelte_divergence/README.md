# compound assignment to an additive call argument - Svelte divergence

`g(a + b >>= c)` assigns to an addition: the bare statement `a + b >>= c` has no reading under any parser, but as a call argument acorn-typescript accepts it, building an
