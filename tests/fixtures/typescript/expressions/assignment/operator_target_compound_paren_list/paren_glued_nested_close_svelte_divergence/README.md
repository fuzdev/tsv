# compound assignment through a glued nested type-argument close in parens - Svelte divergence

`x = (f<A<B>>=c)` assigns to a comparison: the outer close is glued to `=`, so the longest punctuator swallows it, neither list closes, and the source reads `f < A < B >>= c`. In parens acorn-typescript accepts it, building an
`AssignmentExpression` (`((f < A) < B) >>= c`) whose `left` is a `BinaryExpression` —
