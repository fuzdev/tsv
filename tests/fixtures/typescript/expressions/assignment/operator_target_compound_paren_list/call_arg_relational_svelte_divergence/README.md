# compound assignment to a relational call argument - Svelte divergence

`g(a < b >>= c)` assigns to a comparison: the first `>` of `>>=` is not a type-argument close (the longest punctuator swallows it), so this is `a < b` followed by `>>=`. As a call argument acorn-typescript accepts it, building an
`AssignmentExpression` (`(a < b) >>= c`) whose `left` is a `BinaryExpression` —
