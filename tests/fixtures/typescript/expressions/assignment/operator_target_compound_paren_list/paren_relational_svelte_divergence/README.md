# compound assignment to a parenthesized relational - Svelte divergence

`(a < b >>= c)` assigns to a comparison: the first `>` of `>>=` is not a type-argument close, so this is `a < b` followed by `>>=`, wrapped in parens. There acorn-typescript accepts it, building an
