<script lang="ts">
	...
	const gen1 = () => {
		return `
		export const X = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
	`;
	};
	...
	const gen2 = () => {
		return `
			export const Y = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
		`;
	};
	...
	const gen3 = () => {
		return `
${aaaaaaaa.bbbbbbb_cccccccc_ssssssssssssssssssssssssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}`;
	};

	// Case 4: No newline before ${, content line at 100 chars (fits)
	const gen4 = `${aaaaaaaa.bbbbbbb_cccccccc_ssssssssssssssssssssssssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}`;

	// Case 5: No newline before ${, content line at 101 chars (wraps)
	const gen5 = `${aaaaaaaa.bbbbbbb_cccccccc_sssssssssssssssssssssssssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}`;

	// Case 6: 4 spaces template indent → base 2, content 3 tabs, closing 2 tabs
	const gen6 = () => {return `
    export const A = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
	`;
	};
	...
	const gen7 = () => {
		return `
      export const B = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
	`;
	};
	...
	const gen8 = () => {
		return `
   export const C = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
	`;
	};
	...
	const gen9 = () => {
		return `
	  export const D = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
	`;
	};
	...
	const gen10 = () => {
		return `
				export const E = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
	`;
	};

	// Case 11: 1 tab template indent (same as script base indent)
	const gen11 = `
	export const F = ffffff([${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]);
`;
	...
	const gen13 = () => {
		return `
		     deeply indented content [${aaaaaaaa.bbbbbbb_cccccccc_sssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]
	`;
	};
	...
	const gen14 = () => {
		return `
    		deeply indented content [${aaaaaaaa.bbbbbbb_cccccccc_ssssssssssssssssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]
    `;
	};
	...
	const gen15 = () => {
		return `
    	  deeply indented content [${aaaaaaaa.bbbbbbb_cccccccc_sssssssssssssssssssssss.map((ssss) => `'${ssss.tttttt}'`).join(',')}]
    `;
	};
