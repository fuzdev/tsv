<script lang="ts">
	...
	const gen1 = () => {
		return `
		export const X = ffffff([${

			aaaaaaaa
			.bbbbbbb_cccccccc_sssss
			.map((ssss) => `'${ ssss.tttttt}'`).join(',')
...
${
	...
	const gen4 = `${
		...
		aaaaaaaa
		.bbbbbbb_cccccccc_ssssssssssssssssssssssssss.map((ssss) => `'${ ssss.tttttt}'`).join(',')

	}`;

	// Case 5: No newline before ${, content line at 101 chars (wraps)
	const gen5 = `${
		aaaaaaaa.bbbbbbb_cccccccc_sssssssssssssssssssssssssss
			.map((ssss) => `'${ ssss.tttttt}'`)
			.join(',')
	}`;
	...
	const gen14 = () => {
		return `
    		deeply indented content [${
					...
					aaaaaaaa.bbbbbbb_cccccccc_ssssssssssssssssss.map((ssss) => `'${ ssss.tttttt}'`).join(',')
	...
	const gen15 = () => {
		return `
    	  deeply indented content [${
					...
					aaaaaaaa.bbbbbbb_cccccccc_sssssssssssssssssssssss
						.map((ssss) => `'${ ssss.tttttt}'`)
						.join(',')
										...
										aaaaaaaa.bbbbbbb_cccccccc_sssss
											.map((ssss) => {
												...
												return `{
													...
													e: '${
														ssss.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa().bbbb().ccccccccccc()
													}',
													// At 107 chars - exceeds, breaks
													e: '${
														ssss.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa().bbbb().ccccccccccc()
													}',
