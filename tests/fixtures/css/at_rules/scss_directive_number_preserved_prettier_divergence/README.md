...
whose params it re-parses with `parseValue`; its `value-number` nodes then print
through `printCssNumber` + `printUnit`, which is what canonicalizes the number.
tsv applies that same number normalization only to standard-CSS contexts —
declaration values and `@media`/`@supports`/`@import` preludes — leaving
