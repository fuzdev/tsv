...
See [conformance_prettier_css.md §CSS: At-Rules](../../../../../docs/conformance_prettier_css.md#css-at-rules) (`@import line wrap`, Print width). tsv uses a consistent >100 threshold across all CSS constructs. Prettier has an off-by-one quirk for `@import` media conditions, same pattern as `@media`, `@supports`, and `@container`.
