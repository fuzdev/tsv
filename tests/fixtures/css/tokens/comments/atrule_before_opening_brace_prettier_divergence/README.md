...
Stable quirk. tsv normalizes comment spacing in the contexts whose grammar it parses (`@media`/`@supports` preludes, selectors, declaration values). Prettier adds space after `*/` but not before `/*` in this position, despite normalizing both in other contexts. (Raw at-rule preludes — `@layer`/`@keyframes`/`@namespace`/… — are kept verbatim instead, matching prettier, so they are *not* a divergence.) See [conformance_prettier_css.md §CSS: Comments](../../../../../../docs/conformance_prettier_css.md#css-comments).
